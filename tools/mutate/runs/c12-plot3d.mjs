...
const CMD = "npx vitest run test/unit/plot-plot3d.test.ts";
...
const results = await runPass({
  ...
  control: {
    ...
    // **Re-anchored at the compose rather than inside it** (F496). `halfRows`
    // held one blank branch and the control sat on it; the silhouette alphabet
    // replaced it with `mixedRows`, whose blank paths are three. Emptying the
    // arm's output is the same control and cannot go stale the next time those
    // branches move.
    from: "    half ? mixedRows(ink, depth, kind, w, rows, ctx)",
    to: "    half ? mixedRows(ink, depth, kind, w, rows, ctx).map((l) => l.map(() => ({ text: \" \" })))",
  ...
  mutations: [
    ...
    {
      // **The sparse raster's third glyph gone** (F443), re-anchored at the
      // branch that now draws it (F498).
      //
      // A cell inked only below becomes the *upper* half block with a
      // foreground, so the terminal paints the top half in the mark's colour — a
      // solid bar where there is one sample. `HALF_BLOCK_LOWER` is gone and
      // `QUADRANT[12]` is the same character, reached from the branch that draws
      // a **mark**.
      //
      // **The first re-anchor pointed this at `areaGlyph` and it survived**,
      // which is the finding worth keeping: the silhouette alphabet *looks* like
      // the successor to the half-block code, and for a **point** it is not.
      // Points and surfaces stopped sharing a kind in the same session, so the
      // successor for a cloud is the `MARK` branch and every row in this run
      // draws a cloud. A mutation re-pointed at the mechanism that resembles the
      // old one, rather than at the one its rows exercise, cannot be caught by
    ...
    },
    {
      ...
      name: "plot3d takes scatter's style arms",
      file: T,
      from: '  plot3d: ["braille", "line", "marker"],',
      to: '  plot3d: ["candlestick", "solid"],',
      expect: "SC2",
    },
    {
