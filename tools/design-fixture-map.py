...
M = {
 ...
 70:("owed","M2","/colour and /label, tinting the chrome — **no longer parked; the question that held it was wrong about the registry.** Parked 16 read *no palette anywhere in the registry* and searched for a **record** carrying all ten hues; the registry stores them as **theme rules** — 300 hue tokens, `c-h` 100, `bg-h` 100, `c-hi` 100, ten hues in every one of the ten themes including `mono` and both high-contrast pairs. A matcher that sees one encoding reports absence when the value changes form, and M2's own plan text names what it missed: *10 `h-*`/`hi-*` hue pairs*. **And §093 settles the order with its argument** — `blue orange cyan pink lime violet yellow green red purple`, by **perceptual separation** rather than spectrally, because the first assignment was *red, orange, yellow, lime, green — five identities a deuteranope cannot separate*, and *the first four are separable under every common deficiency, which is where a session with three subagents lands*. **So what is owed is the port, not a decision**: the hexes appear nowhere in `src/`, `test/` or `docs/components/`, and `categorical` is a separate eight-slot cycle (C10 I37), not these. **The remainder, named and genuinely the application's**: `/colour` and `/label` are verbs over `.calcium/config.toml`, and the contrast question the section gates a literal hex behind is M15's question work — neither is the palette"),
