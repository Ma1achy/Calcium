...
M = {
 ...
 70:("owed","M2","/colour and /label, tinting the chrome — **the palette landed and the resolver cannot reach it.** Parked 16 read *no palette anywhere in the registry* and searched for a **record** carrying all ten hues; the registry stores them as **theme rules** — 300 hue tokens, `c-h` 100, `bg-h` 100, `c-hi` 100, ten hues in every one of the ten themes including `mono` and both high-contrast pairs. A matcher that sees one encoding reports absence when the value changes form. **§093 settles the order with its argument** — `blue orange cyan pink lime violet yellow green red purple`, by **perceptual separation** rather than spectrally, because the first assignment was *red, orange, yellow, lime, green — five identities a deuteranope cannot separate*. **The port landed** as C10 I53: `ThemeTokens.hues`, ten hues × three tiers × ten themes, emitted per theme with T2.53 holding it against the registry by equality both ways and T2.54 asserting §070's derivation of the band ink — 100 of 100. **What is owed now is the seam, and it is the same defect one layer out.** Measured 2026-09-24 against `dist/`: `t.tokens.hues` carries all ten, and `resolve('hue.blue', theme, {colourDepth:24})` returns `{}`. The theme's palettes are `tone categorical syntax spectrum`; there is no `hue`, and `ColourRef` is `palette.slot`, so **the one resolver every block goes through cannot name a hue.** A block names a palette slot and C10 resolves it — that is the rule the whole tree is built on — and these values sit beside the palettes rather than in them. Carried and unreadable, which is what I53 was written about; it was true of the generator and it is now true of the resolver. The shape the fix takes is not a new mechanism: the ink tier is a `PaletteSpec`, the ground tier is a surface, and the `on` tier is **`bandInk`** — the ink for everything on a band, which `inkOn` already consults and which is exactly what `c-hi` means. **The remainder, named and genuinely the application's**: `/colour` and `/label` are verbs over `.calcium/config.toml`, and the contrast question the section gates a literal hex behind is M15's question work — neither is the palette"),
 71:("owed","Q18","the five permission postures \u2014 **reclassified from `prose`, and the second classification the sweep caught.** \u00a7071 is not a rule table: it draws the footer in each of five postures, measures each symbol against a named trap (`\u23f8` rejected because *it IS an emoji \u2014 the `\u23fa` trap again*, `\u203c` and `\u26a1` likewise), and rules the channel \u2014 *NOT ONE of them is painted*, because the error tag is the only painted label that means a status, so the distinction is the channel and not the hue. `\u00bb` here is the mode line's lead, which is the second r\u00f4le M4's ruling put on the record (`glyphs.ts:213`). **Blocked on parked question 18**: the registry's `permissionPostures` names `ask` where \u00a7071 names `skip`, and `skip` is in the registry only inside `sectionBlocks` text \u2014 the two normative sources disagree on the member that means *no permission check at all*"),
 ...
 105:("owed","Q20","five primitives the harness never needed — **reclassified from `prose`, and the section's own claim is what the row checks.** §105 says *each is built from what already exists* and *the framework already had every part — what it lacked was the name*. Checked part by part, that holds, and **none of the five is a kind**. The TREE: guides are `decoration` and the twisty is `content`, which is `shed.ts`'s `Tier` exactly, and the depth cap is §93's floor rule rather than a constant — the shed step is built, the kind is not. The FORM: *an error REPLACES the hint rather than joining it*, on the same `✗` and the same tone as every other failure, so it introduces no visual idea; `✗` is the registry's `failure` and the tone is C10's. The SPLIT: *the divider is a CONTROL*, drawn `│` with `┃` on the focused pane — which is the **scrollbar's own pair**, `SCROLLBAR_UNICODE_SET.track` and `.thumb` (`| #` at the ASCII rung), so the claim is literally true and the set degrades whole by §021's rule already. The PALETTE: *nothing new: a PANEL (§96), a LADDER, and a RESIDUE* — all three ship, and its `+58 more` is the container residue row, whose form the tree already draws (`⋯ 14 unchanged lines`). The TOAST: transient, takes no keys, and *what a toast announces is also in the record*, which is §13's argument about notifications and not a new one. **Why it is owed and not merely unbuilt**: the five are new block kinds, the sixteen-MR plan names none of them, and §105's own opening is that *a general app wants these and the agent surface does not, so they were missing rather than refused*. Whether this reconciliation builds them is a scope question and it is parked as 20 rather than answered by building. **The remainder, named**: §105's `⌥←→ from either side` for the divider is the scrollbar's rule on the other axis, and the scrollbar has no such chord either — so the split would settle a binding for both"),
