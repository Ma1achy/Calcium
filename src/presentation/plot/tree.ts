...
type Grid = Readonly<{ text: string[][]; mask: number[][] }>;

function grid(rows: number, columns: number): Grid {
  return {
 ...
 */
function write(row: string[], at: number, body: string, caps: Caps): void {
...
function paint(g: Grid, corners: "rounded" | "sharp", caps: Caps): readonly string[] {
...
const setMask = (g: Grid, r: number, c: number, bits: number): void => {
