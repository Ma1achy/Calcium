...
/**
 * The column count that renders at `aspect` beside `rows` rows (C12 §3ab).
 *
 * `squareColumns` generalised, and it is the same direction: the height is
 * declared and the width derived, because C12 I1 permits no other. At a ratio of 1
 * the two agree by construction rather than by two expressions that match.
 */
export function columnsForAspect(rows: number, aspect: number): number {
  if (!Number.isFinite(aspect) || aspect <= 0) return squareColumns(rows);
  return Math.max(1, Math.round(Math.max(0, rows) * CELL_ASPECT * aspect)); // cells-ok — a column count
}
...
//
// **`aspect` did not need it**, which is worth recording because the plan that
// added `aspect` said it would. That member derives a *width* from a declared
// height — `columnsForAspect` above — and `squareRows` derives a height from a
// width, which C12 I1 forbids a plot from doing at all. The entry stands owed
// (C12 §3ab).
