...
~~**Three combinations are refused at parse (I20)**, in I19's shape and for I19's reason:~~ — **struck with the field** (F1253). Each refusal was right and each is kept below as the record of a defect it closed; none has a subject now that `view` is not declarable.
...
**`parseManifest` returns the app's tools plus Calcium's eight**, and this belongs here rather than in C22 because it is a statement about what a manifest *is*.

**And an eighth would be a breaking change for every app already using the name.** I6 makes the collision a parse error at construction, so the app does not start, and no row in the framework's own suite can see it — nothing in `src/` holds an app's manifest. The check is the examples' suites, which is where `/profile` becoming the seventh found both examples declaring their own `profile`: nine rows red in the proof gate for docker-tui, the plots demo refusing to start in its own suite, and nothing red in the framework's 5 841 rows (F953). A verb joins this list with a grep of every manifest in the tree, not after. **`/capabilities` is the eighth** (C22 I125, §107, ruling 29 — *the route appears in `/capabilities`*), and it joined that way: on 2026-09-25 no manifest in the tree declared the name but `framework.ts`'s own row, and the examples' suites ran green with it reserved. It is breaking for any app outside the tree that declares `capabilities`, which is this paragraph's first sentence arriving.

`/help`, `/capabilities`, `/clear`, `/theme`, `/history`, `/debug`, `/exit` and `/profile` are verbs. They have names, they take arguments, they complete, they validate, and they appear in help. Everything the manifest exists to describe is true of them, and the only reason they were absent is that nobody wrote them down. C23 ships the handlers (C23 §2); C18 classifies `local` from the manifest; so without the rows, the handlers are registered for verbs nothing can ever classify to — which is precisely what C23 I27's reconciliation reports, and it is right to.
...
- **I6** — Tool names are unique. Duplicates are a parse error, not a last-wins — **including against the framework's eight** (§3), so an app shadowing `clear` fails at parse rather than silently overriding a verb Calcium's own handlers depend on.
...
- **I20** — **Retired with the field** (F1253). (C22 §13a, R-EXA-082.) It read *`view` is declarable on a `ToolDef` and on a `FlagDef`, and an invocation is a view if either declares it*, refused at parse with `interactive`, `oneShot` and `local`, permitted with `streams`. Every refusal was sound and each closed a real defect — the `local` pair had parsed, sealed, validated, run and appended an ordinary entry with no view and nothing saying so (F1022, closing F23 and F129). **What retires is the thing being declared**: the design rules that a verb's result is an entry, so there is no tier to select, and the field, its two declaration sites, its three refusals and `isViewInvocation` go together. The argument for declaring it *here* rather than on the adapter's document survives as the record of why the pushed view had to be decided before step 3, and it is written at C22 §13a. A manifest still carrying `view` parses: unknown keys are dropped rather than rejected (§`parse`), so the removal is soft on the JSON side and a compile error on the typed one.
...
- **I24** — A cross-field refusal reads every declaration of each field it names, not the tool's. `view` on a flag with `interactive` on the tool was the pair I20 forbade, written the other way, and it parsed (F118) — **the measured instance, kept although both the field and its refusal have retired** (F1253), because the rule is about how a refusal reads its operands and that is unchanged. The pairs it governs now are the ones among `interactive`, `oneShot`, `local`, `shellOnly` and `persist` that declare at both levels. The conservative form is deliberate: an arm resolving `interactive` to `false` beside a `view` flag would be legal, and refusing it costs an app nothing today because no app declares one — **the limit is recorded rather than discovered**, and the first consumer to want the pair is the argument for narrowing it.
...
17. ~~`view` declares that a result is a pushed view rather than a transcript entry~~ — **retired with the field; every result is a transcript entry** (I20, C22 §13a, R-EXA-082).
...
19. `--help` is a flag Calcium reserves on every verb, on the same terms as the eight verbs it reserves — appended, collision-checked, and absent from what round-trips (I22).
...
- ~~**T1.23**~~ — **struck with `view`** (F1253, C22 §13a). It asserted the `local` refusal and I24's flag-side read; both had a field to refuse and no longer do. The controls it carried — a spawned tool declaring `view` parses, a `local` tool declaring none parses — are the shape a cross-field row needs and are kept by T1.19b for the fields that remain.
...
- ~~**T6.15**~~ — **struck with `view`** (F1253, C22 §13a). It reverted I24's refusal to reading the tool's declaration alone. The rule survives and its **subject** does not: I24 is now exercised through the fields that still pair, and the `view`/`interactive` pair that measured F118 is the record rather than the row.
...
- ~~**T6.16**~~ — **struck with `view`** (F1253, C22 §13a). It dropped the `local` arm from the tier's parse refusal. F1022's finding stands as written — the pair parsed, sealed, validated and ran with the only symptom a transcript entry where a view was declared — and a transcript entry is now the correct outcome, which is the cleanest possible statement of why the field was doing nothing.
