...
- **T2.8** (I16): a far side supplying its own `command` loses on every §4 route **and on the identity route by name**, which is the one that regressed — the transcript drew `ps --json`, the spawned form carrying a flag D16 appends and the user never wrote. **The identity route is asserted through `meta.adapter`, not assumed** (F933): `identityDocument` validates rather than sniffs, so a payload short of a whole document is refused and comes back through the fallback, which assigns `command` at its own site — both answer `/ps`, and only the supplied `meta` carried across by I13 tells them apart. Beside it, over the file, because the behavioural half can only reach the routes it enumerated: every `command:` assignment in the registry is `ctx.command`, **as a list compared by equality** — the count as well as the value, since the funnel's assignment is the one a route can lose while every site that remains still spells it (F932).
- **T2.9** (I17): `ProducerContext`'s field set is read from the declaration and compared **by equality** against `width`, `height`, `capabilities`, `measure`. **Not enforced by omission**, because omission did not enforce it — a fifth field arriving is the event this row exists to catch, whatever it is called — and the placement half is asserted as a property rather than a list, since a producer that positions loses on the next resize.
- **T2.10** (I21): an adapter reads `flags` as **values** — `args.raw` is `true`, not `"--raw"` — on an invocation whose `argv` demonstrably lacks `--raw` and carries `--json`, which is the `shellOnly` asymmetry that makes the field necessary. And `userRequestedJson` is shown to be a **second axis** rather than a synonym: with it set the registered adapter does not run at all, so a single field meaning both would make `--raw` do that too.
### Tier 3 — edge cases
