...
@fmx/calcium/launch     prepareLaunch — the loader hooks a launcher installs before the app (I37)
...
**`launch` is production, and it is separate because of *when* it must run rather than what it costs.** Ink imports one name, `throttle`, from `es-toolkit/compat`, and that barrel is 1,319 modules — more than half of a cold import's 2,442, against a hundred for Ink itself (F1192). `prepareLaunch()` registers two synchronous loader hooks (`module.registerHooks`, Node 22.15) that hand Ink the one file in the barrel's place; a redirect has to be installed before the loader reads Ink's import, and a static import of the runtime barrel hoists past anything in the same module, so the call cannot live in `dist/index.js` and cannot be made after it. It lives where R01 R4.6 already put the compile cache: in the launcher, between `enableCompileCache()` and the dynamic import of the app. It is Calcium's entry and not a launcher recipe because the thing it names — Ink's import line and es-toolkit's file layout — is exactly what this surface hides from a consumer; a consumer who never calls it pays the barrel and loses nothing else, which is why it is an entry and not a behaviour of the runtime.
...
**What an entry resolves to** (I38, F1193). Every `default` target is a file under `dist/bundle/`, built by esbuild from the `tsc` tree in one code-split graph with every package external (A04 §5); every `types` target stays on the `tsc` tree's declarations. The loader's unit is the file, and it was two thirds of a cold import over the 820 files `tsc` emits; one chunk per entry is a third of the import and a quarter of the heap after it. A consumer cannot reach the file tree — `exports` has no `./dist/*` pattern, which is A04 §1's seal — so the bundle and the tree are never two instances in one process from outside this repository; inside it, `test/` imports `src/` or `dist/` files by relative path and the examples import the package, and no suite does both.

**`profiling` is not dev-only, and that is the difference from the two above it.** The profiler ships and is off by default, so a consumer reading a `ProfileReport` in production — a `--profile` flag on their own CLI, a support bundle — needs the types at runtime. It is a separate entry rather than part of the root because it is **types plus one enum and no behaviour**: a consumer who never profiles imports none of it, and the root's surface does not grow by twenty names for a feature most sessions never switch on. Its `exports` target is `./dist/bundle/shell/profiling/index.js`, because C28 is L4 and the subpath is flat where the source is not (C22 §2c, C28 §1).
...
36. **A launcher can narrow the renderer's dependency graph before the app imports it, and the narrowing is exact or absent** (I37, F1192, R01 R4.7).
...
- **I37** (F1192) — **`prepareLaunch()` narrows Ink's es-toolkit import to the one module it binds, exactly or not at all.** A `load` hook reads Ink's `build/ink.js` as the loader hands it over and *arms* the redirect only when the source contains the line `import { throttle } from 'es-toolkit/compat';` byte for byte; a `resolve` hook then answers Ink's `es-toolkit/compat` with the resolved barrel's URL rewritten from `compat/index.mjs` to `compat/function/throttle.mjs` — inside whichever copy of es-toolkit the loader resolved — and only when that file is on disk. Any other import text, any other importer, any other specifier, a missing file or a Node without `registerHooks` leaves the barrel in place, so the two failure modes land on the behaviour a consumer who never called it has. The function Ink receives is the same module object the barrel re-exports, so no frame changes by a byte; **the observable is the graph and never a duration** (I36's rule): the process lists under ten modules from `es-toolkit/` armed and over a thousand not.
...
- **T1.12** (I37, F1192): the arming predicate over Ink's source — the exact line arms; the same line binding two names (`import { throttle, debounce } from 'es-toolkit/compat';`), the line with double quotes, or a source without it does not; and the URL rewrite maps `…/es-toolkit/dist/compat/index.mjs` to `…/compat/function/throttle.mjs` and any other URL to itself.
...
- **T5.6** (I36, F1188): a child importing `dist/index.js` under `test/support/import-trace.mjs` loads more than a hundred modules, the barrel among them, and none whose URL is under `beautiful-mermaid/` or `elkjs/`; the same child then imports `dist/mermaid.js`, after which the renderer's bundle is in the list and `mermaidCode` on a two-node flowchart under full capabilities equals the contract corpus's rendering byte for byte — the second import is what makes the first an assertion rather than an absence. `package.json`'s `exports["./mermaid"]` resolves to `./dist/mermaid.js` with its types beside it, and `dist/index.js` names no `mermaidCode`.
- **T5.7** (I37, F1192): a child under `test/support/import-trace.mjs` that calls `prepareLaunch()` from `dist/launch.js` and then imports `dist/index.js` lists fewer than ten modules whose URL is under `es-toolkit/` and none of the barrel's re-exports (`compat/array/chunk.mjs` stands for them); the same child run without the call lists more than a thousand — the fabricated violation in the same row, so the instrument is shown to see the barrel; and the armed child then renders a block through the registry, because a redirect that links and never runs is what a graph row cannot see. `package.json`'s `exports["./launch"]` resolves to `./dist/launch.js` with its types beside it.
- **T5.8** (I38, F1193): a child under `test/support/import-trace.mjs` that calls `prepareLaunch()` from the bundled `launch` entry and imports the bundled runtime lists fewer than four hundred modules and none from `@xterm/headless`, `beautiful-mermaid` or `elkjs`; for each of the six entries the bundled module's export keys equal the `tsc` file's; importing the bundled `testing` entry in the same child, `SurfaceError` reached through the runtime is the same object `expectDocument`'s own graph holds; after one shell command through a session over fakes the trace holds an `emulator-*.js` chunk and the emulator's package, and not before. `package.json`'s six `default` targets are under `./dist/bundle/` and every `types` target under `./dist/`, beside a file that exists. Not deferred on a component: the code commit replaces this row.
...
- **T6.20** (I37, F1192): arming on `includes("es-toolkit/compat")` instead of the exact line → **T1.12**'s two-name arm fails, and the day Ink binds a second name the redirect links `debounce` against a file exporting `throttle` — a `SyntaxError` at start for a startup optimisation; dropping the `existsSync` guard → the fallback arm of T1.12 has nothing to hold, and a copy of es-toolkit laid out differently fails to start instead of starting slowly.
