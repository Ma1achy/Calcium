...
`b.steps`, `b.kv`, `b.comparison`, `b.patch`, `b.code`, `b.tree` and `b.tip` set
...
of its own, as a table is, and not a row of peers set against the one before it.
...
- **T5.7** (I37, F1192): **Retired with I37** (F1209). It spawned a child under the import trace, called `prepareLaunch()` and counted es-toolkit modules — under ten armed, over a thousand without, which was the fabricated violation in the same row. There is no es-toolkit in the tree to count.
