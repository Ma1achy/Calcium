...
105. **A ruling deferred on cost names the cost, and something watches the condition rather than the remedy** (I117). `bubble`'s size channel stays a member of `series` for one more change, and the count of forms whose `series` carries a non-position is asserted at **one** — so a second form inheriting all four symptoms fails a row, where `FS3` would have stayed green. The fourth symptom was found by hiding a series and reading the frame, which is the instrument no assertion indexed by inputs reaches (F271, F1051).
...
- **T6.101** (I119, F405): the three tables returned to `Record<BlockKind, …>` → the example's own build fails, because an augmenting consumer is asked for entries the framework never had. **The revert is measured in the example and not in `src/`**, which is the point: the framework compiles either way, and only a consumer can see it.
