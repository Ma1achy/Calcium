...
| F7 | an action dispatches while a field is being edited: a pointer release on a `submit` button, or a `fill` from another entry | **the borrow ends first and writes, and then the action runs** — writes even where focus is still on the field, which F6's predicate would read as a discard. A pointer reaches this only on a path that did not first move focus, because a first click focuses and the reconcile after that event has already written; the seam holds for the paths nobody measured. Otherwise a submit reads the value from before the edit, and a `fill` writes a line the restore then overwrites |
