...
type Padding  = Readonly<{ padding?: Readonly<{ l?: number; r?: number; t?: number; b?: number }> }>;
...
declaring padding** (§3a) — which is now none of them, because a block's own height includes its padding.
...
### 3a. `padding` — the block's own space, on four edges

```typescript
padding?: Readonly<{ l?: number; r?: number; t?: number; b?: number }>   // default none
```

**This replaced `gapBefore`, and it is a replacement rather than an addition.**
R19 held the reason for ten months: a document carrying both would have two ways
to say *a blank row above this block*, and every measurer, every container's
child-width computation and every sequence would have to agree about them. So
`gapBefore: true` is `padding: { t: 1 }`, one field, and the old one is gone from
the vocabulary rather than deprecated in it.
...
must not close up the space above it.

**Height is stated at the block, and that is the sentence that changed.** It used
to read *stated at the sequence, not at the block*, which was true of a field the
sequence owned. Padding is inside the block: `measure(block, w)` is
`t + b + measure of the kind at w - l - r`, and a sequence of blocks occupies
`Σ measure(b, w)` with nothing added. Two consequences, both intended and both
stronger than the rule they replace:

- A block measures the same wherever it appears, so C14's cache stays keyed on
  the block and the width alone — **and the key now covers the spacing too**,
  where before the spacing lived outside every key.
- Every container applies the same rule, because none of them applies it: the
  registry insets and pads once, around every block, so the document's top level,
  a `panel`'s children and a `column` group's children cannot disagree. **A `row`
  group's child is padded like any other** — the old field was meaningless side
  by side and was ignored; space inside a box is not, and this is a frame
  movement named when the change landed.

**It applies to the first block too.** Padding on the first block of a sequence
is a leading blank row, because the alternative — silently dropping it — makes
the field mean two things depending on position, and a document assembled by
concatenating two others would then render differently from either. **`childGap`
is the other question and not this one**: a uniform gap *between* children is a
container's property, it would give no leading row, and folding the two together
would turn a per-block opt-in into a default (`docs/notes/C04_PADDING_WALK.md`).
...
its top padding, rebuilt every frame, while the stored document keeps the block and its identity.
...
| 9 | `flex` on a `direction: "column"` group | a column's children all take `w` | the field is on the group, which has both directions | **R9 — ignored, not an error, on this section's own precedent**: `gapBefore` inside a row group *was meaningless and was ignored rather than being an error* (§3a — and its replacement is **not** ignored there, which is A4's named frame movement), so the same block travelling into either direction does not fail. **Knowingly vacuous**, which is why it is written down: an ignored field is how a value comes to be silently unread |
...
| 19 | **`padding` × `gapBefore`** | roadmap 38: *padding as a general property rather than `gapBefore` being the only spacing that exists* | `gapBefore` was **the one field that is vertical rhythm** (§3a), and a blank row above is padding-top | **R19 — BUILT, as the replacement this row required.** The reason it stood for ten months was duplication rather than cost: a document with both has two ways to say *a blank row above this block*, and every measurer, every container's child-width computation and every sequence would have to agree about them. So `gapBefore` became `padding.t` and left the vocabulary in the same commit, which is what *the change is a replacement rather than an addition* meant. The walk that landed it is `docs/notes/C04_PADDING_WALK.md`, and it found the plan's *`gapBefore` → `padding` + `childGap`* to be two changes: `childGap` is a container-level uniform with no leading row, so it cannot be the replacement |
| 12 | **`padding` × everything** | roadmap 38 bundles *padding as a general property rather than `gapBefore` being the only spacing* | padding on any block changes what `measure` returns for it | **R12 — BUILT, and split out exactly as this row said it would have to be.** *A bundled row can only be split*: the shared blocker was never any of the subjects' own, and padding landed on its own walk, its own spec commit and its own landing while the weights stayed where they were |
...
- **R9 — a weight on a column group is ignored**, on the precedent `gapBefore` set for an
  unread field before it was replaced (§3a).
...
- **R19 — `padding` is built**, and `gapBefore` became its top edge and left the vocabulary in
  the same commit, exactly as this row required of it.
...
- **R1 — a child is placed inside its cell, and the cell is the group's.** In a `row`, a child's
  ...
  component has nothing to move inside and is **ignored**, exactly as `gapBefore` was ignored on a
...
**And the half that was missing is the direction.** *This kind is not the one that performs it* is
true of the window a scroll would be **given** — its height is declared, so it cannot measure less
— and says nothing about the window a scroll must **take** of each child. Nothing did, so a box
holding one child taller than itself painted the child whole and measured seven rows against
thirty-two, with `follow` inert and the residue row counting against a view that was never applied
(F855). The seam is C09 I58's `windowChild` and the bound is C09 I59; the ruling above is unchanged,
because it was about the wrong window all along rather than wrong about its own.
...
the whole of what it draws** — *⋯ +N more*, `measure` 1 at every width (I104). Expanded again
it is an ordinary box. **The collapsed text is the count alone, and the reason is the vacuity
class** (F826): the row read *⋯ 0 above, N below* until 2026-09-06, and *0 above* on a box that
shows nothing is a clause that cannot be false — there is no *above* when no row is visible. The
design's `+N more · ⏎ to expand` supplied the count's form; its key name stays off the row, because
a key named in a notice is a second keymap (C16 I19) and the footer already shows the `expand`
label for the focused element. One mechanism, two texts by state, and neither names a key. The design's §9b reasoning panel and §9c tool call are both this shape:
...
click on *⋯ +N more* addresses the first child and expands (C16 §4a's hit-test, unchanged).
...
| S2 | *collapsed* × *`follow`* | the offset is forced to 0 while collapsed, so no child straddles a held offset and nothing is drawn above the fold — the residue reads *⋯ +N more* by I104 whatever is held (T2.42, and the group child that made the zero observable: F968); expanded, `follow` applies. A collapsed follow box has nothing to follow |
...
| S5 | *`expand` target* × *a row id and a block id* | rows are searched first, then blocks, **both at any depth**. I14 keeps block ids unique but says nothing about a row id equalling a block id; the order is recorded so a collision is a known answer rather than a surprise. **The depth qualifier was written on the block half alone** and the row half shipped a top-level scan, in the same function, forty lines apart (F1015) — it belongs to the search and not to one of its two arms |
...
| 6 | the floor × `padding` | the padding is the **block's** (§3a) and is inside what `measure` returns, so `max` applies to the padded block — which is what a layer above reserved space for. Under `gapBefore` the gap was the sequence's and added after the block's rows, so the floor applied to the block alone; the walk ruled the new reading at A6 |
...
**It is what the protocol arm caches at a placement** — an unchanged picture transmits once, and with no placement scope it is the placement's identity too; scoped by entry, the placement is the block's (C09 I66, F987) — and
...
| | sequence | ruling |
|---|---|---|
| T1 | transmit → place → evict → the same digest arrives again | the placement is released when its block leaves the document, and the digest returning is a new transmission, so this re-transmits and does not leak. **Ruled here and built at C09 I66** (F987): `#sentImages` was never cleared before it |
| T2 | two blocks, one digest, in one document | two placements; one transmission with no placement scope and two once the shell scopes placements by entry (C09 I66) — a placement's identity is the block's, so the second is a second placement and not a leak |
...
| T5 | an entry evicted while its image is on screen | the placement rows go with the entry; the placement is released by the seam's sweep on the frame after, so the last frame drawn is never missing its image (C09 I66) |
...
**Three attributes in the first pass — and §3am.1 admits two more members on 2026-09-04, so the
paragraph below is the record of why they were held back and not the ruling in force** (I85). A
span may say `bold`, `italic` or `underline`. Not `tone`: a run-level tone would collapse at 1-bit onto the same `bold`/`dim` the span's
...
**`elide`** (I105). The call grammar's head is one committed row whose argument gives way first —
`⬤ pytest tests/unit · 4.2s · 47 passed` at a width that cannot hold it loses the tail of
`tests/unit` and never the verb or the count (C09 I46). Which run is the argument is something
only the composer knows, so the span says it: `elide: true` marks the run a fitter shortens first,
from its end, through `truncate`. **A boundary, never an appearance** — `measure` reads it exactly
as it reads a span's offsets (I83), and outside a fitted token it is inert: a wrapped notice with
an `elide` span wraps as it always did, because the member says *shorten me first* and a wrapped
...
| An unknown `blockId` on any op that carries one | The caller is addressing something that is not there — a bug worth surfacing, exactly as C13 §6 surfaces a patch to a settled entry. **Any**, because the ops carrying the field are `replace`, `merge`, `reserve` and `expand`, and a table naming two of them is what left the fourth scanning `doc.blocks` (I115, F1015) |
...
**Four ops address a block by `id`** — `replace`, `merge`, `reserve` and `expand` — so a document with two blocks sharing one is not addressable. `validateDocument` checks it, across nested children as well as top-level blocks, and `applyPatch` fails rather than guessing when it meets one.

**And the id resolves wherever the block lives** (I115). Containment is not part of an address: I14 makes ids unique *nested children included*, so the walk that finds a duplicate is the walk that finds the target, and every arm asks it once through `countId` and `rewrite`. **Written here as a list of two, the rule reached the two ops it named and neither of the two added after them** — `reserve` argued its own case in a comment at its call site and `expand` never did, so one arm of four scanned the top level and answered *no block "tbl" in the document* about a document holding it (F1015). The enumeration is `tree.ts`'s failure one layer up: the defence was against a second copy of the walk, and what went wrong was a second *list of subjects* for it.
...
| 3 | *pure data* × *the round trip* | a property whose value is an explicit `undefined` | `JSON.stringify` drops the key, so the parsed object lacks it. **Asserted modulo this**, and the reason is that it is unreachable through the framework: `exactOptionalPropertyTypes` makes `{padding: undefined}` a different type from `{}`, and every constructor spreads-if-present rather than assigning `undefined` — the adapter mapping says so in its own comment |
...
- **I25** — **`padding` is content and it is inside the block**: `merge` carries it, `measure` counts it — `t + b` on the height and `l + r` off the width the kind is asked for — and a sequence of blocks adds nothing. A composer that inserts spacing of its own instead (C23 §2) makes a document's height unknowable from the document. **This replaced `gapBefore`, whose rule was the mirror image**: the gap was the sequence's, `measure` never saw it, and three consumers reconstructed the blank row independently because it belonged to no block.
...
- **I49** — **A scroll whose content cannot fit draws a residue marker, in both directions, and pays a row for it out of its own height.** *N above · M below* rather than silence: a bounded region with a marker is normal and one that silently ends is a defect (F123's class, D40's eviction marker). **`measure` returns `height + 1` exactly where the children measure taller than `height`, and `height` where they do not** — the condition is on `(block, width)` and never on the offset, so the content area does not change size as the reader scrolls and I47's offset-independence survives the marker rather than being weakened by it. The glyph is C09's and never a literal (F6). The row's text by state is I104's.
...
- **I55** — **A style field is a literal union, and where its values are generated the union is generated beside them.** `colormap` is `ColormapName`, 142 members generated with the tables; `palette` shipped next to it as a bare `string`, so `palette: "tab-10"` compiled and failed at render — the exact shape I50's colormap clause refuses, one field along.
  ...
  **Those five were an example and were read as the class** (I118, F213, F1076). `Plot` declares **24** members whose type is a union of string literals, and **eight** had no membership rule at the document gate — the three of the five still open, plus `layout`, `binning`, `box3`, `axes3` and `colourBy`, which appear in no document. **A class named by enumeration is the size of the enumeration**, and the five clauses F213 asked for would have closed a third of it while leaving the twenty-fifth member unchecked on the day it landed. `PLOT_UNIONS` is the class; MG31 keeps it equal to the type.
...
- **I100** — **`align` takes both axes, and a group places each child inside its cell.** An entry is a `Valign`, a `Halign` or `"v-h"` with the vertical first; absent is `top-left`, which renders byte for byte what an unaligned group rendered. A `row` places on both axes inside its allocation by the group's height; a `column` places on the horizontal only, its vertical component ignored as `gapBefore` was on a row (§3a). An entry outside the fifteen, or a length that does not match the children, is refused at the builder (§3 *Both axes* R1, R2).
...
- **I115** — **A `ViewPatch`'s `blockId` resolves at every depth, and it is the *field* that carries the rule rather than a list of ops.** `replace`, `merge`, `reserve` and `expand` all name a block by id, and I14 makes that id unique *nested children included* — so containment is not part of an address, and an arm that scans `doc.blocks` is answering about a different document from the one it holds. Every arm asks `tree.ts`'s one question through `countId` and `rewrite`; two enumerations agreeing today is what §4a used to rest on. **`expand` was the arm that did not**, and it stated the miss as *no block "tbl" in the document* about a document holding it — a false statement about the document, in the file that owns the walk, while `replace`, `merge` and `reserve` all found the same block (F1015). **Refusing a duplicate id comes with the walk rather than beside it**: a `find` silently takes the first of two where I14 says there is no correct target, which is F1010's second hole arriving in `applyPatch` itself. The same rule governs the dispatcher's two-arm search (I98), because *where is this block* has one answer per document and not one per caller.
- **I116** — **`logs` earns its place on the per-line record, and the windowing argument that decided it is false.** The kind has one caller in the whole tree — `test/contract/builders.test.ts:131`, the per-kind builder table — and F141 kept it against removal on two reasons. Re-derived at HEAD on 2026-09-10: the **appearance** reason holds (`lines: { ts, level, message }[]` is three columns re-aligned at each width, and `levelTone` has exactly one caller, the `logs` renderer, so a `raw` line draws `WARN` in body colour), and the **deciding** reason does not. *`logs` is the only kind in the tree that implements `window` … and `raw` has none* was true when it was written and stopped being true in `f7a95bc5` (2026-09-04, *Windows for code and raw*). **Seven** kinds implement `window` today — `keyValue`, `table`, `logs`, `code`, `patch`, `terminal`, `raw` — so a `raw` block scrolls. The count is taken from the registry rather than from `blocks/kinds/`: grepping the directory returns **five**, because `table` and `patch` register from `src/presentation/table/` and `src/presentation/patch/` and a source scan over one directory cannot see a kind another component supplies (T2.127). **What is left of the residue is one fact and it is C04's**: no `ViewPatch` arm appends a line, so a growing log is `replace` with the whole block per tick, and the kind's absent consumer is the streaming route declining an operation that does not exist rather than a surface declining the kind. The op is additive to a union apps produce rather than exhaustively consume, so it lands after publication; **until it does, the reason `logs` is declared is the record and never the window** (F141, F1055).
- **I117** — **A channel that is not a position is not a member of `series`, and `bubble` is the one place the rule is broken — ruled, costed, and watched by its own count.** F271 named three symptoms of `block.series[1]` being a bubble's size channel; re-measured at HEAD on 2026-09-10 there are **four**, and the fourth is the one that says the defect gets worse rather than staying still. Driven at 72 columns over `[{label:"value",values:[10,40,25]},{label:"size",values:[1,4,2]}]`: (1) `seriesRange` returns `{min:1,max:40}`, so the ordinate's floor is a *size*; (2) `overlaidRows` maps every member, so the figure carries **3 marks per series, 6 in all** — the sizes drawn as bubbles sized by themselves; (3) `identityOf` yields `["value","size"]`, so the legend reports two data series; and (4) **`HAS_HIDEABLE_SERIES.bubble` is `true`**, so C22 I78's reader-facing control acts on the channel: hiding the entry the legend calls `size` removes a rasterisation the reader never asked to see, **while the sizes keep sizing the value bubbles and keep setting the value axis**, because `bubbleRows` reads `block.series[1]` directly and never asks `seriesHidden`. Symptom 4 is in no entry, was invisible to `FS3`, and is a *control that does the wrong thing* rather than a figure that looks wrong. **The ruling, taken**: the size channel is its own member — `sizes?: readonly (number | null)[]`, refused on every form but `bubble` and required there, the shape `segments`, `quartiles`, `ohlc` and `vectors` already have — and `bubble` refuses a second `series`. All four symptoms go by construction rather than by four fixes, because each is a *correct* rule about `series` applied to something that was not one. **Measured on landing: eleven golden frames and the `plot-forms` snapshot moved, and nothing else did** — the cost was named before it was paid and it was the cost. The catalogue's gutter runs `20 · 30 · 40 · 50 · 60` for data spanning 20–60 where it ran `0 · 20 · 40 · 60`; one set of bubbles, no *value · size* legend. **Three things the change reached that the ruling did not name**, each caught by a gate or a walk rather than by the plan: `sizes` needs its own `requireFiniteNumbers`, because leaving `series` also leaves the loop that checked it (F1082 is the class); MG27 refused the commit until `b.plot` could pass it, since a field no consumer can reach is a surface the spec describes and the API does not; and `bubbleRows`' ASCII arm had **never run** — it chose `RAMP_DOTS` and folded braille, and a two-series bubble had been routed down `overlaidRows`' shared-cells path instead, so the arm was selected and unreachable, which reads exactly like selected and correct (A03 §2). **What watched it was T2.128 and not `FS3`**: `FS3` asserted the disagreement and was green for exactly as long as the defect was, so a fifth symptom or a second form with a second channel would have passed it silently; T2.128 pinned the *count* of offending forms, and that count is now zero (F271, F1051, → C12 I62, C22 I78).
- **I118** — **Every string-literal union `Plot` declares is refused by value at the document gate, from one table, and the table equals the type.** `PLOT_UNIONS` in `validate.ts` carries all 25 members and their values — **24 declared inline and one by reference**, which are two counts and not a disagreement; `plotUnionErrors` loops it before any form rule runs, and every form rule that reads such a member asks `isKnownPlotValue` before it speaks — so a bad value is one fault and the rule below it says nothing about a value the document does not contain. **Being a union is a compile-time fact and this gate's subject is a document**: a `Plot` built in TypeScript cannot hold a bad value, and a `Plot` arriving as JSON from the far side is what the gate exists for, which is how a sentence true about the type was read as true about the check. MG31 compares the table's keys and values against `Plot`'s own declaration **by equality in both directions**, so a member added to the type is unchecked-and-reported rather than unchecked, and a table entry naming no union is a rule with no subject. **The rule reads text, so it resolves a member declared by reference** — `xFormat?: Plot["yFormat"]` — against its referent, and does not merely skip it: the parse's own limits were stated when the rule landed *and asserted not to be reached*, and one of the two was reached. `Plot["yFormat"]` is the file's convention rather than an accident, written three times — `BarSpec.format`, `AxisSpec3.format` and `Plot.xFormat` — and the top-level one accepted any value while `yFormat`, the member it is declared from, was refused. **Naming a limit is a claim about the rule and review can check it; claiming the corpus does not reach the limit is a claim about the corpus, and only a measurement can** (F1085). What an unchecked value did, measured from the consumer: `matrixAnchor` fell through to `window`, `legend` reserved zero rows on a plot that asked for one, `plotFrame` took the frame renderer's default arm, and `orientation` — the only one of the eight that produced an error — refused a `pie` carrying `"vertial"` with a message about a *vertical arm* the caller never asked for (F213, F1076; → C12 I34, C10 I31).



- **I119** — **The block union is open at the type level, and the framework's exhaustive tables are closed.** `Block` is `BlockKinds[keyof BlockKinds]`, and `BlockKinds` extends a closed `KnownBlockKinds` holding the framework's own kinds; an app declares its kind by augmenting `BlockKinds`. This is the type-level half of a mechanism the runtime has had since F1 — `TuiConfig.blocks` takes the definitions, `createBlockRegistry` registers `table`, `plot` and `patch` through the same public `register`, and an unknown kind is drawn degraded as `raw` rather than refused.

  **The three questions F405 said the ruling had to answer were already answered, each at its own site and each citing F1**, which is why this invariant is narrower than the finding that asked for it. `validateDocument` skips an unknown kind — `if (KNOWN_KINDS.has(kind))` — rather than refusing it. `childBlocksOf` asks `tree.ts` **by name** and does not descend into an app kind's `children`, because they may be things that are not blocks. `uncoveredKinds` takes `string` and not `BlockKind`, deliberately, so a registry's app kinds are the ones most likely to be reported missing a fixture. Three site-level rulings stood as the stated blocker on a finding for as long as nobody carried them back to it.

  **Every table asserting one entry per kind is keyed on `KnownBlockKind`.** `KIND_CHECKS`, `ANIMATES` and `RAMP_EXTENT` are the three — measured, not assumed; the two `ReadonlyMap<BlockKind, …>` in `expect-document.ts` assert nothing, because a `Map`'s type demands no key. A table keyed on the open union is wrong in both directions: the framework's own build would demand an entry for a kind it has never heard of, and in a consumer's program its **declared** type would promise a value at a key it does not hold.

- **I120** — **Every member `Plot` declares is refused a wrong value at the document gate, and the members that are not are a named list carrying reasons.** I118 with its subject widened from the unions to the whole type, and the widening is the finding: `PLOT_UNIONS` closed twenty-four members and nothing asked about the other forty-two. Measured behaviourally — five wrong values at every optional member, driven across all forty-eight forms of `PlotForm`, over fixtures first shown to validate clean — **twenty of sixty-six are refused by no form**: `xLabels xMin xMax xFormat yMin yMax emptyMessage categories quartiles offsets totals startDate bands facets segments xScale yScale camera bandwidth axisStyle3`.

  **The proxy that found fourteen was wrong in both directions**, which is why the rule is stated over behaviour and not over the file. Six members that appear in `validate.ts` are unchecked, and `axes`, which appears nowhere, is refused. **Three ways a check reads as present, each defeating a different reader.** *Absent* fails a grep. *Read then skipped* passes one: `plotAxisCrossErrors` is the only function that touches `yMin`, three guards deep behind `axisCross` being present, being `"zero"` and the form honouring it, and the line that touches the type is `if (typeof lo !== "number" || typeof hi !== "number") return;` — the type check used to **skip** rather than to refuse, which from a grep is indistinguishable from the opposite. *Gated behind a sibling* passes a grep **and** carries a detailed refusal: `startDate`'s whole check sits under `if (unit === undefined) return;`, so `form: "calendar"` with `startDate: 12345` and no `calendarUnit` is accepted and the same document with `calendarUnit: "day"` is refused.

  **What a wrong value does is what makes this a gate and not a tidiness rule.** One form per member at width 60: nothing escapes containment, three become an ERROR card (`yMin`, `yMax`, `categories`), ten are inert at that form and value, and **seven draw a different frame with no error at all** — `xLabels`, `xMin`, `xMax`, `quartiles`, `totals`, `bands`, `facets`. Containment catches what crashes and says nothing about what lies. And the two edges are unconstrained **against each other**: `yMin: 6, yMax: 0` is accepted and draws a *collapsed* axis, three identical gutter captions with the series flattened to a straight line through the middle, while `yMin: "x"` on the same block is loud. The mistyped value is the one a reader sees; the swapped one is not.

  **The mechanism is a wrong value per member, and the completeness is equality against the declaration** — the allow-list shape rather than a narrowed scope, so a member with no check is *named* rather than absent. A member that legitimately admits any value sits on the exemption list with its reason, and a member added to `Plot` with neither is unchecked-and-reported rather than unchecked. `facets` is the one that closes a class rather than a member: it holds child plots, and `childBlocksOf` has arms for containers and for a table's row detail and none for it, so every rule in the file stopped at the first nesting (F1082, F1085; → C12 I62).

  **The registration surface is the other half, and the first draft of this invariant was wrong about it.** It said `BlockDefinition<B>` puts `B` in return position, is therefore invariant, and that `fullRegistry`'s cast would survive. Building it falsified both halves: `B` sits in parameter position **only** — deliberately, and `Windowed`'s note in C09 says why — so `BlockDefinition<B>` is *contravariant*, and a definition handling only `Table` is correctly refused where one handling any `Block` is asked for. Correct about assignment, wrong about the registry, which dispatches by `kind` and hands each definition nothing but its own block.

  So the element type is a union over the kinds — `AnyBlockDefinition` — and `TuiConfig.blocks`, `register` and `DEFAULT_DEFINITIONS` all take it. `BlockDefinition<Table>` is a member because `table` is, and an app's `BlockDefinition<Faulty>` is a member the moment `faulty` joins `BlockKinds`. Measured: **five** `as unknown as BlockDefinition` casts in `src/` — three in `construct.ts`, one in `expect-document.ts`, one in `defaults.ts` — plus one per definition in every consumer, become **one**, at the line that stores into a `Map` keyed by kind. That one is the dispatch the compiler cannot see, and it is the same class of contract `Windowed` already carries.
...
103. **Containment is not part of an address** (I115). One walk answers *where is this block* for every op that names one, so the arm added last cannot be the arm that scans the top level — which is how `expand` came to refuse a document that held its target, and to say so in a sentence that was not true (F1015).
...
- **T1.49** (I125, §3ao, §095): §095's own moves, reproduced — the window sliding by the minimum, the `«n` that appears costing it a member on top of the one it slid past, and the mark that disappears at an end giving a cell back. Then the property, over a sweep of member widths, widths, currents **and held starts**: the end is the maximum run that fits and the start is the first at or after the held one that reaches the current, against a reference that asks the same question exhaustively. **The held start is its own axis** — the first draft passed it equal to the current, so the current was inside the window in every case and two of the three branches were never taken: 700 assertions exercising one arm, which is A03 §2's vacuity class wearing a sweep.
...
- **T1.40** (I1, F1065): the control — an object frozen **shallowly** by something else is still walked to depth. This is the row that separates the memo from `Object.isFrozen`, which passes T1.39 completely and leaves a nested array mutable.
- **T1.42** (I119, F405): a document holding a kind the framework has never heard of validates, and the block's own fields survive the round trip — the runtime half, asserted from the same side a consumer sees it.
- **T1.43** (I120, F1082): **a wrong value per member, driven at every member `Plot` declares.** The table is compared to the declaration by equality in both directions, so a member added to the type has no row and fails here rather than arriving unchecked; the member count is asserted at **69** first, because a loop over an empty table satisfies every expectation inside it. Each entry is refused by `validateDocument`. **The exemption list is driven too**, because moving a member onto it drops the member out of every loop and silences it as effectively as having no rule — so each stated exemption has to hold, and `kind`'s does: an unknown kind is skipped rather than refused (I119). **And the nesting, which the table cannot reach**: a well-formed facet carrying a bad member is refused, a facet reusing its parent's id is refused, and the same facet with nothing wrong is accepted. In `test/unit/plot-members.test.ts`.
- **T1.44** (I120): the control — every member's own **legitimate** value is accepted on a form that carries it. Without it T1.43 is satisfied by a gate that refuses everything, which is T1.36's argument one type wider.
- **T1.45** (I120, F1082): the three shapes named rather than looped, because each was invisible to a different reader. `yMin: "x"` is refused where `plotAxisCrossErrors` would have skipped it; `form: "calendar"` with `startDate: 12345` and **no** `calendarUnit` is refused, where the check existed and its sibling gated it; and `yMin: 6, yMax: 0` is refused on the **relation**, which no member rule can see — the frame it drew was a collapsed axis with three identical captions and the series flattened to a line.
- **T1.41** (I1, F1065): the array fast path's stated narrowing, asserted rather than assumed — an array carrying an own non-index property is frozen, its elements are walked, that property cannot be replaced, and its **interior is not frozen**. A limit that is tested is a limit; one that is only written down is a hope, and this is the shape a later reader would otherwise discover from a mutation.
...
- **T1.35** (I118): every member of `PLOT_UNIONS` refuses a value outside its union at `validateDocument`, and the refusal names the member; the table's own count is asserted at **25** first, because a loop that found nothing satisfies every expectation inside it — and the number is the table's entries, against `Plot`'s **24** inline unions plus `xFormat`, declared by reference. In `test/unit/plot-unions.test.ts`.
...
- **T2.36** (I90): a `raw` block with `colormap: "viridis"` and three valued spans paints three backgrounds from the map at 8-bit, and none at 4-bit; `value: 1.5`, `value: -0`'s sign aside, and a valued span on a block with no `colormap` are each one gate error. `measure` at width 6 of `"x(abcde)yz"` with `abcde` valued is one row more than without — `x(` / `abcde)` / `yz` against `x(abcd` / `e)yz` — because the token wrapped whole where the plain wrapper had no break point; `"The cat sat on the mat ."` with a value per token measures the same as unvalued at every width, because prose already breaks between tokens (the first example written here, `"alpha beta gamma"` at 12, was measured and could not fail — F588); `colormap` on a `rule` is one gate error; and for a valued notice `measure` equals the rendered row count at every width of the sweep.
...
- **T3.81** (I115, F1015): **the four arms answer one question.** The same table, once at the top level and once inside a `group`, a `panel` and another table's `detail`: `expand` succeeds in all four, and `replace`, `merge` and `reserve` are asserted beside it on the nested document so the row is *the arms agree* rather than *this arm works*. The control is the document that has no such block, where all four still refuse. **Measured before the fix**: `expand` `{ok: false}` on all three containers against `{ok: true}` for its three siblings.
- **T3.82** (I115, I14): a duplicate `blockId` — one copy nested — is refused by `expand`, in the same sentence `replace` and `merge` refuse it with. A top-level `find` returns the first of two and reports success, which is the case I14 declares impossible and an adapter's document can be.
- **T3.83** (I98, §3c S5, F1015): **the dispatcher finds the block holding a row at any depth.** An `expand` action naming a row of a table inside a `panel` produces an `op: "expand"` patch carrying that table's id; before the fix it produced *nothing to expand — no row or folded block* — a second false statement, in the arm forty lines above the one that already descended. The fold arm is asserted in the same row against the same walk, because one walk read twice is the ruling and two that agree is not.
