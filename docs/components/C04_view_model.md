...
`candlestick` passes as a *style* rather than a form — the same test `scatter3d` itself **failed**
...
forced declarations, each a question the form must answer — was the argument *for* `scatter3d`
being a form. A `line3d` would force the same 24 and every answer would be `scatter3d`'s, because
...
cloud and a parametric curve is a path with no samples, so `scatter3d` with `lines3` and no
...
comes from a **zero-width range**, where `unitOf` maps an axis to its centre and collapses the
surface to a line — so the caller cannot avoid it by validating their mesh, and the gate cannot
refuse it without refusing a legal document. It is the renderer's, and it draws at ambient.
...
- **T2.4h** (I79): `surfaces3` and `light3` are refused off `scatter3d` at **both** gates with their converses, and a `scatter3d` carrying **only** `surfaces3` is **accepted** — the third instance of T2.4g's second half, and the one that says the gate reads a set rather than a widened pair of names.
- **T2.4g** (I78): `lines3` is refused off `scatter3d` at **both** gates with its converse, and a `scatter3d` carrying **only** `lines3` is **accepted** — the second half is the row, because the gate as first written refused it and the member rule alone reads as correct. Paired with `points3`-only and with both, all three accepted.
