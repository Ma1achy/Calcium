...
| S2 | *collapsed* × *`follow`* | the offset is forced to 0 while collapsed, so every hidden row is below and the residue reads *⋯ +N more* (I104); expanded, `follow` applies. A collapsed follow box has nothing to follow |
