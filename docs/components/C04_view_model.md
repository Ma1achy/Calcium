...
- **T1.43** (I120, F1082): **a wrong value per member, driven at every member `Plot` declares.** The table is compared to the declaration by equality in both directions, so a member added to the type has no row and fails here rather than arriving unchecked; the member count is asserted first, because a loop over an empty table satisfies every expectation inside it. Each entry is refused by `validateDocument`, and the refusal names the member. In `test/unit/plot-members.test.ts`.
...
- **T1.35** (I118): every member of `PLOT_UNIONS` refuses a value outside its union at `validateDocument`, and the refusal names the member; the table's own count is asserted at **24** first, because a loop that found nothing satisfies every expectation inside it. In `test/unit/plot-unions.test.ts`.
...
- **T2.128** (I118, MG31): the rule fires on a union in the type and not in the table, on a table entry naming no union, and on a values disagreement; is silent on agreement including `false`; reports rather than passes when it can parse neither side; and is clean over the real tree with the corpus asserted at 24. In `test/unit/enforce-rules.test.ts`.
...
- **T6.102** (I120): the per-member table reduced to the twenty-four unions → T1.43's equality fires on forty-two members rather than passing, which is the direction that matters: a table that shrinks must be loud. Any single member's check deleted from `validate.ts` → that member's row in T1.43 fails and **only** that one, because the table drives each member separately rather than asserting a total. The exemption list widened to hold a member that does have a check → T1.44's control fails, since the check refuses the value the control offers. `plotAxisCrossErrors` given back the `typeof … return` skip → T1.45's first row fails while T1.43 stays green, because a member with a wrong value refused elsewhere is still refused. The `facets` recursion removed → T1.43's `facets` row fails and every nested rule goes quiet again.
