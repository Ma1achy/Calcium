...
- **T3.56** (I78): `closed: true` on a polyline of **two** points emits **one** segment and not two, and on **one** point emits none — asserted on the segment count, because a retraced segment is invisible in a frame by construction.
