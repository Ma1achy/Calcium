...
- **T1.75** (I45, C10 I45, R-THM-003): a focused call head in `hcDark` and `hcLight`, over the five states → five distinct head characters, each the state's own mark; the same heads unfocused → one `●`. The control is `dark`, whose focus ground is not a band: focused or not, `●`.
