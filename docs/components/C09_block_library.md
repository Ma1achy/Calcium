...
- **T2.143** (I72, F1209): over the block corpus × seven widths × the three capability sets, `renderToLines` — the rows arm — equals the committed capture of that block rendered through Ink, its rows wrapped back into an element with `elementOf`, byte for byte; and a sequence of rows-answering and element-answering blocks with gaps, a floored block and a capped one equals the sequence rendered whole through Ink; and the probe reads `rows` for the rows arm and `react` for the element arm, never both for one block.
