...
owns**, and that seam does not exist. **Phase 1 does not fake it**: the escape construction is built
and asserted structurally, and `imageDefinition` takes the **dither at every protocol including
`kitty`** — because a placement with no transmission draws nothing, and nothing is worse than a
dither on a terminal that could have shown one.

**The expiry is a symbol rather than a sentence**: `transmitImage`, the L4 seam that writes once per
digest outside the frame. Until it exists the protocol arm is a module with an instrument and no
call site, which is recorded in `UNCONSUMED_MEMBERS` rather than papered over with a branch that
draws blanks.
