...
- **T6.119** (I72, I75): the open codes written before every visible character rather than where the state changes → T1.46 on the first two-character styled run; the final undo dropped → T1.46 on any styled row; trailing blanks kept → T1.46 on a padded row; the intensity codes replacing rather than accumulating → T1.46 on `1` then `2`; the reset treated as a style → T1.46; an OSC control written through → T1.46; the mark clustering with a sequence's last byte kept → T1.46's seeded rows; the gap row emitted as `" "` rather than `""` → T2.143's sequence; the rows arm opening the `react` span as well → T2.143's probe arm; the cap's marker above the block's rows → T6.22, **not T2.143**, whose Ink side composes through the same registry and moves with the mutation; the floor padded one row short → T3.54, for the same reason — `elementOf` lifts the padded rows, so both arms carry the shortfall. **Two mutations were run and fail nothing, and are written down as the row's limit rather than rewritten around**: the floor's pad rows holding a space rather than nothing — the normaliser trims a blank row to nothing, so the frame cannot see it, which is the arm working as specified; and the marker-first and floor-short forms against T2.143 alone, which is why those two name other rows. `tools/mutate/runs/c09-rows-arm.mjs`, whose control returns every row unnormalised; eleven caught, none survived, 2026-09-16. Since F1180 (I75): `38;2;r;g;b` applied as five parts → T1.46 where the tokeniser writes one code; a shared end written for each code → T1.46 on bold and dim closed together; `38` closed with `49` from its bytes → T1.46. Fourteen caught.
