...
- **T2.112** (I45): every non-ASCII character in `GLYPH_TABLE`'s unicode column, `UNICODE`'s slots and every spinner set's frames is absent from the derived emoji-variation set; the control asserts `⏺` U+23FA and `ℹ` U+2139 **are** in it and `*` is not, so an empty table cannot pass the row and neither can one that kept the keycap bases. T2.71 in `spinners.test.ts` runs over the same set in place of its hand list.
