...
and `render` take the scratch — `RenderContext.scratch` on the render side, a sixth argument
beside the memo on the window side — and a windowable block resolves once per width: the `Form`
...
65. **The window seam takes the caller's scratch, and the form is held in it** (I76, F1191). Handed through `windowSequence` and `render` to each kind's `window`; a windowable block resolves once per width; a kind with no `window` touches the store not at all.
