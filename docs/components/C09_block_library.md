...
- **I74** — **A unit of the rasterised alphabets is its own cluster unless the unit after it can extend it, and the walks take it without asking the segmenter.** I63's run counts printable ASCII by its length and hands the segmenter what follows; a plot row inverts that shape — eighty braille or box-drawing units and no ASCII — so every unit went to `containing`, a segment record each, and `clusterAt` was the largest frame-path function in the `/all` profile (F1177). By UAX #29 a character that is not Hangul, a Prepend, a regional indicator or the joiner is followed by a boundary unless the next unit is Extend, ZWJ or SpacingMark; no unit below U+0300 is any of those — Extend begins at U+0300 and the joiner is U+200D — and no unit of `CELL_PER_UNIT_RANGES` — arrows, box drawing and block elements, braille, the sextants — is any of those or any of the four exceptions. So **a unit of that table whose next unit is the end of the text, a unit below U+0300, or a unit of the table is a whole cluster**, and `cells`, `fitStyled` and `sliceCells` take it as one and measure it by `clusterCells` — the same function the segmenter's one-unit cluster would have reached — while everything else, a mark, a selector, a joiner, a keycap's enclosing mark, a spacing mark, an emoji, a flag, Hangul, CJK, a surrogate, still goes to the segmenter. `swallowed` in the rows arm answers zero on the same ground when the unit after an SGR's `m` is one of the table's, where it sliced the row's tail and segmented it. **The bytes do not move**: the same cluster, the same width, one lookup in place of a call; T1.48 pairs every range of the table with every kind of extender and asks each walk to keep the cluster whole, and the goldens hold the frames (→ I63, I65, C12 I118, F955, F1177).
