...
85. **A meter's label column costs nothing when there is no label** (I104, §036, C23 I76). It was `width / 3` unconditionally, so §036's bar row — which carries no label, because the verb is on the head above it — spent a third of its width on blank. Measured at 56 columns before the change: nineteen cells of padding and a bar nineteen cells short.
...
- **I104** — *(§036, §4, `R-BLK-246`, `R-BLK-259`, C23 I76)* **A meter's label column is drawn only where there is a label, and an empty label costs no cells.** The column was a third of the row unconditionally — `Math.floor(width / 3)`, padded — so a bar with no label spent nineteen cells of fifty-six on blank, which is §036's operation row drawn as an indent nobody asked for. §036 puts the verb on the head above and the bar on a row of its own, `▰▰▰…  77%`, and that row has no label at all; a meter that reserves for one regardless makes the shape the design draws unreachable from the outside. **The predicate is the label's text, not a flag**: a caller that wants no column omits the label, which is the same thing it already does, and the block needs no second member saying so. The clamp is unchanged where a label exists — `stripControl` still walks the whole of it and `truncate` still throws most of it away, and `measure` is 1 either way (C28 I45).
