...
the ratio against a CJK row, which must segment every cluster).
...
- **T3.85** (I63, F955): a 200-cell row holding one `│` among ASCII — the patch gutter's shape — fits in under a sixth of the time a 200-cell CJK row does. The ratio between them is the number of clusters that reach the segmenter, one against a hundred: a row segmented whole for its one glyph measured 2.8×, the fix 12×, and the bound at 6 sits between; the control is that the CJK row costs more at all.
