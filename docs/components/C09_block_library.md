...
**Two protocol readings this rests on, stated as unmeasured.** That `a=T` at a live id replaces the
picture — F421 found it asserted five times and measured never, and `tools/terminal-probe`'s pair
at `i=911` is the instrument, not yet read in front of a terminal; and that a replaced GIF's `a=T`
discards the frames the earlier `a=f`s uploaded. Both are checked at the first real-terminal read
of this arm, and a refusal on the first reverses the ruling rather than patching it.
