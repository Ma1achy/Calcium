...
- **T4.59** (I113, C15 §2a, C26 §5): through a session, bytes on stdin — a `keyValue` whose value is shed at the frame's width; `↓` from the prompt lands on `shed-0` and a peek is up whose rows hold the withheld value; `↓` onto a second shedding row moves the peek with it. The control is the same entry at a width where nothing sheds: `↓` does not stop on the block.
