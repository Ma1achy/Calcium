...
- **T5.6** (I71): a child importing `dist/presentation/blocks/kinds/code.js` under `test/support/import-trace.mjs` lists `highlight.js/lib/core.js`, exactly the sixteen `highlight.js/lib/languages/` files §4a names, and nothing from `lowlight`; the same child after `registerGrammar` with a seventeenth grammar lists that one file more. In `test/e2e/code-graph.test.ts`.
