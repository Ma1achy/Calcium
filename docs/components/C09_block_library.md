...
inside a cluster is the child's and stays; and four label writers — treemap,
tree, graph, sankey — place one code point per cell and drop the zero-width
ones, so a family arrives as three faces and a keycap as a digit under an
enclosing mark. They paint no escape inside a cluster because they keep no
cluster whole, and F969 carries that as open rather than as covered.
