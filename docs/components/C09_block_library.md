...
- **T6.120** (I73): the pad taken from the cell's width rather than from where the row ended → T2.144 on a row group with a short child; the gutter dropped → T2.144 on every row group; a child's row not normalised before the pad → T2.144 on a child whose row ends in an open style; the composed line not normalised again → T2.144 on adjacent cells sharing a style; the alignment offset dropped → T2.144 on an aligned child; the rails drawn to the body's height rather than the measured one → T3.89; the panel's empty body drawn as no row → T2.144; the scroll's pads dropped → T2.144; the detail inset dropped → T2.144 on the expanded table; the fallback dropped so an element child is coerced → T3.89. `tools/mutate/runs/c09-rows-containers.mjs`.
