...
- **T2.163** (I98, `R-MOT-010`): **`SPINNER_SETS` against the registry's `spinners`, by equality in both directions and field by field** — membership, `frames`, `intervalMs` and `asciiPattern`. Not a subset in either direction: a set the registry registers and the tree lacks, and one the tree ships unregistered, are one divergence each. It is **red on HEAD** on nineteen of twenty-seven `ascii` columns, so it lands with a shipped defect to its name rather than as a rule with nothing to be wrong about.
