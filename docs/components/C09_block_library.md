...
- **T2.147** (I73, I35, F1210): a mosaic whose children are taller than their regions draws each cut to its own region and not to the grid — the second cell's rows unaffected by the first's overflow; a region under one cell wide or one row tall draws nothing; **a mosaic declaring a height below one draws one blank row**, which is C09's degraded answer rather than a state C04 permits — C04 I71 refuses it at validation and says in the refusal what the renderer draws, so the floor is reachable exactly for a document that skipped the gate, which is the same reason an unregistered kind resolves to `raw` rather than throwing; and each equals the committed capture of the element path byte for byte. The over-tall child is the case the cut exists for, and a grid-wide cut passes a single-cell mosaic, so the corpus carries two cells of different heights.
...
- **T3.89** (I73): a group with a child that answers an element composes elements and the probe reads `react` — the child a **registered test kind**, because since I73 reached `mosaic` no kind this component ships answers an element and the fallback would otherwise have no constructor; a row group whose children's widths and gutter exceed its width composes elements; a row group whose child answers fewer rows than measured leaves that cell's remaining rows blank and the group at the tallest child's height; a panel whose body answers more rows than measured extends the body below the rails with the rails' columns blank. Not deferred on a component: the code commit replaces this row.
