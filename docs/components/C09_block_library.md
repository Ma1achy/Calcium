...
- **I111** — *(§088 §4, `R-COL-006`, question 26)* **`trendUp` and `trendDown` are `↑` and `↓`, and `^` and `v` at ASCII, in the inline domain.** The registry records them as `trend-up` and `trend-down` with `collisionDomains: ["inline"]`. `^` and `v` are also `sort-asc`/`sort-desc`'s ASCII, and the two are separated by **domain** (`table-header` against `inline`) as question 26 ruled, which the domain model allows even though a trend cell and a sorted header sit in one table. Both arrows are East Asian Ambiguous, so at `ambiguousWidth: "wide"` they take the ASCII rung with the rest of the vocabulary (I48). → T2.174
