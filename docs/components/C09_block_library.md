...
- **T6.121** (I74): the extender test dropped, a table unit taken as solo whatever follows → T1.48 on the selector pair's width and the window of one over a marked unit; the marks' block admitted to the table → T1.48 on the mark after a braille unit; `swallowed` answering zero for every next unit at or above U+0300 → T1.46 on a mark after an SGR. **The joiner admitted as a unit that cannot extend was written and survived**: U+200D lies above U+0300, so the clause that excepted it below U+0300 could never fire, the mutation was a no-op, and the clause is gone — the survivor indicted the code, not the row. `tools/mutate/runs/c09-solo-cluster.mjs`.
