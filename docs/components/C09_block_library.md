...
**The deviation is recorded rather than inherited.** `DRAWN_AS_GEOMETRY` keeps the nine blocks the framework draws its own geometry from whole, even where the property says Neutral: **625 code points are Ambiguous for no reason but that list** (576 Neutral, 49 Wide). The reason is that §4c's gates read this answer — `halfBlockEligible`, C12's line-draw and ramp arms — so classifying those glyphs Ambiguous is what makes the wide-convention gate fire at all; and the deviation errs in the harmless direction, since an over-count pads a row and an under-count wraps it. **The premise to re-check is that those gates still read this function**; the day one takes the capability directly, its range is dead. U+2B00..U+2B1F has **no Ambiguous member at all** in the property and is deviation entire.
...
**What this does not fix, with its figure.** `isWide` is the same table written the same way and its errors land in the **default** mode: **8,619 code points are Wide in the property and measure one cell**, including Tangut (7,382), the Yijing hexagrams, Hangul Jamo Extended-A, and thirty-odd emoji singletons — `⌚` `⏰` `⚡` `⚪` `⛄` `✅` `✨` `❌` `❗` `➕` `⭐` `⭕`. Every one is an under-count at `ambiguousWidth: "narrow"`, which is every golden frame in the tree, and it is owed rather than done.
