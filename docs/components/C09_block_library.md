...
- **T6.121** (I74): the extender test dropped, a table unit taken as solo whatever follows → T1.48 on the combining mark after a box unit; the joiner admitted as a unit that cannot extend → T1.48 on the joiner-and-pictograph pair; the marks' block admitted to the table → T1.48 on the mark after a braille unit; `swallowed` answering zero for every next unit at or above U+0300 → T1.46 on a mark after an SGR. `tools/mutate/runs/c09-solo-cluster.mjs`.
