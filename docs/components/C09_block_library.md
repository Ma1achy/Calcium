...
- **I102** — *(§073 `R-BLK-730`, C10 I51, I47, C26 I17)* **A notice that declares an action is a button, and a button is drawn at three rungs: a ground at rest, the chosen pair when focused, and brackets when the ground is gone.** C09 §3 already rules *one button, and the notice is it*; what was missing is that it looked like nothing. Measured before the change, at 24-bit: resting drew `Approve` as bare text, focused drew it on `focusGround` with no padding and no mark, and ASCII drew bare text again — so **an action was invisible at every rung**, and §073's *the ground is the affordance* had no subject in this tree.
  ...
  **The brackets are the second carrier and the reason the rung exists.** A ground dies at one bit and in every theme that declares none, and there `[ Approve ]` is what remains — without it a button at 1-bit carries **zero** carriers, which is the state measured above and the one M11's matrix forbids. The predicate is the ground resolving, not the colour depth, so a theme with no `pick` takes the same rung as a monochrome terminal and one question answers both (C09 I96's shape). The brackets cost four cells and are reserved the same way; the two rungs therefore have **different** widths, which is correct and is the thing to assert — a reservation that split the difference would leave one rung short.

  **A call head is not a button** (I47). `declaresElement` is true for both, so the predicate here is `action !== undefined` alone: a call head stands in the ring without being something you press, and painting it would put a second affordance in a gutter that already has a mark.
...
- **T1.67** (I102, C04 I122, §073): the chrome is reserved — `measure` equals the rendered rows for a button at every width from 8 to 60 at each of the three rungs, and **no row overruns the frame**. The rungs' widths are asserted to **differ**: two cells of padding against four of brackets, so a reservation that split the difference leaves one rung short and fails here rather than wrapping a row nobody counted.
