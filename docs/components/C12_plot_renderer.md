...
| `legend` × a form with no series | *a legend names series, segments and annotations* (§3ag.4) meets *a tree has none of the three* | **10** — no entries, so `"right"` costs no width and `"below"` costs no row. The second half is F222's fix, which this form needs as a **precondition** rather than inheriting as a consequence |
...
- **I27** — **A legend costs width or a row, and only the width-costing placements may enable themselves.** A row must declare its cost before the data is visible (I1); width is already data-dependent through the gutter, so it may not. That is why `"right"` is the default and `"above"`/`"below"` are opt-in — not a preference. **Skipped entirely at `colourDepth: 1`**, where a swatch carries nothing and still takes the row. **And the row is declared by whether there is anything to name, which the type could not see** (F222): `legendRows` took a `Pick` excluding `series`, `segments`, `ohlc` and `annotations` — every source `legendEntries` reads — so `legend: "below"` on a form with none of them declared the row and drew it blank, a `treemap` at `height: 4` rendering five rows with the fifth empty. **The narrow parameter reads as this invariant's own discipline being kept**, and it is that discipline kept about the wrong four members: an entry count is a count of *block members*, knowable before a single label is placed (§3ag.4), so the `Pick` grows by exactly those four and `values` stays outside it. The guard is about **which** members height may read, never about how many.
