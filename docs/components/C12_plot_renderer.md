...
| the arm | `plotStyle: "braille"` | `plotFill: "solid"` |
|---|---|---|
...
**The raincloud rungs do not, and that is a ruling rather than a gap.** Their cloud is **one cell
row**, drawn with `ladderFor("height")` — `▁▂▃▄▅▆▇█`, **eight levels inside the cell**. Braille
gives **four** dot rows. A braille cloud there is not a finer picture, it is half the resolution
in a different alphabet, and §3c's rule cuts the other way: the renderer names the axis and the
*vocabulary that fits* is the ladder's.

So the style **degrades** on those rungs rather than refusing, which is I18's precedent and the
solid pie's at one bit: the rung is chosen from the height and construction cannot see a height
(C04 I56), so a refusal would name a condition the caller could not have checked.
