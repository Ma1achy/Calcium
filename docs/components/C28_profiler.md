...
- **T3.3** (I13): a sample over a window shorter than the resolution → every delay figure is below the resolution and carries it, so nothing presents a sub-floor number bare; **and with `loopDelaySamples` at zero the pane prints no figure at all**, which is I13's second clause and the answer to F898. Both arms in one row, because the interesting cell is the boundary between them: a floor reading and an absence look identical in the sample and must not on the screen. The row as first written asked for *the histogram is empty rather than zero-filled*, which is not built and is not what I13 says — measured, `max` is 0 and both percentiles are 0.000511 ms at a 10 ms resolution — and F1005 rules that the emptiness belongs to the consumer rather than to the recorder.
