...
export type SpanName =
  | "frame" | "compose" | "measure" | "elements" | "paint" | "rows" | "assemble" | "write"
...
// `rows` is the rows arm's normalisation (C09 I72), a `draw` span. It stood beside
// `react`, the element arm's, and every block opened one of the two and never both;
// there is one arm since C09 I73 reached `mosaic`, and `react` is gone with Ink
// (F1209) — so a deck's arm counts are a statement about history.
...
export type ProfileReport = Readonly<{
  ...
  /** Every frame the *session's reader* caused, in commit order, whatever its
   *  `outcome`. `worst` and the durations are the projection I6 governs; this
   *  one is filtered by cause alone, and `outcome` is here to be read (I54). */
...
export interface ResourceProbe {
  sample(suspended: boolean, at: number): ResourceSample;   // the stamp is handed in — the probe reads no clock (I53)
  dispose(): void;
}
```
...
**And the sampler's stamp comes from a third seam, `sampleClock`** (I53, F971).
`ProfileOptions.sampleClock?: () => number` on the profiler and `ResolvedConfig.sampleClock` at the
root: the *untapped* `elapsed` — the same function the app injected, by identity, before the
recording's tap wraps it — so a recording never holds the sampler's reads. It defaults to `elapsed`
where no recording exists, which is every profiler a test builds, and a replay supplies its own
because a replay's untapped `elapsed` is the positional array itself. The recorder stamps the sample
— `probe.sample(suspended, sampleClock())` — and the probe reads no clock at all, which is why the
listing above hands `at` in.
...
**An entry, composed by C23's `/profile` local verb** (C23 §2, R-EXA-082, F1254). `/profile
<section>` returns a document whose blocks are the section's cards, each a `b.panel` carrying the
card's stamp; the transcript holds it, C14 scrolls it, C26 focuses it and `y` copies it.

**It was a layer** — `kind: "view"`, `placement: fill`, id `profile-view`, owned by
`src/shell/profile-view.ts`, which followed `document-view.ts` and pushed onto an empty stack,
updated in place and walked its deck with `n`/`p`. It was the third and last producer of that layer
kind, which retires with it. The design's test for anything wanting a frame of its own is *does it
have its own **prompt** and its own **context**?* — and a reading of the profiler has neither.

**`profileCard` was already the seam**, described here as *the seam a consumer with its own
...
Three groups, contiguous, the app's first. **A group is an invocation** — `/profile verdict`,
`/profile app`, `/profile framework` — and its cards arrive together in one entry, in deck order.
`n`/`p` walked cards and `tab`/`⇧tab` walked groups while the deck was a layer (C16 I33); both
retire with the target, and what replaces them is what replaces every view's motions: the
transcript's own scroll, C26's focus, and `⏎` to expand in place.

**`deckOf` is what orders them and it is unchanged**, including its expansion of a per-frame card to
one entry per retained frame (I58) — the addresses are `(card, seq)` either way, and `seq` is why a
tick that drops a frame from `worst` cannot silently rename what a reader is looking at.
...
| `a periodic stall` | is something beating against the frame window | `autocorrelation` | a 100 ms timer against a 16 ms window is a peak at lag 6, and nothing else in C12 finds it |
...
| the sampler's stamp | `ResolvedConfig.sampleClock` — the untapped `elapsed` | `samples[].at`, off the recorded channel (I53) |
...
| the sampler's stamp | *none* — `sampleClock` is the untapped `elapsed` | *not* recorded | **a periodic reader cannot sit on a positional channel** (I53, F971) |
...
| 6 | the sampler ticks between two of the session's `elapsed` reads | the tick is the injected timer's and the recording is served by index, so the read lands at a position no replay reproduces and every read after it is served one place stale — one per second of session, measured 2 against 0 over three seconds and 5 against 0 over six, the only reader of 2 908 that disagreed. The stamp is taken from `sampleClock`, which the recording never sees (I53, F971) |
| 7 | the readout's one-second wake draws a frame before the far side answers | the recording holds that frame ahead of the `far` line, so a stand-in serving the answer when the session asks settles the card before the wake and the replay never draws it — diverged at frame 5 with three stalls, measured. `driveRecording` releases each `far` event when it reaches it, after the frames before it, and the stand-in waits its turn (I46, F974) |
...
| a fallback frame | counted, on `timeline` with `outcome: "fallback"`, and excluded from every duration histogram — a fallback is a frame's absence, not its cost, and the absence is a reading rather than a gap: it is where composition failed (I6, I54) |
...
- **I6** — A frame whose composition returned `fallback` is counted, carries `outcome: "fallback"`, and is excluded from every duration histogram — from `latency`, `byReason` and `worst`, which is the whole of what this asks. It is **not** excluded from the per-frame series, and the day one filter served both was the day `outcome` became a member with one observable value (I54, F899).
...
- **I10** — Every `Histogram` carries the relative error of its bucketing, and the report's `regime` repeats it; a percentile taken over a ring with `dropped.frames > 0` is labelled as over-the-window rather than over the session. **The figure is a `±` and the estimate must earn that shape** (F1140): `error` is half a sub-bucket at the widest, so an order statistic is reported at its bucket's **midpoint**, clamped into `[min, max]` — an estimate at the bucket's lower edge is up to a full bucket low and never high, which is a one-sided interval published as a symmetric one. Measured over fifteen populations and five quantiles each, the edge is worst at **2.13%** against the declared **1.5625%** and the midpoint at **1.12%**; `min` and `max` stay the true observed values, which is what the clamp protects.
...
- **I31** — **An element's cost is measured per element, never divided out of a total.** Attribution comes from a span entered once per block through the registry seam. A figure obtained by apportioning a sequence's duration across the blocks in it is not an attribution: a `plot` measures 260× a `rule`, so an equal share reports how many blocks of each kind were on screen while reading as what they cost. **The count is split by operation, because a sum of two is a rate of neither.** `measures` and `renders` are separate members of a node and `calls` is their sum. The registry seam is entered once by the `measure` wrapper and once by the `render` wrapper, so a block that is both measured and rendered has a `calls / frames` floor of **2** and a marker set above 1 fires on every one of them. Measured 2026-09-11 over a 35-frame session with a counter per operation: `group#chrome.header` is **0** measures against 35 renders, `table#profile-table` is **67** against 0, and the header's pills are 35 of each — a spread no single counter can express, under which the two nodes genuinely measured twice inside a frame sat at 3.0, one step above a floor the table never stated (F1098). So the thrash figure is `measures / frames`; `renders` is beside it because a block rendered twice in a frame is the same defect on the other seam; and `calls` is kept as the **seam-entry** count, which is what a reader wants when asking how often the registry was entered at all. It is F892's symptom arriving from a second merge — that one merged two entries under one key, this one merged two operations under one counter — so the general form is that **a ratio whose numerator is a union names a rate neither population has**, and the check is to ask what the numerator unions before reading the quotient.
...
- **I49** — **Retired with the view whose commits it bracketed** (F1254). (§3c, R-EXA-082.) It read *every commit the view raises runs inside `profiler.own`, and the commit seam reads the bracket rather than the call site* — so a frame raised only by the view was `selfInflicted` and excluded (I12), and one the reader also raised was not. **The rule was right and its subject raised commits**: a deck that redrew itself on a timer was measuring a cost it created. A section is composed once, inside the submission that asked for it, and the frame that draws it is the reader's — which is I12's answer arriving by construction rather than by a bracket.
- **I50** — **Retired with the view whose lifetime held the tier** (F1254). (§3c, R-EXA-082.) It read *opening the view raises the tier to `spans` only when it is below, remembers the tier it found, and closing calls `setTier` only when opening did*, restored from C15's change stream so that `Esc`, the ⌃c ladder and the owner's own `pop()` all reached it. **A raise needs a close to pay it back, and an entry has none** — so the deck takes the answer the live card already shipped with its reason written down (§3c): at a tier below `spans` it draws C28's own notice rather than turning the profiler on behind the reader. The teardown-from-the-change-stream half survives as C15 I25's, where it is stated and asserted.
- **I51** — **Retired with the view it redrew** (F1254). (§3c, R-EXA-082.) It read *the view redraws on the injected timer and on a key, never per frame, and what it draws is the one-row header plus a block-boundary window of `profileDeck(...)`* — the cadence, the single layer id, the one `report()` per draw, and the block-boundary projection written a second time beside `document-view.ts`'s. **The duplication §3c recorded rather than resolved is resolved by deletion**: there is no window, because an entry is as tall as its blocks and C14 scrolls it. Currency is `/profile live`'s question and is answered there by a part that refetches.
- **I52** — **The verdict card measures at most 23 rows at 80 columns for every report — **the figure, and its reason has changed** (F1254) — through the registry that draws it.** It was the bound of a 24-row terminal minus the view's one-row header, and the card a reader opens first *had* to fit because a pushed view could not be scrolled. It is an entry now (§3c, R-EXA-082) and the transcript scrolls it, so the bound is no longer a requirement — it is kept as a **measured property of the card's closed parts**, which is what makes it checkable, and the `b.panel` frame the entry wraps it in costs two rows the card does not control. every other card is one figure sized to the region it is drawn at. Its parts are bounded by closed sets — `checkBudget`'s rows, which are A01 Appendix B's six, a two-row `kv` of the regime and a one-line key hint — so the bound is a property of the type rather than of a fixture. **The bound is the rule and the measurement is beside it**: 11, 15, 16 and 16 rows with the header at 80 over the four report fixtures — empty, twelve frames, twelve measured, twenty over every `CommitReason` — so a card that grows four rows inside the slack the bound leaves fails T1.100 before it reaches a screen. Measured through a registry with `tableDefinition` and `plotDefinition` registered, because the harness's default has neither and measures a plot as its JSON, which is how a 40-row pane was once recorded as 26 (F947, F959, F960).
- **I53** — **The sampler stamps its samples from a clock the recording never wraps — `sampleClock`, the untapped `elapsed` the root hands down as the same function the app injected — and the resource probe reads no clock at all.** A periodic reader cannot sit on a positional channel: the sampler's tick is the injected timer's and the recording is served by index, so a tick's read lands between two of the session's reads at a position no replay reproduces, and every read after it is served one place stale — one read per second of session, measured 2 against 0 over three seconds and 5 against 0 over six, and the only reader of 2 908 that disagreed (F971). Every other `elapsed` read — construction's 2 003, each span's pair, each frame's three — is a function of the session's inputs and stays on the channel. The recorder stamps the sample, `probe.sample(suspended, sampleClock())`, so `node.ts` — the one file that reads the process — reads no clock; a replay supplies its own `sampleClock`, because its untapped `elapsed` is the positional array itself (§5a table row 6, T1.102, T1.104, T5.1c).
...
- **I65** (F1193) — **A sampled frame is located through its chunk's source map when the profile is folded, and a frame with no map keeps its URL.** In the bundled package (A04 §5) a `callFrame` names `dist/bundle/chunk-*.js` and a line in it; the fold takes a locator that reads the linked `.map` beside that file once, through `node:module`'s `SourceMap`, and answers the original source and line — esbuild composes `tsc`'s maps, so the answer is the `src/` file and line the deck shows under vitest. The map is read when a card is folded and never at start: enabling Node's source-map support process-wide costs the start 15–60 ms in five pairs, to repair a card drawn on request. A frame whose file has no `.map` beside it, or whose map does not parse, or whose position the map does not cover, is drawn as it was — the URL and line the profile gave — because a card that drops a frame it cannot name is a card that lies about where the time went (I34's rule from the other side).
...
14. **An element's cost is measured, never apportioned.** Per block, at every depth, with the count beside the duration **and the count split by the operation that produced it**, so thrash and expense are different readings and a measure is never summed with a render. (I31, I32)
...
24. **The cheap tier records everything cheap.** A count costs one integer, so `counters` records every count there is — frames included — and a consumer never divides by a member that means one thing at one tier and another at the next. (I44)
...
26. **A recording is inputs; a frame is the answer.** One ordered stream over the seams the configuration already injects, with the verdicts a session computes — its capabilities, its frames — re-derived rather than replayed, so the gate covers the code that computes them. A truncated recording compares as a prefix and says so. (I14, I15, I46, I47)
27. **A recording survives the way sessions actually end.** The clock batch is flushed and the `end` line written from an `exit` hook, because a signal exit never reaches `stop()` — and the hook exists only when a recording does (I48).
28. **A component's own data is a measured input.** Every block kind gauges what its render walks in full, before the clamp that makes the drawn size a constant — so a frame slowed by one enormous block has a figure that names it, and the cheap kinds get a gauge rather than a span because a span on a `rule` is a third of what it measures. (I45)
29. **An element belongs to the entry it was drawn for.** A block id is unique within its document and a transcript holds many, so the aggregate is keyed by the pair or two components become one row with a thrash figure neither of them has. What no entry claims is reported as unclaimed. (I42)
...
33. **The first card fits.** The verdict is 23 rows or fewer at 80 columns for every report — the region minus the header — because its parts are bounded by closed sets; every other card is one figure sized to the region it is drawn at. (I52)
...
40. **A frame in a bundled chunk is drawn under its source file, located when the card is folded and never at start** (I65, F1193).
...
**Rows naming *the view* are kept as the record of a walk, not as live rulings** (F1254,
R-EXA-082), for the reason given above the sequence trace below. B2, B3, B4, B5, B7 and B9 are
those rows, and the test ids they point at — T1.16d, T1.93, T1.96, T1.97, T1.98, T1.99, T1.100,
T1.115b — are struck or re-aimed in §10: what survives of each is named there rather than
rewritten here, because a walk edited to agree with the code afterwards is a record of nothing.
...
| B5 | a card taller than the region | R-o × the window | block-boundary window through `measureSequence`, at least one block; a block taller than the region is shown under a hidden-rows notice — the document view's I47 shape **without its *n/p move by block* clause**, because here `n`/`p` switch cards and the sentence would be false. **The ruling was taken against a six-block pane and a card is one block**, so the window's motions have nothing to move and the hidden rows cannot be reached: F1138, recorded with both remedies, and T1.115b is the row that found it drives the motion rather than the member. → I51, T1.98, T1.115b |
...
| B7 | the terminal's capabilities against the deck's ASCII default | R-n | the view hands `detection.capabilities` whole; under `unicode: "ascii"` the separator is the ASCII arm and under a unicode terminal it is `·`, never the default regardless of terminal. The slot it is read at is the card's **generated footer**, which is where the deck joins its population and its exclusions. → I51, T1.96, T4.9 |
...
| B9 | the first card against a 24-row region | R-o × I52's budget | the budget is 23 rows — the region minus the header — and it is met for **every** report rather than for a fixture. **This ruling was about the overview and the deck answers it differently**: the tables whose rows no type bounds are cards of their own, so nothing has to be moved or capped, and the first card is bounded because `checkBudget`'s rows are A01 Appendix B's six. What survives unchanged is the argument — a cap with an *N of M* marker drops the figures a reader opened the view for — and it is why every card is one figure rather than a denser pane. → I52, T1.100 |
...
| B11 | a card whose form throws against a region below its floor | R-t × R-k | **the kit refuses above the form.** C12's contract is *refuse, never ignore*, so a `violin` under two rows per band and a `boxplot` under one throw at construction — inside a render path with no catch and with `arm` already past. The empty-state rule says *draw a notice*; the form rule says *throw*; a card that meets both throws. Each card declares its floor, the kit compares it to the region before building, and the card that cannot be drawn names the floor it missed. → I60, T1.112, F1130 |
| B12 | a fixed-height form against a region larger than it | R-x × the one-figure principle | `waffle` is ten rows and ignores `height`; on a forty-row region the naive reading of *one figure per card* stretches it to forty and draws thirty blank ones. **The panel is sized to the figure, not to the region** — one figure per card is a rule about how many, not about how tall. → §3c |
| B13 | the verdict card at tier `counters` | R-k × `bullet`'s carrier | `counters` has no histograms, so `checkBudget` has no rows, and a `bullet` with no `quartiles` draws an **empty plot** rather than refusing. An empty bullet is a zero that reads as measured. At `counters` the card is text and the contents list, with no figure at all. → I11, I52 |
| B14 | *cost against position* against what the report carries | the question × `NodeStat` | `byEntry` is keyed by entry id and `NodeStat` has nine members, **not one ordinal**; `snapshot()` sorts by `self`, so even insertion order is gone before a consumer sees it. The card is **deferred**, blocker `NodeStat.firstFrame`, rather than drawn from the sort it happens to receive — which would be a figure whose x axis is a rendering artefact. → F1129 |
| B15 | a session-site span name inside `FrameRecord.spans` | R-q × `PHASE_GROUP` | `frameSpans` accumulates whatever closed since the last frame reset, so `local` — `PHASE_GROUP: "compute"`, `SPAN_SITE: "session"` — lands in whichever frame's record it closed under. A compute-against-draw area built by grouping the record is therefore **session work drawn as a frame's**, which is F888 with a picture on it. Every per-frame series filters `SPAN_SITE === "frame"` first, and the footer names the session-site total it left out. → I41, I57 |
...
**Rows naming *the view* are kept as the record of a walk, not as live rulings** (F1254,
R-EXA-082). The deck is a transcript entry and there is no layer to open, to pop, to resize
under or to raise a tier for, so those cells describe a surface that no longer exists. They
stay because the *interactions* they found are what the walk was for; deleting them would
leave the method's record shorter than the method.
...
| S9 | open → `n` | keys × the window × R-d | the card switches, the offset resets to 0, and the redraw is bracketed with reason `input`. **`n` crosses a group boundary** — the verdict holds one card, so the unit key leaves the section on the first press and the header names the group it arrived in; past the last card of the last group it clamps rather than wrapping. → T1.115b, T4.7 |
...
| S12 | open → `tab` | C16 I33 × the window | the section changes, the offset resets to 0, the card is the section's first, and the redraw is bracketed with reason `input` — the same shape as S9's `n`, one level out. → C16 I33, T1.3q |
| S13 | open on the third flame card → a tick → a slower frame is recorded | R-p × the timer × R-d | `report()` recomputes `worst`, so the third *position* is now a different frame. The card re-resolves the `seq` it holds; a `seq` that has left the set is named as gone and the card clamps to the nearest rather than silently redrawing its neighbour. **Without this the reader is shown a different frame with nothing on screen changing but the numbers**, once a second. → I58, T1.110 |
...
populations of one name with nothing in the report distinguishing them (F888's shape, and F1127).
B11 and S13 are the two the obvious implementation gets wrong while every single-rule assertion
about it passes: a card sized from the region throws inside a callback with no catch and takes the
re-arm with it (F1130), and a card indexed into `worst` silently redraws a different frame once a
second (F1128). B14 is the one that ended a card rather than shaping it — the question is real, the
form is right, and the report cannot express the abscissa (F1129). **Five of the eighteen new rows
produced a finding, and not one of the five was reachable from a row governed by a single rule.**
...
## 9c. The projections' walk — a table and a trace, indexed by rule interaction

`report()` has structure — four classes of frame and three projections that could each claim one —
and state — a bounded ring that evicts, a tier change that resets it, lifetime counters that do not.
A component with both needs both shapes (CLAUDE.md), and taking the trace alone because a recorder
looks like a state machine is how the structural half goes unexamined. The rules the rows name:
**R-p** I6 (a fallback is out of every duration histogram) · **R-q** I12 (a self-inflicted frame is
out and counted) · **R-r** I9 (the ring is bounded; `dropped` counts what the bound discarded) ·
**R-s** I32 (a tree is retained only on the frames kept as worst) · **R-t** I18 (`setTier` resets the
ring) · **R-u** I11 and I44 (no record below tier `spans`, and the count is kept anyway) · **R-v**
I35 (an NDJSON line is the record's own fields).

**The classification table — structural, two rules at rest.** The cells are `outcome` ×
`selfInflicted`, and all four are constructible.

| # | cell | rules | ruling |
|---|---|---|---|
| P1 | drew, the reader's | — | on `timeline`, eligible for `worst`, in the durations. Governed by one rule and kept only as the other three rows' control. |
| P2 | drew, the profiler's own redraw | R-q | on neither, counted in `excluded.selfInflicted`. The axis is *who caused the frame*: a per-frame series that grows while the reader watches it is the instrument reporting its own cost as its subject's, which is the failure class this component exists to end (I34). → T1.92 |
| P3 | gave up, the reader's | R-p | **on `timeline`, out of the durations.** The cell one filter answered for both, and F899's whole subject. `session.ts` writes a fallback screen and *then* calls `endFrame("fallback")`, so the frame reached the terminal — what it did not do is compose, and *the frames that drew* was never the right name for the set it was excluded from. → I54, T3.15 |
| P4 | gave up, the profiler's own redraw | R-p × R-q | **the cause decides and the outcome is never asked**, so P2's ruling wins and this is on no timeline. And `excluded` is a **ladder rather than a partition**: the recorder tests `fallback` first, so this frame is counted in `excluded.fallback` and not in `excluded.selfInflicted`. Measured. The tempting invariant — *every frame counted in `excluded.fallback` is on `timeline`* — is false in exactly this cell, and the table is why it was not written. → I54, T3.15 |

**The sequence trace — event-mediated.**

| # | sequence | rules | ruling |
|---|---|---|---|
| Q1 | a 40 ms drawn frame → a fallback, into a ring of one | R-p × R-r | **the bound is spent on the fallback either way**, and that is the cost of the old filter nobody had stated. Measured at HEAD before the change: `timeline` `[]`, `frames` 2, `dropped.frames` 1, `latency.work.count` 1 — the fallback took the slot, evicted the drawn frame, and then was filtered away, leaving three figures no reader can reconcile. Under I54 the series is the fallback alone and 2 − 1 = 1 closes. → T3.15 |
| Q2 | a fallback → `report()` → `strip` | R-p × R-s | the fallback reaches `timeline` with its `spans` and **without its `tree`**: it is never in `worst` — a fallback is cheap, because giving up is cheap (T6.3) — so `strip` drops the structure `freezeTree` built. **A known limit rather than an oversight.** Retaining every fallback's tree is the unbounded retention I32 exists to refuse, and the session that keeps falling back is exactly the one that would do it. The reader gets the frame, its reason, its wait, its work and its per-phase self times; what they do not get is its shape. |
| Q3 | a fallback → `setTier` | R-t × the counters | the ring resets and `excluded` does not. Measured: `excluded.fallback` 1 with `timeline` empty and `dropped.frames` 0. The two are **never summable**, which is why I54 says so rather than leaving a reader to derive it from a session that happens to agree. |
| Q4 | a fallback at tier `counters` | R-u | no record is pushed at all — `endFrame` increments `frames` and returns above the `spanning()` guard — so `timeline` is empty and `excluded.fallback` is 0 while `frames` is 2. The projection question does not arise below `spans`, and a row asserting I54 at `counters` would be vacuous rather than passing. Measured. |
| Q5 | a fallback → `toNdjson` | R-v | the line carries `outcome: "fallback"`. **This is where the invariant becomes observable**, and it is also why no row could have caught the defect by reading the report: the export has published an `outcome` column for its whole life, and the column had one value in every session that has ever run. → T3.15 |

**What the walk changed before the code.** P4 stopped an invariant being written that is false in one
cell by construction. Q1 is the measurement the ruling rests on — the old filter's cost was not only
the missing frame but a ring slot spent on a record nothing could show. Q2 named what the ruling
leaves behind *before* it was written down, which is what C23 §8a A4 is the argument for. Q3 and Q4
are the two sequences that would have made a naive I54 false and vacuous respectively.
...
- **T1.16** (I23): a report at tier `spans` with **nothing recorded** → every card draws **no plot at all**, and every card whose register row names a form draws a notice. Asserted over the whole deck, because the two panes that were right were right by accident: `frame` and `memory` had no tier-shaped field to reach for, so they asked about data, and a row naming the pane that broke would have been satisfied by the accident and blind to the next one written from the same template (F895) — an argument that is stronger at thirty-seven cards from five templates than it was at four panes. **The one card with no form is counted rather than excluded**: `the-instrument` is a `kv` of spans measured, frames dropped, the tier and the node version, every one of which is a count that exists at `off`, so a notice there would be a card refusing to draw numbers it holds. The fixture asserts its own emptiness first — a setup that quietly recorded a frame makes every assertion here vacuous.
...
- **T1.16c** (I23): its control — six recorded frames with a span at `spans` → the **set** of cards that draw a plot, named rather than counted: eighteen of thirty-seven, from `verdict` to `the-pairs`. A guard widened until it refuses everything passes T1.16 perfectly, and a count would be satisfied by a deck drawing one figure where it owes eighteen. **The set is the reading in both directions**: every card absent from it is absent for a reason the fixture states — no resource sample, no cache, no gauge, no leak, no mark, no far-side span — so a card joining the list is a card drawing over data the fixture does not hold, and one leaving is a card that has stopped answering.
...
- **T1.33** (I31): one document measured three times inside one frame → `frames` is 1 and `measures` is at least 3, so `measures / frames` is above 1 while `renders` is 0. A node measured four times cheaply and one measured once expensively carry the same self time, and only this column separates them.
- **T1.33b** (I31, **the union control**): one block measured once and rendered once inside one frame → `measures / frames` is **1**, `renders / frames` is **1**, and `calls / frames` is **2**. Both halves, because each alone is satisfied by the merge this row exists to forbid: a single counter reports 2 for a block that was measured once, which is the reading eight of nine rows of a real session carried, and a split that dropped the sum would leave `panes.ts`'s seam-entry column with nothing behind it (F1098).
...
- **T1.35** (I32): two frames at `worst: 1` → the kept record has a `tree` and the other has **no `tree` member at all**. Absence reads as *not retained*; an empty tree reads as *this frame was not nested*, which is a measurement that was never taken. **The second half was in the row's title and not in its assertions** until F1020: three lines about the frame that *was* retained and none about the one that was not, so `strip` returning every record with its tree — the retention bound gone entirely — left the row green and failed one tier-3 row about a different invariant. It now reads the un-retained frame off `timeline`, which carries it (I54). A comment describing an assertion is not the assertion.
...
- **T1.42** (I30, **the control for the split**): `plot.area.cells` at 200, 2 000 and 20 000 samples in a 4-row box → **700 in all three**, and 4 300 at height 40. A renderer attributing everything to one phase satisfies T1.40 and T1.41 exactly, so only a comparison separates the phases — and the comparison is the box rather than the data, because the downsampler collapses a series onto the area's columns before anything is drawn. Two configurations differing by 100× in data give an equality where every timing assertion would give a threshold. The split itself is asserted on the kept frame's tree: `plot.area` holds neither `plot.layout` nor `plot.furniture` and the three are siblings — exact, where the ratio between the height axis and the samples axis that stood here was carried by the merge and not the raster, and fell to 3.9× against 4.0× under the suite's load the day the merge became linear (F981, F983). What a clock still has to answer is that equal cells is equal work: a hundred times the samples reads the area under 40×, where a renderer visiting the samples per cell reads fifty or more.
...
- **T1.102** (I53): a profiler over a real recording tap, a probe that echoes the stamp it is handed, and a held timer → three ticks take no read on the recorded `elapsed` and each sample's `at` is the sampler clock's; the recording's `mono` array holds exactly what the tap saw, which is construction's 2 003 — the cost measurement's 2 002 and the start stamp — and nothing from the ticks. **The control is the tree's old state**: with no `sampleClock` the tick stamps from `elapsed`, one recorded read per tick, so the row is shown able to see the channel and a profiler built without a recording keeps the spans' own axis.
- **T1.103** (I14, I46): a recording with a `far` line after two frames, the stand-in gated on `driveRecording`'s release → the answer is not served before its turn, it is served after the frame recorded ahead of it and not before (the harness's write count at settlement is the prologue's three plus the two frames), and the stand-in reads the mono clock exactly twice per answer — the start read at the call, where C06 takes it, and the end read after the turn. **The control is the ungated stand-in**, which serves after one turn with nothing drawn: the state the tree was in (F974).
- **T1.104** (I53): `resolveConfig` with a recording on → `sampleClock` is the given `elapsed` **by identity** and `elapsed` is not; two reads through `sampleClock` and a flush leave the recording's `mono` batch empty, and one read through `elapsed` puts its value there. The recorder row cannot see the wiring — T1.102 shows the recorder stamps from whatever it is handed — and this is what shows the root hands it the clock before the tap, threaded rather than read anew.
...
- **T1.108** (I56): a `Hist` fed a known population answers `q1` and `q3` at the quartiles of that population, and a `QuartileSummary` built from the snapshot is accepted by `b.plot` on `boxplot`, `violin`, `forest` and `bullet` — the four forms that take one. The second half is the point: a five-number summary that the form refuses is three numbers and a type error waiting for a caller. **The reading is checked against the published `error` rather than exactly** — the buckets are logarithmic, so an exact assertion would be about the bucket layout — and it is the row that found F1140: the bound held only once the estimate moved from the bucket's edge to its midpoint. Monotonicity is asserted with the three bounds, because a pair computed at the wrong quantiles fails a bound while a pair *swapped* satisfies both bounds read as a set.
...
- **T1.109b** (I57, I41): every card in the register declares the population its spans came from, and a card reading no span data declares none — driven over the whole deck, because the claim is about the register and not about a card. **The control is that both populations are actually in it**: a deck where every card said `frame` satisfies the per-card assertion and means the session-site half is drawn nowhere, which is the state §3c was in before the `gantt` was recut (F1131) and the state a reader could not see.
...
- **T1.129** (I65, F1193): the locator over a chunk written to a temporary directory beside a map that names it — a frame at the chunk's line and column answers the map's original source resolved against the map's location and the original line; the same frame with the map file removed answers `null`; with the map replaced by text that does not parse answers `null`; at a position the map does not cover answers `null`; and the map is read once across three frames in one chunk. `foldCpuProfile` with the locator draws the located name and without it the URL, over one synthetic profile.
...
- **T2.3** (I13): a sample at `resolution: 10`, fired until the figures have a population → `loopDelayResolutionMs` is 10, and **every card whose figure is about the loop carries the resolution and the qualifier**. The row sweeps the deck by what a card *draws* — a `plot` block mentioning the loop — rather than by what the register declares, since a loop over the declared set would ask the register about itself; the two lists are then compared, so a card that starts drawing a delay without declaring one fails here rather than shipping a bare percentile. **The discriminator took two drafts and the second one found three more cards**: *the card mentions the loop* matched the verdict, whose contents hint names every card in the deck and therefore names `the-loop-spectrogram` — a menu entry read as a figure — while `vitals`, `co-variance` and `the-pairs` genuinely draw a delay series and carried no resolution at all (F1137). **Fired four times, because the fixture must respond**: the spectrogram needs three sampled windows and the utilisation two, so a single sample satisfies *the resolution is on the sample* over a deck where no card draws the figure.
...
- **T3.3** (I13): a sample over a window shorter than the resolution → every delay figure is below the resolution and carries it, so nothing presents a sub-floor number bare; **and with `loopDelaySamples` at zero the footer says `no window sampled yet` and quotes no resolution at all**, which is I13's second clause and the answer to F898. Both arms in one row, because the interesting cell is the boundary between them: a floor reading and an absence look identical in the sample and must not on the screen. The sentence lives in the **generated** footer rather than in a drawing, so there is one place for it to be and one place for it to go missing (F1137); the panes it replaces carried it in two `keyValue` rows, and this row's first draft was satisfied by either of them.
...
- **T3.7** (I6): a composition that throws with two spans open → the frame is counted, excluded from every duration, and both spans close at their own self times rather than being handed the frame's tail by `freezeTree`. **`worst` is empty and `timeline` carries it** (I54): the exclusion I6 asks for is from the durations, and the frame that gave up is on the session's series with `outcome: "fallback"`. The row asserted its absence from both for as long as one filter served both purposes — which is what a row asserting a defect does. It is green for exactly as long as the defect lives and says nothing when it gets worse (F899, F1020).
...
- **T3.10** (I27): a report whose newest sample is suspended → the figures come from the last **running** sample, the card's footer says how many of how many were suspended and that the series is not continuous, and a report whose samples are **all** suspended draws a refusal and no series at all. **`suspended` was on `ResourceSample` from the start and no consumer read it** (F900): an idle machine and a paused one drew the same picture, which is the reading I27 names — and the deck lost the caption again when the panes were replaced, because a coverage gate keyed by report field cannot see a sentence inside one (F1137). Its control is a report with nothing suspended, unchanged — a caveat on every report is a caveat nobody reads — and the control asserts the **clause** rather than the word, since a utilisation card draws a series labelled `suspended` and a `not.toContain("suspended")` fails for a reason that has nothing to do with the caveat.
- **T3.10b** (I27): a sampler tick that falls inside `suspend()`/`resume()` takes the flag from the lifecycle rather than from its caller. **Deferred**: neither method exists. `ResourceProbe.sample(suspended: boolean, at: number)` takes it as a parameter and the only caller passes a constant, so nothing can put the session into the state this row is about.
...
- **T3.15** (I54): three arms, each a cell of §9c, and the third is the one that could not exist before. **The ring against the filter** (Q1) — a 40 ms drawn frame then a fallback into a ring of one → `timeline` is the fallback alone, `dropped.frames` is 1, `frames` is 2, `worst` is empty and `latency.work.max` is 40. The bound was spent on the fallback either way; before I54 this published an empty series for a session that drew twice. **The cause decides, not the outcome** (P4) — a fallback raised by the profiler's own commit is on no timeline and is counted in `excluded.fallback` rather than `excluded.selfInflicted`, because the exclusion is a ladder; **its control is the same fallback with no own commit**, which is on the timeline, so a filter that dropped the outcome test entirely fails here. **The value is observable** (Q5) — `toNdjson` over the first arm carries a line whose `outcome` is `"fallback"`, which is the published column the old filter held constant in every session that ever ran.
...
- ~~**T1.92**–**T1.95**, **T1.97**–**T1.99**, **T3.12**–**T3.14**~~ — **struck with the pushed view** (F1254, §3c, R-EXA-082). Ten rows over the layer: the `own` bracket around its commits, the tier raised at `open` and restored from C15's change stream, the timer, the block-boundary window, the refusal of a second `open`, and the resize. **Two of the claims outlive the rows and are asserted where they still hold** — a removal emits one change carrying its id before the removing call returns is C15 I25's and is exercised there, and *with no profiler the verb says so* is now every arm's notice rather than one owner's refusal string.
...
- **T4.9** (§3c, C09 I49, with C22 I49): the view on a graph whose `capabilities` override says `unicode: "ascii"` → the card's generated footer carries the ASCII separator; the same graph without the override → `·`. **The capability record the view hands over is `detection.capabilities`, resolved after C22 I49's overrides** — a view reading the environment for itself, or taking the deck's default, is F124's second derivation and F828's wide cell.
...
- **T5.1c** (I14, I53): recorded with a 1 000 ms pause between the answer and the resize, so that a wall-clock second boundary always falls between the answer frame's last read and the resize repaint's header read → `consumed.wall === recorded.wall`, **`consumed.mono === recorded.mono`**, and identical. Without a transport stand-in that reads the clock where C06 does the row is red on every run — 52 consumed against 54 recorded, diverged at the resize repaint's header (F963). The pause *is* the window, which is why the row takes it as a knob rather than waiting on a race. **Mono was reported and not asserted** until F971: the count differed by one read per second of session and the comment guessed at the `^D` path; tagging every mono read by its caller put the whole difference on the sampler — 2 against 0 over three seconds, 5 against 0 over six, every other reader equal — and with the sampler on `sampleClock` the counts agree at 2 908 on a 2.8 s session and on a 5.5 s one.
- **T5.1d** (I14): recorded through `CALCIUM_FARSIDE` with a far side that sleeps 2 500 ms before answering → the recording holds the readout's wake frame **ahead of the `far` line**, `ps(--limit 20) · ⠙ 1s`, and a settled head carrying a whole-second figure and the row count, `· 2s · 20 rows`, matched by shape rather than by the second it names, which no `CLOCK_DERIVED` member matches; the replay is identical with `stalled` 0 and both channels at parity — the figure reproduced and not excused. **Two wake periods inside the call, not one and a fifth** (F1081): at 1 200 ms the wake had 222 ms of margin and lost it once, and the same recordings put the settled figure 41 ms from the second boundary that flips it — two races on one knob, pulling opposite ways, so widening alone moves the failure rather than ending it and the figure is read out of the frame instead of pinned. **The falsifier for three seams at once**: the sampler on the recorded channel (I53), the transport's pair on the wall clock while the stand-in mirrors mono (C06 I19, F972), and a stand-in serving the answer before the wake's frame (F974) each fail it, measured — on the tree before them the replay diverged at frame 5 with three stalls, wall 54 against 59 and mono 2 906 against 2 994. **What it does not catch, measured**: the card's figure taken from the wall clock survives it, because a positional channel reproduces whatever is computed from it; that seam's row is C23 T4.68, which skews the wall clock alone (F975).
...
- **T5.5** (I65, F1193): a fold over a profile whose one node names the built `dist/bundle/` chunk holding `createBlockRegistry`, at that function's line and column found by reading the chunk, draws the frame at `src/presentation/blocks/registry.ts` and the line of the declaration in `src/` — the built artefact, not a synthetic map. Tier 5 because it reads `dist/`, which `make test` does not build and `make e2e` does.
...
- **T6.17** (I53): the tick stamping from `elapsed` — `probe.sample(suspended, elapsed())` — → T1.102 fails on the recorded read count and T5.1c on mono parity; the root handing the tapped `elapsed` as `sampleClock` → T5.1c fails and T1.104 cannot see it (the wiring is `session.ts`'s, one file below `resolveConfig`), which is why that run's expectation is tier 5's and listed as such in `anchors.mjs`; `sampleClock` resolved to `ambient.elapsed` rather than the injected clock, or to the tap → T1.104 fails, on identity and on the batch respectively. **The first is the natural line rather than a mistake**: it compiles to a TS6133 on the now-unused `sampleClock` and still emits, so the build's exit and not its output is what says so.
- **T6.18** (I14, I46): the stand-in taking no mono reads → T1.103 fails on the count and T5.1c on parity; the stand-in serving without awaiting its turn → T1.103 fails on the write count at settlement and T5.1d on `stalled`, with T5.1c green — a fast far side answers inside one frame, so only the slow-far-side row can see pacing (F974).
- **T6.19** (I54): the revert — build `timeline` from `drawn` again, one filter for both projections. It reads as a simplification and it is what shipped: the series and the durations are both *the frames that drew*, and I6's exclusion is applied once instead of twice. **T3.15's first arm is what fails** — a ring of one holding a fallback publishes an empty `timeline` for a session that drew two frames, so `frames`, `dropped.frames` and `timeline.length` cannot be reconciled by any reader — and its third fails with it, because `FrameRecord.outcome` returns to a published member with one realised value on every surface. **The revert is invisible to every other row in the suite**, which is how it shipped: no assertion anywhere disagreed with it, and the only frame that could have was one nobody could read (F899).
- **T6.20** (I65, F1193): reading the map at start instead of at the fold → no row here fails and C24 T5.8's module-count arm does not either — the cost is a duration, which is why I65 states it and A04 §5 carries the measured 15–60 ms rather than a row; dropping the *keeps its URL* arm so an unlocated frame is dropped → **T1.129**'s three `null` arms turn into a shorter tree and the attributed total no longer equals the sampled total (I34).
