...
- **T1.16d** (I23): `setTier("spans")` from `counters`, then the view closes → the tier is `counters` again, not `off`. **Deferred on the interactive view**, which is the drawing round: `profilePane` is a pure function from a report to blocks and raises nothing, and no caller in `src/` opens or closes a pane. This half was bundled with T1.16 under a single *not deferred on a component* marker that was true of neither — the pane function had existed all round, and the raise/restore has no subject at all. N subjects, N blockers.
...
- **T4.4** (I12): the profiler view open → its own frames are excluded and `excluded.selfInflicted` is non-zero. **One blocker now, and it is the surface.** The origin travels with the *call*: `profiler.own(fn)` marks every commit raised inside it, synchronously and depth-counted, and the commit seam reads the bracket rather than trying to infer an origin it was never handed — the block id that would distinguish a self-raised frame is C23's and does not travel with a commit. What is still missing is something to bracket: `profilePane` is a pure function from a report to blocks with no caller in `src/` outside `profiling/`, so there is no profiler view to open. That arrives with the drawing round; the mechanism is asserted meanwhile by T1.88.
...
- **T6.16b** (I47): recording the capability verdict and replaying it → T1.83 fails. The mutation is the obvious saving rather than a mistake, and what it removes from the gate is the whole of C02.
