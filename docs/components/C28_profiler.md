...
- **T4.69** (I65, F1193): a fold over a profile whose one node names the built `dist/bundle/` chunk holding `createBlockRegistry`, at that function's line and column found by reading the chunk, draws the frame at `src/presentation/blocks/registry.ts` and the line of the declaration in `src/` — the built artefact, not a synthetic map. Not deferred on a component: the code commit replaces this row.
