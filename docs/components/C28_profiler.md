...
- **T1.83** (I47): a recording replayed → the detector runs again and its query escapes appear in the replayed writes. **The fabricated violation is a recording that carries the verdict**: the replayed session skips the queries, so its first bytes differ and every byte after them agrees, and a detector returning the wrong answer replays byte-identically because both runs trusted the same recorded one.
