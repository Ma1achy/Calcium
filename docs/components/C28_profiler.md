...
- **I51** — **The view redraws on the injected timer and on a key, never per frame, and what it draws is the one-row header plus a block-boundary window of `profileDeck(report, section, index, region, caps)`, measured through the registry with the terminal's capabilities.** One layer id, updated in place; a redraw after the layer has left the stack changes nothing, because `update` on an unknown id is `false` and raises no commit (C15 I14). **One `report()` per draw**, taken by the caller and handed to the header and the card both: the first cut had each pull its own, which is two readings a second of a value that costs something to produce and, on a clock that advances per read, two readings that disagree — a header naming a deck of one length above a card resolved against another. **The header is chrome and not a member of the window**: a card is one `panel`, so a projection treating the header as the window's first block showed the header *instead of* the figure at any region the card did not fit. **A card taller than the region is shown under a notice that says how many rows are hidden** (C15 I8), and those rows cannot be reached — the windowed sequence is one block long, so every offset clamps to zero and the four page motions answer `false` at every card. That is recorded rather than designed (F1138): the remedies are a row window through the registry, which is a seam C09 does not offer, or a refusal above the form (I60) with a card's non-figure rows inside the floor comparison, which is the smaller of the two.
- **I52** — **The verdict card measures at most 23 rows at 80 columns for every report — a 24-row terminal minus the view's one-row header — through the registry that draws it.** The card a reader opens first is the one that must fit without paging; every other card is one figure sized to the region it is drawn at. Its parts are bounded by closed sets — `checkBudget`'s rows, which are A01 Appendix B's six, a two-row `kv` of the regime and a one-line key hint — so the bound is a property of the type rather than of a fixture. **The bound is the rule and the measurement is beside it**: 11, 15, 16 and 16 rows with the header at 80 over the four report fixtures — empty, twelve frames, twelve measured, twenty over every `CommitReason` — so a card that grows four rows inside the slack the bound leaves fails T1.100 before it reaches a screen. Measured through a registry with `tableDefinition` and `plotDefinition` registered, because the harness's default has neither and measures a plot as its JSON, which is how a 40-row pane was once recorded as 26 (F947, F959, F960).
...
- **T1.98** (I51, C15 I8): a card at the region it was built for → the layer is the header and the card, both within the region, and the four page motions answer `false` because there is nothing to page. The region then shrinks under it, as a resize does before the next tick → the card is shown under a notice naming the rows that are hidden, **without** the document view's *n/p move by block* clause, because here `n`/`p` switch cards; the header is still the first block, which the projection that treated it as the window's first member did not manage — it showed the header *instead of* the figure. Asserted on the layer's content, because a window that fits is indistinguishable from one C15 clipped unless the blocks are counted.
...
- **T4.7** (I50, I51, with C16): the same view, `n`, `tab` then `Esc` as bytes → `n` crosses from the verdict into `app` and the header names the group it arrived in, `tab` moves to `framework` and back to that section's first card, and `Esc` reaches this owner through `viewPop` — stack empty, tier restored — rather than the document view's `pop`, which is the owner `keys.ts` asked first before this round. **All three keys through the keymap**, so the row fails the day the third owner falls out of `keys.ts`'s dispatch while the bindings stay in `/help keys`; and `tab` is driven as bytes rather than called as a member, because a binding added to a job and not to the table is F1090's defect.
