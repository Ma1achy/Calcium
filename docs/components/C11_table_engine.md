...
21. **A bar or spark cell draws its glyph, inside the width the column planned** (I23). C04 obliges the mark and this is where it is spent; the series takes what the mark leaves, and where neither fits the series keeps the column (→ C04 I6, C12 I25, FINDINGS F1104).
...
- **T1.26** (I23): a table cell carrying `bar`, `tone: "error"` and `glyph: "warn"` in a column planned at 17 → the rendered cell is `⚠ ████████ 101.2%`, **exactly 17 cells**, with the mark its own run so a span's offsets stay offsets into the text. The control is the same cell with no glyph: 17 cells and a run two longer, which is what says the lead was taken from the series rather than added to the column. Asserted at `colourDepth: 1`, because the tone is unavailable there and the mark is the whole of what separates a busy container from a quiet one.
- **T1.27** (I23): the same cell in a column planned at 3 → the glyph is **dropped** and the series keeps all three cells. A mark that pushed the quantity out of a narrow column would answer a question the cell is not asked, and the row is the boundary rather than a width nobody uses: `spark` at width 1 is C12 I13's own case.
...
- **T3.21** (I23): at `ambiguousWidth: "wide"`, a glyph whose rendering is two cells beside a bar in a column planned at 17 → the cell is 17 cells and the series is two shorter than at `"narrow"`. The lead is measured with `cells()` at the session's convention and not assumed to be one, which is the arithmetic C12 I24's four crooked gutters were made of.
...
- **T6.18** (I23): restoring the early `return` in the `bar` branch before the glyph is read → T1.26 fails on the mark and the golden `table-value-bar` moves, which is the shipped behaviour this replaces. Handing `valueBar` the full `planned.width` while still drawing the lead → the cell is 19 cells in a 17-cell column and T1.26's length assertion fails, which is the plausible half-fix: the mark appears and the row beneath it starts two cells right. Dropping the series instead of the glyph when neither fits → T1.27 fails.
