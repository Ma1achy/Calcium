...
- **I28** — *(§078 `R-TBL-004`, `R-TBL-005`, I27, I26, C04 I86)* **A numeric column groups its thousands, and it is a property of the column rather than of a cell.** §078 states both halves of the rule in one line — *thousands are grouped, units travel with the value: `1,204` and `184` · `2m` and `41m` · `4.2s` and `0.02s`. A units row in the header is a row you have to look back at* — and the two halves land in different places. **The unit half is already satisfied and by a shape rather than by code**: the unit travels *in the cell's text*, `src/presentation/table/sort.ts` `asDuration` parses `12m`, `1h 12m` and `2h ago` back out of it *since the unit carries the magnitude*, and no units row exists anywhere in the render. What the rule forbids there is a header row, and there is none to remove. The grouping half is what is built here.
  ...
  **Which columns group is one predicate with three clauses, computed once per block beside the alignments and the points, and each clause is all-or-nothing over the column.** That shape is not a convenience: a column that grouped some of its rows and not others would put `1,204` above `41208`, which is worse than grouping neither and is exactly the comparison `R-TBL-001` says a reader makes by shape without reading.
  ...
  1. **The column is numeric** (I27). A year, an id, a port and a line number are all digits and none of them groups, and the repository cannot tell them apart — but it does not have to, because the only ones it groups are the ones its own values classify as quantities, and a column holding one non-numeric value is text by I27's agreement rule and never reaches this at all. **The clause's only live subject is a duration**, which is the finding the mutation pass produced: dropping it failed nothing, because a text column is already `left` and never arrives, and the one kind that resolves to `right` without being a bare quantity is `12m`/`1h 12m`. A five-digit duration would otherwise draw `10,000s`, which the design shows nowhere — §078's durations are `2m`, `41m` and `1h`, none near a thousand — so bare is the reading that invents nothing, and T1.35's fourth arm is the row that was missing rather than a licence.
...
---
- **I29** — *(§078 `R-TBL-003`, question 13)* **A missing number is the absent mark, never zero and never blank.** A cell that is absent or empty in a column whose values are numbers or durations (`columnKind`, I27) draws the absent mark, `pairFor(caps).absent` — `—`, or `-` at ASCII and at `ambiguousWidth: "wide"` — in `muted`, **where the column's values end** — under the last digit in an aligned decimal column (its point plus its widest fraction, `decimalEnds`), and at the column's inline edge otherwise — which is where §078 draws it in both its decimal and its duration column. The first build put it at the edge, and a decimal column wider than its values drew the dash two cells right of every digit above it. **The one mark the tree already draws for a missing value** (C12 I20's bar), so one fact keeps one character. A text column's empty cell stays blank: §078's rule is about numbers, and a dash in a name column would be a value. And the mark is the framework's, so a producer need not write `—` — which, written, would classify the column as text (`isMissing` is untouched: a placeholder a producer writes is data). → T1.38
- **I30** — *(§088 §4, `R-COL-006`, C04 I128, C09 I111)* **A trend cell draws its arrow in the tone its column's polarity gives it.** The arrow is `trendUp` when `to > from` and `trendDown` when `to < from`; the tone is `ok` when that direction is the column's `polarity` (`higher` with up, `lower` with down), `error` when it is the other one, and the cell's default when the column is `neutral` or undeclared. The arrow leads the cell's text as a glyph does (I23's lead), so it is inside the planned width. **A trend with no movement draws no arrow** and the text alone, in the default tone — the design draws no flat trend, so this is taken rather than ruled (question 37). **Not built until question 38 rules `trendDown`'s ASCII half** (C09 I111). → T1.39
- **I31** — *(§078 `R-TBL-005`, I10, I7, I27, I19)* **A number column is planned at its widest value, so it is shown whole or dropped and never cut.** §078 rule 5: *a number column never truncates, because half a number is a different number* — and `planColumns` reads no content (I7), so a bare `41208` was cut to `4120…` in a four-cell column like any name. **The planner stays pure**: the definition hands it an effective `minWidth` for every column whose values are numbers or durations (`columnKind`, I27) — the greater of the declared minimum and the widest **bare** value the column holds, in cells — and I10 does the rest, since a column whose `minWidth` equals its longest value is shown whole or dropped. Bare, not grouped, because I28 decides grouping from the plan and the plan would otherwise depend on itself; a column that cannot afford its separators draws bare (I28 clause 3), which is whole. **A window pins it** as it pins alignment: the effective minimum is resolved over the whole table and written into the window's column declarations, so scrolling never re-plans a column from the slice it happens to show. `measure`, `render`, `window` and the row detail read one helper (`plannedColumns`), so none of them plans against different columns. **The other clause of `R-TBL-005` is not this invariant**: *the widest text column gives up its width first* needs each column's natural width as a planner input, and a window has no declared field to pin it in. → T1.40, T2.16
...
- **T1.38** (I29, R-TBL-003): a table with a numeric column, a duration column and a text column, one row missing all three → the numeric and duration cells draw `—` in `muted` — the metric's under its last digit, the age's at the column's edge — and the text cell is blank; at ASCII and at `wide` the mark is `-`. The control is the same row with values, which draws no dash.
- **T1.39** (I30, R-COL-006): §088's three rows — `lower` going down is `↓` in `ok`, `higher` going up is `↑` in `ok`, `lower` going up is `↑` in `error` — and a `neutral` column's arrow takes the default tone; `from === to` draws the text with no arrow; at ASCII the arrows are `v` and `^`.
- **T1.40** (I31, I10, R-TBL-005): a four-cell numeric column holding `41208` and `88` plans at five cells and draws `41208` whole; at a width with no room for five it is dropped, never cut; a text column at the same `minWidth` still truncates. A duration column holding `1h 12m` plans at six.
...
- **T2.13** (I14, C10 I48, §4k.2 row 1, F1240): **a focused row and a selected row keep every cell's own tone.** On a table whose `state` cell is `tone: "error"`, the failed cell's ink at 24-bit is the theme's composed `tone.error` **for the ground the row took** — `surface.focusGround` under focus, `surface.selection` under selection — and not `accent` or `default`; a span's own tone survives the same way. Asserted through `inkOn` rather than against a hex, so the row is the rule and not the theme's current values. **And on a banded theme it is the band's single ink**, which is the same rule and not an exception (R-THM-005).
...
- **T2.16** (I31, I19, I27): a window over rows whose own values are narrower than the table's widest number plans that column at the **table's** width — the window's column declarations carry the whole table's effective `minWidth` — so the column starts at the same cell in every window.
...
- **T6.27** (I28): four mutations, run by hand on landing. Dropping clause 3 → **T1.34, T1.35 and T1.37**, the column grouping past its planned width. Dropping clause 2, the spans guard → **T1.36 alone**. Measuring the point before grouping → **T1.37 alone**, with no width assertion anywhere moving, which is why that row exists. Dropping clause 1 → **nothing**, on the first pass — a mutation that fails nothing is a finding about the tests, and the missing subject was a duration column, now T1.35's fourth arm.
