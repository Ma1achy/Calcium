...
- **T1.69** (I73, I36, §7f): choosing `reply…` moves the same question from replacing to floating, with its id and its owner unchanged, and the prompt comes live beneath it. The discriminator is the **identity**: a build that closed one layer and opened another passes every assertion about what is on the screen and drops the handler that is awaiting the answer. Then the answer carries both facts — the reply choice's `key` and the composed `text` — and escaping instead resolves with the default's `key` and no `text`, which is the arm that says `text` is a record of what happened rather than a field that is always filled.
