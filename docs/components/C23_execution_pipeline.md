...
| `expand` | An `op: "expand"` patch toggling the flag of a table's row **or a tree's node** (C04 §4, C04 I129), at `"shell"` origin. The op names the operation readably; the origin is what gets it past a settled entry (C13 §6). **`target` names a row, so the block holding it is found first — at any depth, from the same walk the fold arm reads** (→ C04 I98, → C04 I115) |
...
- **I28** — A submission clears the prompt, whatever becomes of it. The clear sits between the parse and the route, so a refusal, a parse error and a successful verb all leave the same empty prompt — bash's behaviour, and the only one that does not require the user to work out whether their line survived. Restoring it on refusal was the alternative and it is worse: the notice says what happened, and a line that sometimes stays is a prompt whose contents depend on a decision made after the keystroke. **A typed reply is not that case, and the difference is who took the line.** §101's third row hands the prompt to a question that is already open: the reply borrows the line, so the draft that was there is **held on entering the reply state and restored exactly on settling** (C17 I28), whichever way the question was answered. Nothing here depends on a decision taken after the keystroke — the reader's own line comes back because it was never submitted, and the reply's line clears because it was. The argument above is about a line whose fate is *decided*; this is a line that was *borrowed*, and the two would only be one rule if the prompt had one owner.
...
- **I41** — **`height` is `null` on every route, because no route defines a region for a producer** (C07 I18, C22 §13a, R-EXA-082). It read *non-null on the view route and `null` everywhere else*, decided from `isViewInvocation` before step 3, and the view route is what the design removed: a verb's result is an entry, and an entry is as tall as its blocks. **Amended rather than retired, and it is stronger for it** — the old form had one arm nothing could make false without a second view route, and this one is a property of every call site in the file. A producer that wants a bound asks for one; nothing hands it a region it did not ask about.
...
- **I73** — *(§7f, §101, `R-QST-001`)* **Whether a question replaces the prompt is derived from whether its answer needs one, and never declared by its caller.** A question with choices alone replaces the prompt, because there is nothing to type and a prompt with no job is a row spent on nothing; a question taking a typed reply floats above a live prompt, because the answer is composed there. **The two are one question in two states**, not two kinds: choosing `reply…` moves the question up and brings the prompt live beneath it, so a caller that declared the placement would be declaring it for a state the question has not reached yet. A peek, a completion menu and a search float and do not block; a typed reply floats **and** blocks, which is the combination that shows the two axes are independent — and it never becomes dismissable, because an owner is still waiting.
  ...
  **Floating is an ownership fact before it is a placement.** A question that has moved to the reply row owns exactly two keys — `⏎`, which answers with that choice's `key` and the composed `text`, and `Esc`/`⌃c`, which answer with the default's `key` and no `text` — and **passes every other key down to the prompt beneath it**. That pass-through is what makes it float: a build that drew the question above a live prompt and went on consuming keystrokes satisfies every assertion about where the rows are and leaves a prompt that cannot be typed into. The accelerators go with it — while a reply is being composed `y` is a letter, not a choice, or a question whose choices spell a word would be unanswerable by typing it.

  **The composed line is the prompt's, read at the keystroke and cleared like a submitted one.** The text is read when `⏎` arrives rather than held as the reader types, because the prompt is the record and a copy taken earlier is a second one; and C23 I28's *the prompt clears whatever becomes of the line* reaches the third state unchanged — the reply **became** the line. The draft that was there **before** the question is a different fact and is not this seam's.
- **I74** — *(§7f, C22 I80, §101)* **Replacing is the prompt's rows answered by something else, not a second layout.** The frame's height identity is `header + rule + region + rule + prompt + footer`, and the prompt's rows are read by the measurer and by the paint from one function; a replacing question answers that function instead of the editor, and both readers come to the same place. **The region shrinks by exactly what the question takes, and that is the claim rather than its opposite** — an earlier wording said the region does not move, which the identity forbids: a question taller than one row cannot occupy the prompt's rows and leave every other term where it was. What *is* preserved is that the identity holds, that nothing is drawn over the transcript, and that no second layout exists for those rows — C22 T6.30 records what two records of that one number cost, a wrapped prompt drawn as a lone elision marker. The editor is not drawn at all while a question replaces it, and neither is its caret or its selection: the rows on screen are the question's, and a caret placed by the editor's arithmetic would sit inside a box the editor has no coordinates in.


- **I75** — *(§7f, §051, §098, §106, `R-QST-002`, `R-QST-004`)* **An inspection suspends a question; it is not an answer, and every question resolves exactly once.** The payload of an approval is **replaced** rather than marked when it does not fit (`render`'s cut arm, entry 16 R2), and that ruling is right about the frame and leaves the reader approving a change they cannot see — so the way to the evidence is an **inspection choice**, a third peer beside the answers. Choosing it does not settle: the same layer, with the same id and the same owner awaiting, shows the payload bounded, and `Esc` inside it **leaves the inspection and not the request** — which is the whole of *suspends*. A suspended question is **unresolved**, so a second question still waits behind it (§106 case 3) and the entry's own settlement is as far away as it was.

- **I76** — *(§036, `R-PRG-001`, `R-BLK-247`, `R-BLK-249`, `R-BLK-261`, `R-BLK-263`, `R-BLK-265`, → C09 I104)* **An operation's head is a verb with a parenthesised aside while it runs and a flat head once it stops, and the bar goes with the motion.** §036 draws both and they are not the same line. *The parentheses group the two as an aside, so the eye reads the VERB first and the numbers second* — so while the operation runs the elapsed and the delta are **one grouped field**, `Compacting conversation… (5m 38s · ↓ 8.0k tokens)`, joined inside the group by C09 I49's separator and never by a literal. **The group is a running device and it does not survive settlement**: §036's settled line is `● compacted · 6m 12s · 41k → 12k tokens · 18 turns summarised`, flat, every field a peer. Two correct statements meet here — *the aside groups the elapsed and the delta* and *a head joins its fields with the separator* — and only one of them applies at each end; the wrong answer, `● compacted · (6m 12s · 41k → 12k tokens)`, satisfies a row that only ever asserted the running line. **An empty group is not drawn**: an operation with neither an elapsed nor a delta is the verb alone, and one with a single member takes no separator, because parentheses around nothing say *these are the numbers* about no numbers — I49's argument for the separator being a slot, arriving at the bracket.
- **I77** — *(§052, `R-QST-003`, → C17 I29, C16 I54)* **A typed reply's history is its own.** While a question composes a reply, `↑` and `↓` walk **the question's** history, never the prompt's commands; the question's history is taken with the borrow and holds what was submitted through it, which for a question asked once is nothing. **`⌃R` is not the reply's at all.** The reverse search is a panel over the prompt's commands (C20 §7, C15 §2c), so opening one from inside a reply raises a second owner from inside an unanswered first; it meets C16 I8's reject with the rest of what C16 I54 does not forward. **And `↓` past the end stays in the question.** At the prompt, C20's walk has a floor and `↓` past it enters the live block (C22's effect table, C16 I22); at a question that is a focus move out of a layer that must be answered, which I8 forbids — so the floor is the end of the walk and nothing more. §052 lists *history* among the four a question owns, so the prompt's command history appearing in a commit-message reply is the leak `R-QST-003` names, not a convenience; and C22's comment calling the one editor *§101's whole point: the same history* was a claim no design file makes.
- **I78** — *(§047, R-HON-002)* **A stale reading's age is the reading's, and it is kept true.** Two parts. **It is measured while a fetch is in flight**: a hung fetch is the stale reading §047 draws — the last good content standing while nothing replaces it — and the sweep skipped every source with `inFlight` set, so the one case the notice exists for could not reach it. **And it is rewritten when the figure changes**: the age was computed once at the moment of staleness and never again, so a panel hung for an hour said `120s ago`. The driver asks C09's `age` whether the text would change, `countdown`'s rule (I52), and wakes once a second while a visible part is stale. Only `staleForMs` moves: the part's own block is not re-rendered, because nothing new arrived.
- **I79** — **A key's emission is not a submission** (C16 I57, `R-KEY-005`). `emitLocal(line)` classifies the line as a typed one is and runs its `local` handler, completes and cards the document as `runLocal` does, and appends it through `appendAndCommit` with **no settle** — the documented non-submission path, so I29 records nothing. It clears no prompt (I28 is about submissions), takes no guard and so is never queued (I5 is about submissions), and a line that does not classify as `local` emits nothing. Its only caller is C16 §6a's help action; a second caller is a key that means *show*, never *run*.
- **I80** — *(§075, `R-HON-008`, parked 28, 43)* **The configuration table is `key · value · source`, and the source cell's tone is the ladder.** `configBlock(settings)` is one `table` block, one row per setting in the order given; the `source` cell takes `muted` for `default`, `meta` for `config`, `warn` for `env` and `error` for `flag` — *the later it is applied, the louder it is, because a flag beating your config file is the thing you forget you did* (§075). It is drawn as a transcript entry, never a pushed view (§082). **§075's `⏎ edit` and `r reset to default` are not built**: nothing writes a setting, and an affordance naming a key that does nothing is C16 I19's second keymap. **The verb that answers with it is parked as 43**: §075 names it `/config`, and docker-tui already ships a `/config` of its own, so a framework verb of that name is a parse error for the example (C05 I6). Until 43 is answered the block has no verb, and `UNCONSUMED_MEMBERS` names the handler as its queued consumer. **And `env` and `flag` are refused today, by C04 I6** — *a `warn` or `error` tone requires a non-empty glyph* — which counts carriers per effect where tie-break 4 counts them per fact: the word `flag` is the fact's carrier and the tone is its second. Amending C04 I6 weakens an invariant, so it is parked as **44**; the ladder stays §075's as data, and neither source has a producer to reach it.

  **The bar goes when the motion does, and for two reasons that are not one.** *A 100% bar on a finished thing is a row spent on nothing*, which is settlement; and *a stopped bar is a claim about progress that is not being made any more*, which is cancellation and failure. Both end with no bar and the arguments do not substitute for each other — a rule written only about settlement is satisfied by a cancelled operation still drawing its frozen fill, which is the state that lies. So the predicate is **the operation is still running**, asked once, and `cancelled` and `failed` are inside it rather than beside it. What survives in all three is the head: muted for cancelled, error for failed, and the settled delta is the finished one — *41k → 12k, not "done"*.

  **The verb is the application's and the grammar is the framework's.** *The head names what is HAPPENING, not a tool — a gerund, because an operation is a process rather than a call*; nothing here can compose a gerund, and nothing here needs to. What this owns is the bracket, the separator inside it, the flattening at settlement and the bar's disappearance — and the delta's units are the operation's own, a string the caller supplies, because *a percentage is the same word for all four*.

  **One route in, and it is the one the reader can see** (`R-QST-004`). A key-only overflow path — a chord that opens the evidence without appearing among the choices — is a second mechanism for one fact, and it is invisible at exactly the moment it matters: a reader who cannot see the payload also cannot see that there is a way to. The inspection is a `Choice` with a label for the same reason the answers are.

  **The count is what makes it checkable.** *Resolves exactly once* is not satisfied by *resolves*: an inspection that settled and re-asked would draw an identical sequence of frames and hand the owner two answers, and an inspection that resolved on `Esc` would answer a question the reader was still reading. The row counts settlements across a suspend, an escape out of it, and an answer — one, at the end.
...
37. A bound is `null` on every route, because no route defines a region for a producer (I41, C07 I18, C22 §13a).
...
65. **An inspection is a choice that suspends rather than answers, and it is the only way to a replaced payload** (I75, §051, `R-QST-002`, `R-QST-004`). The question keeps its identity across it, `Esc` leaves the inspection rather than the request, and the count of settlements over the whole sequence is one.
66. **An operation's head brackets its numbers while it runs and flattens when it stops, and the bar goes with the motion** (I76, §036, `R-PRG-001`, → C09 I104). The aside is a running device and the settled head is flat; an empty aside is not drawn; and `settled`, `cancelled` and `failed` all end with no bar, by one predicate rather than three — a bar that stopped is a claim about progress nobody is making.
...
- **T1.36** (I35): a part whose data is older than `staleAfter` → its panel carries the age as `staleForMs` and its title is the declared one; the next success clears it **and the part never stopped ticking**.
...
- **T1.67** (I68, C05 §3): `FRAMEWORK_TOOLS`' `profile` row is `local` and takes two optional arguments — `section`, an `enum`, and `card`, a string — and the enum's `values` equal C28's `SECTIONS` followed by `snapshot` and `live`, member for member; and the eight names are the six, `profile` and `capabilities` (C22 I125). The L0 copy of an L4 list, held equal by the only file that may import both. **The two verbs are in the equality and not excused from it**: the enum holds two kinds of value, and a row asserting the sections alone passes on a menu that has lost a verb.
...
- **T1.69** (I73, I36, §7f): choosing `reply…` moves the same question from replacing to floating, with its id and its owner unchanged, and the prompt comes live beneath it. The discriminator is **what `settle` can still reach**, and the first three drafts of this row had it wrong. *One layer with that id on the stack* and *an owner still awaiting* are both satisfied by a transition that disposes the layer and pushes a replacement — the two draw the same picture, and the mutation survived a row asserting exactly those. What a pop-and-push breaks is the handle `ask` captured: the replacement does not own it, so answering the question **leaves it on the screen**. The row asserts the stack is empty after the answer and after the escape, and the keys the floating question does not own fall through rather than being consumed. It also reads **the borrow**: a line typed before the question arrives is gone while the reply is composed and back exactly afterwards, on the escaping path as well as the answering one — a restore written on the `⏎` arm alone loses the draft on precisely the path where the reader changed their mind about typing (C17 I28). Then the answer carries both facts — the reply choice's `key` and the composed `text` — and escaping instead resolves with the default's `key` and no `text`, which is the arm that says `text` is a record of what happened rather than a field that is always filled.
- **T1.70** (I75, I36, §051, `R-QST-002`): an inspection choice suspends and does not answer — the source is open **bounded**, the layer's id is unchanged, the owner is still awaiting, and `Esc` returns to the **same unresolved question** rather than resolving it. **Read off the `scroll` box and not off the payload's text**, which is the row's first draft corrected: the payload is drawn with the question whenever it fits, so *the patch is on screen* is true before anything has been inspected and measures nothing. What is the inspection's alone is that the source is bounded, which is the reason it exists — and the row reads the **height**, not the box's presence: a box as tall as the patch draws past the bottom of the screen and loses its own way back, which is `render`'s cut-arm failure arriving one level down. Then answering it resolves, and the settlement count over the whole sequence — suspend, escape, answer — is **one**. The count is the row's subject: *resolves* is satisfied by a build that settles twice, and a reader watching the frames cannot tell the two apart.
- **T1.70b** (I75, `R-QST-004`): the inspection is reachable as a choice and by nothing else — no key that is not one of the question's own choices opens it, which is the arm that refuses a second key-only route. The control is the inspection choice's own accelerator, which does open it.
- **T1.71** (I76, §036, `R-BLK-249`, `R-BLK-263`): the operation head across its four running shapes and its four stopped ones, as a table — elapsed and delta together, elapsed alone, delta alone, neither; then settled, cancelled, failed and settled-with-nothing. Running, the numbers are **inside one pair of parentheses** and joined by C09 I49's separator; with one member there is no separator, and with none there are **no parentheses at all**. Stopped, the same numbers are **flat peers** — `● compacted · 6m 12s · 41k → 12k tokens · 18 turns summarised` — which is the row the walk found and I would not have written: *the aside groups the elapsed and the delta* and *a head joins its fields with the separator* are both true, and a row asserting only the running line passes against a build that brackets the settled one too. The control is the call head through `toolCallHeader`, which brackets nothing at either end.
- **T1.72** (I76, §036, `R-BLK-263`, `R-BLK-265`): the bar is present while the operation runs and **absent in all three stopped states**, asserted as three separate cases rather than as *settled*. Two arguments end with no bar and neither substitutes for the other — *a 100% bar on a finished thing is a row spent on nothing* is settlement's, and *a stopped bar is a claim about progress that is not being made any more* is cancellation's — so a row written about settlement alone is satisfied by a cancelled operation still drawing its frozen fill, which is the state that lies. The control is the running document, which has the bar; and the head survives all three, because the assertion is *the bar went* and not *the operation went*.
- **T1.73** (I78): a part whose second fetch never settles goes stale at `staleAfter` — it is not skipped for being in flight.
- **T1.74** (I78): a stale part's `staleForMs` advances with the clock — the panel reads `1m` and later `3m` — and no write lands between two instants that draw the same figure.
- **T1.75** (I80, `R-HON-008`, §075): `PROVENANCE_TONE` is §075's ladder — `muted`, `meta`, `warn`, `error`; `configBlock` over a `default` and a `config` setting draws a `key value source` header and one row each, their source cells `muted` and `meta`, read off the block and off the ASCII frame's text; and over an `env` or a `flag` setting it throws C04 I6's `BlockShapeError` — the state parked as 44, recorded so the day 44 is answered this clause goes red rather than quietly passing. **The control is two `default` settings**, both `muted`, so the row is about the ladder and not a column that is always one tone.
- **T4.70** (I74, → C22 I80, §7f): a replacing question in a session takes the prompt's rows and nothing else — the editor's line is gone, the question sits directly above the footer's rule, the region shrinks by exactly the rows the question added, and C22's height identity holds at both frames. Read off the screen and asserted as the arithmetic, because *the question is drawn* is satisfied by a build that drew it over the transcript and by one that drew it twice.
- **T4.71** (I77, C16 I54, C17 I29, §052): through a built session with two commands in history and `mine` at the prompt — `reply…` chosen, letters typed: `↑` never shows either command, `↓` past the end leaves focus where it was, `⌃R` opens no search, `⌃z` never shows `mine`; answered, the prompt reads `mine` and its own `⌃z` never shows the reply's text. The control is the same `↑` at the prompt after the answer, which does show the newest command.
- **T4.72** (I31, C04 I129, C04 §3ap E1): an entry holding a `tree` inside a `panel` — `⏎`-equivalent `expand` aimed at a node two levels down inverts that node's flag through a shell-origin patch and the entry's rows grow by its children; aimed again, they shrink back. The control is a target no node or row carries, which notifies *nothing to expand* and patches nothing.
