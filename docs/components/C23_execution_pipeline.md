...
**And the route is taken after the gate, never before it (I38).** This read `tool.interactive` above the `validation.ok` check — which lives inside the non-interactive arm — so a malformed invocation of an interactive verb was handed the terminal and spawned without being validated at all. D17's argument is that a malformed invocation costs nothing rather than an interpreter's startup, and the verbs stepping over it were the ones whose failure takes the screen. F119.
