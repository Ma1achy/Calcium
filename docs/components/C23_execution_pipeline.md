...
- **T6.102** (I72, F1206): the deadline dated from the settle — `dueAt = at + interval`, which is the tree before I72 — → **T2.48**'s fast-source arm fails, the tenth deadline a full ten lateness-sums past the first; the clamp removed so a slow far side chains too → **T2.48**'s 40 ms arm fails with three polls due at once the moment it settles.
