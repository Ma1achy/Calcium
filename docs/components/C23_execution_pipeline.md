...
- **T1.75** (I80, `R-HON-008`, §075): `configBlock` over four settings, one per source, draws a `key value source` header and one row each; the source cells' tones are `muted`, `meta`, `warn`, `error` in that order, read off the block and off the ASCII frame's text. **The control is four `default` settings**, every source cell `muted` — so the row is about the ladder and not a column that is always one tone.
