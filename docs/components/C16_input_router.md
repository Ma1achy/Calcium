...
| **18** | `⌃c` `⌃c` at an empty prompt raises the exit confirm → `⏎` held down | the confirm is **raised**, then the `⏎` is **refused**. On a terminal reporting key releases it stays refused until the key lifts; on one that does not, the guard ends on that first refusal and the next `⏎` answers, because nothing can tell the deliberate second press from the held first one. Two machines, and they compose rather than collide: the exit arm decided the confirm should exist, the guard decides it may not be answered by a key already in flight. The second `⌃c` is not itself refused — it was dispatched before the raise it caused |
...
| **guarded, no releases** | → guarded, afresh | guarded | **refused**, → open (T1.99) | → open (T1.99b) | — |
...
**The two rows are one rule and the terminal decides which** (R-BLK-788). *Wait for the held key to lift* is the answer whenever the terminal will say when it lifted; where it will not, the guard cannot outlive a single keystroke without swallowing the reader's deliberate second press, so it ends on the refusal. C02's `keyboardProtocol` is the pull that distinguishes them — the same field §6a reads, and the same one that decides whether `e.event === "release"` ever occurs.

**There is no window and no clock.** An earlier draft of this section gave the guard 500 ms, which is not in the design and is worse than nothing: it refuses an answer the reader gave deliberately half a second after reading the question, and it stops refusing a key that is still physically held at 501 ms. The boundary the design names is an **event** — neutral, or key-up — and an event is exactly what a held key does not produce.
...
- **T1.98** (I43, I44, R-BLK-786): a dispatch that raises the **question** rung leaves the router guarded and the epoch one higher — both read from the router, not from a handler. **T1.98b**: unguarded, an activation is handled and the epoch does not move; the control is what makes the guard an assertion rather than a restatement of *nothing happened*.
- **T1.99** (I44, R-BLK-788): guarded with no release reporting, the next activation answers `reject` and the guard ends, so the one after it is handled. **T1.99b**: guarded, a **neutral** key is handled and ends the guard — an arrow is readable under a question that has just arrived. **T1.99c** (I43, I44, §4a W8): a rung change that is **not** a question arriving moves the epoch and guards nothing; the next activation is handled. The row trace 19 constructs, and the one a single field cannot pass. **T1.99d** (I44): guarded **with** release reporting, three activations in a row are all refused — the guard does not end on the refusal, because the terminal can say when the key lifts. **T1.99e** (I44): and a key `release` ends it, so the next activation is handled. The pair is the protocol split, and either row alone is passed by a machine that ignores the protocol.
...
- **T1.110** (I58): `chordText(key, false)` over every binding in `defaultKeymap` is printable ASCII and carries none of the eleven glyphs; and by equality on the design's own chords — `⇧⏎` → `S-Enter`, `⌃C` → `C-c`, `⌥⇧C` → `M-C`, `⌘1` → `s-1`, `⌥⌫` → `M-Backspace`, `⇧⇥` → `S-Tab`, `esc` → `Esc`, `⌃]` → `C-]`.
- **T1.111** (I58, C22 §6l): the owner line on every rung, at both capability rungs, spells each chord it prints as `chordText` does — the scope rung at ASCII is exactly `Enter send`, `S-Enter newline`, `Tab complete`, `S-Tab transcript`, and the child rung at Unicode draws `⌃C`, the registry's, not `⌃c`. A pair of keys joins with nothing at Unicode (`↑↓`) and `/` at ASCII (`Up/Down`), one spelling for every pair on the line.
...
- **T3.20** (I44, §4a W9): **the guard reads no clock.** With the injected clock advanced by an hour between the raise and the activation, the activation is still refused — and with a neutral key in between and no time passing at all, it is not. Time does not appear on either axis, which is the assertion a 500 ms window fails and a machine keyed on events passes.
