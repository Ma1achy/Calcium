...
- **I60** — *(C04 §3ar F1–F5, F9, §105, §019, `R-QST-003`, → C17 I29)* **While a form field is being edited, the inside rung answers `⏎` and `esc` and hands the keys a field owns to the `prompt` rung, and to nothing else.** `⏎` commits and `esc` discards (C22 I118). The rest resolve against the `prompt` rung's bindings under the field's action set, which is a typed reply's (I54) without `insertNewline`, `historyPrev` and `historyNext`: a field is one line and owns no history. A key outside the set is rejected by I8 rather than falling to a lower rung, so `↓` never leaves a field in silence, and `⌥←`/`⌥→` are word motion in the field and never the divider of a split the form sits in. The field's branch runs before the rung's own rows, so `←`/`→` move the caret and not a plot's cursor. A paste holding a newline is refused with its reason (§089's *a refused paste*); a one-line paste is one edit.
