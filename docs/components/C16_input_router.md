...
- **`page-scroll` and `wheel` handle at every rung except `copy`, the child included, and
  a `handle` routes to the viewport before the ladder.** §103: *COPY MODE rejects while
  frozen; otherwise the active viewport handles*. *Otherwise* is every other rung, and
  *the active viewport handles* is a destination — so the verdict short-circuits to the
  viewport scroller exactly as `reject` short-circuits to the owner. A question stays open
  and unanswered while the transcript moves beneath it, which is the point.
...
- **I40** — **`page-scroll` and the wheel handle at every rung but `copy`, and a `handle` routes to the viewport before the ladder** (§3, §103). §103's *otherwise the active viewport handles* names a destination, so the verdict short-circuits to the viewport scroller as `reject` short-circuits to the owner; the child is included, because a captured child owns its keys and not the transcript scrolled behind it. A question therefore **stays open and unanswered while the transcript moves**, which is the case the rule is for: a reader cannot answer what they cannot read. `interrupt` is excluded and keeps falling to the ladder — its `handle` is *this rung's own cancel*, a different verb at each rung, and the ladder is where that resolves.
...
- **T1.40** (I40): `⌥↑` with a question open **scrolls the transcript**, and the question is **still open and still unanswered** — the three assertions together, because each alone is passed by a router that does the wrong thing. Stages are `["arming", "intercept:page-scroll:question:handle", "intercept:scroll"]`: the ladder is never reached. The control is `⌃c` at the same rung, which is `reject` and *does* reach the question. The paired mouse row is a wheel under a **non-dismissable** layer moving the viewport while a click beside it is still consumed.
