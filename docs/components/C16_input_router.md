...
- **I25** — **A layer that must be answered gets its keys before the ladder does.** When the top layer carries an answer callback, rung 4 offers it every key — accelerators, `Enter`, `Esc` and `⌃c` — and consumes what it takes, *before* the rung's own `⌃c` clause runs. The order is the invariant, not an implementation detail: the existing clause answers `⌃c` at a non-dismissable top with *consumed, and nothing happens* (I8), which for a question means the key vanishes and the handler awaiting it waits forever. **The rung was written when no layer could be answered, so "nothing happens" was the whole truth; a question makes it a hang.** `Esc` and `⌃c` are routed rather than special-cased here, because what they mean is the question's business — C23 I36 resolves both with the default choice — and a router that knew that would hold half of a rule whose other half lives two layers away. The callback is read from the top layer only, never searched down the stack, for C15 `pop()`'s reason: a question raised over a completion menu must not be answered by the menu.
...
24. A top layer that must be answered receives every key at rung 4 before the rung's own `⌃c` clause, so `Esc` and `⌃c` reach the question rather than being consumed into silence. The callback comes from the top layer only (I25).
