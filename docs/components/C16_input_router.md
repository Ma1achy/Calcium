...
- **I54** — *(§052, §101, §103, `R-QST-003`, C23 I73, → C17 I29, C23 I77)* **A question composing a typed reply hands the keys it does not answer to the `prompt` rung, and to nothing else.** The question rung runs first, so `⏎` answers and `esc` cancels the question (§052's *the target* and *the escape*); a key it passes while it reports composing runs at `prompt` next; a key `prompt` also passes meets I8's reject exactly as before. **The route was named and never built.** `confirm.ts`'s `compose` arm returned `false` under a comment saying *C16 hands a `false` back down the ladder to the prompt beneath*, and dispatch has never fallen between rungs (I27's finding, R-INT-005) — so in a built session every letter typed into a reply met the modal reject, `lastStages` reading `target:overlay, modal-blocked, reject` for each, and the answer carried `""` whatever was typed. C23 T1.69 was green throughout because its world hands the key to the editor itself, which is *a test that calls the mechanism misses the wiring*. **A predicate, not an id.** The question reports *composing* from its own state (C23 I73's `reply…` move); the router asks it, as it asks `overlayAnswerCallback`, and never compares a layer id.
