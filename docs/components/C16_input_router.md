...
- **T1.3q** (I33): `tab` and `⇧tab` resolve to `viewNextSection`/`viewPrevSection` at `pushedView` and to `entryNext`/`entryPrev` at `liveBlock` from the one keymap, with a view up and with none — the ladder separating them and not a second table. And the three owners each answer the call: a fake at its last section returns `false`, a fake with one section returns `false`, and the row asserts the header text differs between the two, because the return value cannot tell them apart and that is the point of I33.
