...
- **`page-scroll` and `wheel` handle at every rung except `copy`, and a `handle` reaches
  the viewport past every rung that is not one.** §103: *COPY MODE rejects while frozen;
  otherwise the active viewport handles*. *Otherwise* is every other rung. **The first
  draft read *the active viewport* as the transcript and routed straight to `global`, and
  three rows measured it wrong**: a focused `scroll` box is a viewport too, and paging
  inside a panel stopped working. So the ladder still resolves *which* viewport —
  `liveBlock` and `interaction` page the box, `prompt` pages the transcript through
  `global` — and what the route takes away is the three rungs that are not viewports at
  all: a question, a substate layer, a captured child. A question therefore stays open and
...
- **I40** — **`page-scroll` and the wheel handle at every rung but `copy`, and a `handle` reaches the viewport past every rung that is not one** (§3, §103). §103's *otherwise the active viewport handles* names a destination, and **the active viewport is not always the transcript**: a focused `scroll` box is one, so the ladder is what resolves *which*. `liveBlock` and `interaction` page the box; `prompt` pages the transcript through `global`. What the reserved route takes away is the three rungs that are **not** viewports — a question, a substate layer, a captured child — because those are the handlers that would consume the key for something other than scrolling. *Unclaimable* means no rung may take it for something else, not that no rung may perform it. The child is among the excluded: it owns its keys, and it does not own the transcript scrolled behind it. A question therefore **stays open and unanswered while the transcript moves**, which is the case the rule is for: a reader cannot answer what they cannot read. `interrupt` is excluded and keeps falling to the ladder — its `handle` is *this rung's own cancel*, a different verb at each rung, and the ladder is where that resolves.
...
- **T1.30** (I8, I40) — **amended**: with a full-region layer on top, `PgUp` **reaches the viewport scroller**, because `page-scroll` is a reserved route and the intercept table is read before the ladder. The row was written when I8 held every key, and it pinned the defect rather than the rule: a reader under a full-region layer could not page the transcript. What it still asserts is that the layer is **not dismissed** and no *other* global binding runs — the theme switch is the control, and it is dropped. Originally: `PgUp` is dropped rather than reaching `global`. The control is the same key under a one-row *dismissable* overlay, which does reach `global` — without it the assertion passes for a router that skips step 3 whenever any layer is open, which is the rule §4 spends a paragraph rejecting.
...
- **T1.40** (I40): `⌥↑` with a question open **scrolls the transcript**, and the question is **still open and still unanswered** — the three assertions together, because each alone is passed by a router that does the wrong thing. Stages are `["arming", "intercept:page-scroll:question:handle", "intercept:scroll:prompt"]`: the question's rung is never reached, and the stage names the viewport the ladder resolved. The control is `⌃c` at the same rung, which is `reject` and *does* reach the question. The paired mouse row is a wheel under a **non-dismissable** layer moving the viewport while a click beside it is still consumed.
