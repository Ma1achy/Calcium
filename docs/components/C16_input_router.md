...
- **I61** — *(C22 §6n, ruling 27, → C22 I127)* **`ESC [ I` and `ESC [ O` decode to `{ kind: "focus", focused }` and are never routed.** A focus report is not a key, not an activation and not a pointer event: it takes no stage, reaches no rung, and leaves C16 I44's guard and I45's pointer arm exactly as they were. Before this they were swallowed whole, so no report ever leaked as a key — and none was ever heard.
