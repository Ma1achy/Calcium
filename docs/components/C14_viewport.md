...
- **T1.16** (I15): exactly one visible entry reports `live: true`, and it is C13's `liveId`; a transcript with no live entry reports none.
- **T1.17** (I15): measured heights are identical with and without the marker — it costs no rows.
