...
- **T4.17u** (I105, F1197): a spinner alone and an orbit alone, each under sixty 16 ms wakes of the fake clock with `synchronisedUpdate` pinned — **at least 11 frames** are written for the spinner and **at least 55** for the orbit, against 5 and 15 over the same 960 ms on the arming F1197 replaced. Frames are counted as writes to the fake terminal outside the synchronised-update brackets, not as renders of a bystander kind: I103 keeps a non-animating block out of a tick's render, so a render count on it reads one. The bound is the rate the invariant states less one frame for the window's phase.
