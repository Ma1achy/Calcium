...
| 9 | `/clear` | | Cleared |
| 10 | **a `steps` block animating through `ctx.tick`** | the key has no `tick` axis | **The cache defect is dormant; the animation defect shipped.** `visibleRows` passes no `tick`, so every transcript render is at 0 — which means no assertion *here* can fail, and this row said *not reachable* on that basis for as long as it has existed. **It is reachable, and reached**: `steps` ships, `b.spinner()` is public API, and a real session drew **one distinct glyph across ten frames** where the same block through the harness drew ten (F227). One phrase named two subjects and nothing forced a choice. The ruling stands unchanged — either the key gains the axis or live entries stop being cached — and it is now owed rather than anticipated |
