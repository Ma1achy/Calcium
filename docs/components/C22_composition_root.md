...
- **T4.89d** (I101): a top-level block that divides — a `logs` taller than the window — is rendered at every range and never held, and the rows equal the fresh render's; a whole block beside it is held. In `test/integration/render-cache.test.ts`.
