...
- **T4.12** (I34, with C14): the height C14 is given equals the composed frame's `region.height`, at three prompt heights — one row, three rows, and a prompt long enough to hit S01 §3's half-terminal cap. A spy on `resize` reads the value; asserting only against a one-row prompt is the setup where the terminal's height and the region's differ by a constant, and a test that hard-codes `rows − 3` agrees with the defect at every other prompt height.
- **T4.13** (I35): a transcript selection longer than the region → the frame is refused and the fallback is drawn, rather than the first `region.height` rows being kept. Constructed by handing the viewport a height the frame did not compose, which is the only way to reach it once I34 holds — and is exactly the state the shipped code was in.
...
- **T6.31** (I35): restoring the truncation in the transcript region → T4.13 fails, and T6.30's defect goes back to being invisible. It is the pair that matters: the refusal is what makes a mis-sized viewport observable, and neither the refusal nor the drift test existed while the defect did.
