...
- **T6.77** (I69): removing the *already held* guard from the request, so the shell reserves on every frame → T4.50 fails. It shows as a permanent re-render at frame rate rather than as a hang, which is the worse of the two because nothing looks broken.
