...
- **T6.32** (I38): caching `spinning` at request time instead of reading it per paint → T1.18 fails and the spinner never appears at all.
- **T6.33** (I38): dropping the wake armed at the threshold → T1.18 fails on the frame that arrives with no further input, and the spinner appears only once the user types again. The same symptom as an unarmed decoder deadline (I32), through a different timer — which is why it is a separate row rather than a second clause.
