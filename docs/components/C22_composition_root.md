...
- **T4.35** (I60, I60a, C09 I32): a real session with an animating block draws **every frame of its spinner set** over thirty samples on fake timers, through the production path — ticker, `commit("spinner")`, C03's window, `#render`, `visibleRows`, the cache. Thirty for a ten-frame set because the set's 80 ms aliases against C03's 100 ms window and a ten-sample run sees eight; the row is about the chain being wired, not about the arithmetic of two cadences. **Measured at one distinct glyph before the fix** (F227).
...
- **T4.17u** (I105, F1197): a spinner alone and an orbit alone, each under thirty 33 ms wakes of the fake clock with `synchronisedUpdate` pinned — the spinner's entry renders **at least 9 times** and the orbit's **at least 27**, against 5 and 15 on the shipped arming. The bound is the rate the invariant states less one frame for the window's phase, and the same harness T4.17j reads.
...
- **T6.120** (I105, F1197): arming the wake for the interval from the paint again — `schedule(…, ms)` in place of the stamps' due time — → **T4.17u** fails on both arms (5 and 15), and T4.17j still passes, because its bound is a ratio the defect satisfies. That is why T4.17u asserts the rates and not their ratio. Anchor in `tools/mutate/runs/c22-ticker-period.mjs`.
