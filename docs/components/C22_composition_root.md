...
- **T4.17v** (I74, C09 I112): an `agent`-only screen, whose wake is armed at 120 ms — after 1200 ms of clock the counter is 15, the elapsed time over 80 ms, and not 10, the wakes. The control is a braille spinner beside it, whose wake is 80 ms and whose counter after the same 1200 ms is the same 15.
...
- **T4.100** (I118, C04 §3ar F6, F7): a field being edited and a pointer press elsewhere — the typed value is written and the line given back; the same field and a pointer activation of `save` — the submitted command carries the typed value, not the one before the edit.
- **T4.101** (I118, C04 §3ar F9, F3): a multi-line paste into a field is refused with its reason and the field is unchanged; `↓` inside a field is rejected and focus stays on the field.
