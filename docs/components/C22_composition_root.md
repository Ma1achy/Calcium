...
- **T4.101** (I118, C04 §3ar F9, F3, C16 I60): a multi-line paste into a field is refused with its reason and the field is unchanged; `↓` inside a field is rejected and focus stays on the field; and `↑` in a session with history leaves the field's text alone — a key passed rather than rejected reaches the prompt's `historyPrev`, and `↓` alone cannot show the difference, because `historyNext` with nothing ahead does nothing.
