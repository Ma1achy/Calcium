...
- **T1.63** (I108, F1206): `pacedSchedule` under a fake `elapsed` and a recording timer — a first arm at 16 is armed at `1000 / 60`; its firing reported 3 ms late and the next arm at 16 is armed at `1000 / 60 − 3`; ten consecutive firings each 1 to 3 ms late leave the tenth deadline exactly ten sixtieths after the first; an arm 20 ms after the last deadline is armed at `1000 / 60` from now; a slot disposed and then re-armed is armed from now; an 80 ms window is armed at 80 and chains from its own deadline; and a window of 100 is not floored; and a window of 0 is armed at 0, chains from nothing and is not remembered.
...
- **T4.92** (I108, F1206): the wiring — a session built over a spy on the ambient timer and an `elapsed` the row advances, which is the untapped clock when nothing records: the first frame's write arms a slot of `1000 / 60`; the slot fires with nothing in it and lapses; the row sets `elapsed` 2 ms past that deadline and resizes, and the coalesced window C03 arms is `1000 / 60 − 2`, which is the composed schedule and not C03's own arithmetic; a window of 0 is armed at 0.
...
- **T6.123** (I108, F1206): the chain removed — every window dated from now, which is the tree before I108 — → **T1.63**'s late-firing rows and **T4.92** read the full window; the floor removed → **T1.63**'s first arm reads 16; a disposed slot left seeding the next → **T1.63**'s dispose row chains from a deadline nothing waited for.
