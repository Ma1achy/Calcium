...
- **I108** — **The schedule C22 hands C03 dates consecutive windows end to end on the clock, at no less than one sixtieth of a second.** `pacedSchedule(sampleClock, schedule)` wraps the ambient timer — reading the **untapped** monotonic clock, the `sampleClock` C28 I53 gave the sampler, because its reads date timers and enter no frame, and a replay's timers are not the recording's, so a read per arm on the positional channel is one the replay consumes at a position it never reaches (C28 I14, T5.1c) — and holds the deadline of the last slot it armed: an arm made **inside that slot's own firing** — the write that opens the next window (C03 I17) — is dated from the deadline, so the delay is `deadline + window − now` and the lateness of the firing is taken off the next; any other arm is dated from now. So a lone commit's latency is unchanged, and a slot that fired with nothing in it and lapsed (C03 I17) seeds nothing either: the commit that follows it is a fresh window, not a frame a window after a deadline nothing drew at. A disposed slot seeds nothing. A positive window shorter than `1000 / 60` ms is armed at `1000 / 60` — a zero window is C03's next turn (C03 I10), neither floored nor chained nor remembered — A02 §7's ceiling as a rate, because C03's windows are integer ceilings on each gap (C03 I15) and a 16 ms window dated end to end would draw 62.5 frames a second. C03 reads no clock (C03 I11) and is not changed; its I17 period — the window and not the window plus the frame — was true of the arms and false of the wall clock, because the timer fires one to three milliseconds late and the next window was dated from the firing, so a 16 ms window drew 54 to 58 frames a second (F1206). Through this seam the period is the window on the clock too.
...
79. **Windows lie end to end on the clock** (I108, F1206). C03 arms each window from the timer's firing and Node fires one to three milliseconds late, so a 16 ms window drew 54 to 58 frames a second and nothing in C03 could recover it without a clock. The composed schedule dates the next window from the last deadline and holds the rate at sixty.
...
- **T1.63** (I108, F1206): `pacedSchedule` under a fake `elapsed` and a recording timer — a first arm at 16 is armed at `1000 / 60`; its firing reported 3 ms late and an arm at 16 made inside that firing is armed at `1000 / 60 − 3`; ten consecutive firings each 1 to 3 ms late, each arming the next inside itself, leave the tenth deadline exactly ten sixtieths after the first; an arm made 2 ms after a firing that armed nothing — a lapsed slot — is armed at `1000 / 60` from now; a slot disposed and then re-armed is armed from now; an 80 ms window is armed at 80 and chains from its own deadline; and a window of 100 is not floored; and a window of 0 is armed at 0, chains from nothing and is not remembered.
...
- **T4.92** (I108, F1206): the wiring — a session built over a spy on the ambient timer and an `elapsed` the row advances, which is the untapped clock when nothing records: the first frame's write arms a slot of `1000 / 60`; a resize inside the slot is pending in it; the row sets `elapsed` 2 ms past the deadline and fires the slot, C03 writes and opens the next window inside the firing, and that window is `1000 / 60 − 2`, which is the composed schedule and not C03's own arithmetic; a window of 0 is armed at 0.
...
- **T6.123** (I108, F1206): the chain removed — every window dated from now, which is the tree before I108 — → **T1.63**'s late-firing rows and **T4.92** read the full window; the floor removed → **T1.63**'s first arm reads 16; a disposed slot left seeding the next → **T1.63**'s dispose row chains from a deadline nothing waited for; the chain taken by time rather than by the firing — any arm within a window of the last deadline — → **T1.63**'s lapsed-slot row is dated a window after a deadline nothing drew at.
