...
46. **The manual camera scheme is the bracket family, the dolly is multiplicative and the tilt is unclamped** (I75). The arrows collide with row navigation and with the prompt; scaling the distance cannot reach the only degenerate value; the pole is unreachable in floating point, and the comment that says otherwise is corrected (→ C26 I2).
...
- **T4.38** (I72, AN1): a static 3D plot renders once and the **second render is served from the cache**. Its control is a nudge between the two, which must miss — and a third arm toggles the orbit flag with the camera held still, which must **hit**, because that is the assertion the flag's absence from the key is for.
- **T4.39** (I73): with an orbit live the ticker commits **`stream`**; with only a spinner on screen it commits **`spinner`**. Asserted on the reason the scheduler received, because the interval alone cannot distinguish them — 100 ms of interval and 100 ms of window produce the same frames for different reasons.
- **T4.40** (I73, AN5): `synchronisedUpdate: false` → the orbit's armed interval is 100 ms and its reason is `spinner`; `true` → 33 ms and `stream`. **Both arms**, because a cap that always applies and one that never does read the same from a passing suite.
- **T4.41** (I74): an orbit and a `steps` spinner on screen together → over a fixed elapsed time the azimuth is what it is with the plot alone, **and** the spinner's counter is what it is with the spinner alone. Two assertions in one row because the defect is symmetric: the shared timer corrupts each animation with the other's cadence.
- **T4.42** (I72, F468): one orbit step changes a **measured fraction** of the frame's cells and not all of them — compared row against row and index against index, never by string position. The row records the fraction; §11's *every cell changes* is what it replaces.
...
- **T6.83** (I73): the orbit commits `spinner` rather than `stream` → **T4.39** and **T4.40** fail, and the rotation runs at 10fps on a terminal that can hold 30. This is the mutation that says the reason is the frame rate; changing the *interval* alone fails nothing, which is the whole finding.
- **T6.84** (I74): the azimuth advances by a constant per wake instead of by `ω · Δt` → **T4.41** fails on the spinner arm and **T1.29** fails outright. Its mirror — advancing `ctx.tick` once per wake — fails T4.41's other assertion, and the pair is what says the two halves are one defect.
- **T6.85** (I72): the orbit flag joins the render key → **T4.38**'s third arm fails, a toggle that changes no cell misses, and every other row still passes.
