...
- **T4.89a** (I101): a column group of six counting children windowed to three, then scrolled one row so a fourth enters, renders on the second frame the entering child alone — the `render` miss is `range` — and the rows are the fresh render's. In `test/integration/render-cache.test.ts`.
