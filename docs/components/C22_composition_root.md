...
- **T4.92** (I108, F1206): the wiring — a session built with the fake schedule and an `elapsed` the row advances: after a stream write the slot C03 arms is `1000 / 60`; the row reads `elapsed` 2 ms past that deadline when the next write arms, and the next slot is `1000 / 60 − 2`, which is the composed schedule and not C03's own arithmetic.
