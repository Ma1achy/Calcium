...
- **I10** — Word-level intra-line highlighting is deferred, and the block shape does not foreclose it. `Hunk` carries whole lines; **the field is `Hunk.lines[].spans`, a `readonly TextSpan[]` of the shape C04 §3am gives `Raw`, `Notice`, `Rule` and `Cell`** — the type exists and the member is withheld from `Hunk` in the first pass (C04 I88), so what is missing is the writer, an intra-line diff, and not the field. A deferral with a symbol: grep `Hunk` in `src/data/viewmodel/types.ts` for `spans`, and this sentence expires the day it is there. **Its carrier is decided and is not a background**: C10 §4a measured a stronger background pair and withdrew it, so a changed span takes `underline` over the line's background (→ C10 §4a). **And `underline` collides with nothing**: a markdown span may also say `underline` (C04 I85), and C10 §4e records why the two never meet on one text — no member has both writers, and a diff line's spans will be written by the diff and not by the translator. The day a diff line can carry markdown, C10 §4e is the row to reopen (→ C10 §4e).
...
| **Word-level highlighting within a changed line** | Deferred. It is where diff viewers earn their keep and also where they get slow — an intra-line diff is a second diff algorithm running per changed line. The block shape does not foreclose it: the field is `Hunk.lines[].spans`, C04 §3am's `TextSpan[]`, additive and withheld until its writer exists (I10) |
