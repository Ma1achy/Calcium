...
- **T2.9** (I14, I15, I16): **a watch, not a coverage row.** §3a records these three as *specified and unbuilt* — `maxExpandHeight` appears nowhere in the tree — so a row asserting the behaviour would be asserting nothing. This one asserts the **absence**, over `src/`, and fails the day the field lands, naming the three invariants that then owe real rows. Beside it, the half that is C25's and is true today: the collapsed and expanded heights are both computable from the block before either is offered, `measure` being pure — and `Measure`'s parameter list carries no height, which §3a names as the one change that must not be made.
- **T2.10** (I17): `PatchViewDeps` is `overlays`, `transcript`, `region`, `redraw` and an optional `probe` — no fetch, no adapter, no transport — so the fullscreen view can read nothing but the block it was opened on, and every hunk of that block is reachable by its motions. *Never needs data the block does not carry* is a claim about the **dependency list**, which no assertion about output can see. The probe is the one member that does not weaken it: an instrument carries nothing in (C28 I30) and the view reads nothing from it — it is there for I22's plan to report its misses through (C22 I41, F1187). Listed by equality, so a sixth is a failure whatever it is for.
...
- **T2.12** (I20a): `clampOffset` stops at the **first offset whose window reaches the last row**, asserted as the property rather than the number — the ceiling's window carries `collapsedAfter` and one row above it does not. **The row existed and the citation did not** (C24 T2.10's situation, third instance in this pass), under the title *"clampOffset stops at the first offset that reaches the end, not at total − height"*.
- **T2.13** (I20b): every offset `clampOffset` returns is one a window may begin at — clamping is **idempotent** — and `windowPatch` on the raw offset gives the window `clampOffset`'s value gives, so a caller holding the clamped value and the window it received agree.
  **A limit, measured and recorded rather than asserted away**: two adjacent valid offsets can produce the *same* window. Offsets 0 and 1 of the three-hunk fixture at width 80 both draw the first three context lines, because the row between them is the sticky path header (I18), which a window re-adds whether or not it was skipped. That is I20b's second clause arriving through the sticky rule rather than through where the snapping happens — and it is **unreachable today**, because `PatchViewMotion` has no line-wise scroll. A line-wise motion added later needs this measured again; the row's comment is where the next reader finds that.
- **T2.14** (I20b): a motion and its inverse return to where they started — `pageDown`/`pageUp` from the top, `nextHunk`/`prevHunk` from a hunk — and `top` and `bottom` are idempotent. **The round trip is the only observable form from outside**: the offset is private, deliberately, since a second source for *where am I* is how two answers come to disagree. If the builder snapped internally the view would store 7 while drawing 4, the inverse would compute from 7, and the reader's place would be gone — and neither direction is visible to a test that drives one motion. The hunk pair starts at a hunk rather than at the top because it is a **seek**: `top → nextHunk → prevHunk` correctly lands on the header, not where it began.
- **T2.15** (C15 I25, §8a A7): the ⌃c ladder's `overlays.pop()` with the view open → the owner's `pop()` returns `false` afterwards rather than dismissing an id that has gone, a motion is a no-op, and a fresh `open` on the same block is accepted with one layer on the stack. **Driven through the manager and not through the owner**, because the owner's own `pop()` always tore down; what F944 measured was the other caller.
