...
- **T2.12** (I17, R-EXA-082): `⏎` on a collapsed patch rewrites the block in place with every hunk uncollapsed and every `collapsedBefore`/`collapsedAfter` gone — the entry is the same entry, the overlay stack is empty across the toggle, and the blocks that followed the patch still follow it. Asserted on the entry rather than on a returned block, because *in place* is a claim about what else moved.
- **T2.13** (I17, R-EXA-082): the toggle is a toggle — a second `⏎` restores the collapsed form, and the round trip is the identity on the document. **The round trip rather than the two states**, for the reason the struck T2.14 gave about the pushed view's offsets: a rewrite that half-applies looks correct in either direction alone.
...
- ~~**T6.23** (the retired offset rule): the ceiling back to `total − height`~~ — struck with the offsets (§3b). Replaced by **T6.23** (I17, R-EXA-082): `⏎` on a patch pushing a layer again rather than rewriting the block → T2.12 fails on the stack being non-empty, and the transcript is a hole where the diff was.
- ~~**T6.24** (the retired offset rule): the snap moved from the clamp back into the builder~~ — struck with the offsets (§3b). Replaced by **T6.24** (I17): the toggle made one-way, so `⏎` expands and never collapses → T2.13 fails. **The revert that reads as a simplification**, because a diff you opened is usually a diff you meant to open.
