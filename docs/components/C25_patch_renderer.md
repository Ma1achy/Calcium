...
**And the third channel on the line is an attribute, not a palette.** Word-level emphasis (I10) is `underline` on `Hunk.lines[].spans`, and a span here may carry attributes only — `tone` and `value` are refused at the gate (C04 I91) — because the line's two palettes are the gutter's and the syntax's, and a third would be C04 I88's collision. Colour is spoken for twice; the underline is the one channel that is not, and it composes with both by spread (C10 §4e).
...
A patch has **two** presentations, and both are rendering.
...
**There was a third and the design deleted it** (§82, R-EXA-082, R-INT-001). `fullscreen — all hunks in a pushed view, when inline is too much` was this section's answer to a tall diff, and *a pushed view takes the screen, you do something, you come back — and the transcript has a hole where that work was*. A run's detail **expands in place**: `⏎` on the row and the detail unfolds under it, the rows below carrying on underneath.

**The argument was already written here, three paragraphs down, and the third rung contradicted it.** *"Enter it and scroll" becomes "expand it and scroll normally" — the same outcome, one scroll position, and a finished diff can be paged past.* That is the whole of the design's ruling, stated by this section against nested scrolling and then not applied to the rung above it: a pushed view is *enter it and scroll* with the transcript taken away as well. So this is not a rule the design overturned. It is a rule this section held and did not carry to the end.
...
collapsed form      admit hunks while the running total ≤ 1 × viewport,
                    ...
                    "and 43 more hunks · ⏎ expand"

inline expansion    always offered. There is no second route, so there is
                    no threshold that chooses between two
```

**`maxExpandHeight` goes with the rung it routed to.** It was never a reading preference on its own — it was *the height above which the reader is sent somewhere else*, and with nowhere else to be sent the number answers a question nobody asks. A tall expansion is paged past, which is the cost §3a already priced and accepted for the expanded form.
...
**The 2× argument is kept as the record of why the number was 2**, and it is no longer a number this component has: one page to read, one to leave; one screenful refuses a legitimate thirty-line diff; three means paging three times past something already finished with. What the reasoning was actually about is the cost of paging past an expanded diff, which is a cost the reader now pays or does not pay by choosing to expand.
...
**Nothing here is configurable any more, and the reason is the rung that went.** `maxExpandHeight` was a multiple of viewport height, default 2, on the argument that someone reading diffs all day on a tall monitor wants 4 and someone on a laptop wants 1.5. That argument was about **where the threshold between two routes sits**, and with one route there is no threshold to place.

**What the paragraph got right survives and is now unconditional**: whether expansion exists is not configurable, because an option that turns it off makes a dropped hunk unreachable, which is D38's failure exactly. *What happens above the threshold* was the second half and it has no subject — there is no above.

**And D38 is better served, not worse.** The old shape reached a tall diff's dropped hunks only through a route that emptied the transcript to show them; the new one reaches them where they are.

## 3b. Expansion is all the hunks, in place
...
**Decided: the expanded form is every hunk, uncollapsed, in the entry it already sits in.** `⏎` on the patch and the collapse markers open; the block grows where it is and the rows below carry on underneath. It needs no data the block does not already have, which is what made it specifiable before L2 existed.

**This section said *fullscreen* until the design language landed, and the correction is a deletion rather than a substitution.** The old ruling was *what `less` gives you on a `git diff`* — a C15 pushed view, the third after S12 and S13, with `esc` popping, arrows and `PgUp`/`PgDn` scrolling, `g`/`G` seeking, and `n`/`p` for next and previous hunk. Every one of those conventions was the *transcript's* already, and the view existed to hand them back after taking the transcript away. R-EXA-082: *a pushed view takes the screen, you do something, you come back — and the transcript has a hole where that work was.*

**`n` and `p` go with it, and that is a chord decision the registry settles by holding neither.** They were the one motion a diff has that a list does not, and they were reachable only at the `pushedView` target. The registry binds `move.up`/`move.down` as *move within the focused thing* and `confirm` as *confirm · send · activate · open*; it has no hunk-navigation action and no rule that asks for one. An expanded patch in the transcript is scrolled the way everything else in the transcript is scrolled, which is the single scroll position A01 D3 spent a decision on.

**What this removes from the tree**, so the deletion is legible: `src/shell/patch-view.ts` entire, and the four exports of `src/presentation/patch/window.ts` that only it consumed — `windowPlan`, `clampOffset`, `hunkHeaderRows` and `windowPatch`. `windowRows` and `planFrom` stay: they are `BlockDefinition.window`'s, which is the transcript route and is what draws an expanded patch across a viewport boundary (I19a).

**Whole-file-with-changes is deferred to the editor, and they are the same component.** Both need the whole file, a scroll model independent of the transcript, and highlighting over a window rather than a fragment. Said explicitly because the alternative is two of them: a read-only file viewer with diff decoration is the smaller first target, it has its consumer in this feature, and it is the same component with editing turned off. **Unchanged by the deletion above** — that paragraph was never about the pushed view, and it is the one place a patch legitimately wants a scroll model of its own.

---

## 3c. The fullscreen view, walked by hand — a record

**The subject of this walk no longer exists, and the section is kept as the record of it.** §3b's ruling was deleted by the design language (R-EXA-082); the eleven rows below were run against the pushed view, five of them produced findings, and four of those changed a ruling. Two of the five are about the window builder's *offsets* — A4's one-piece-of-state and A6's dismissal — and lose their subject with it. The rest are about what a **window of a patch** is, which `BlockDefinition.window` still does for the transcript route, and those survive as I18, I19, I19a and I21.

**Kept rather than deleted, for the reason the record is always kept**: a walk that found five defects before any code is the argument for walking, and an argument deleted with its subject is one the next person re-derives. The rows about offsets are marked where they fall.
...
| **A7** | a view is open · `Ctrl-C` | §5's `pushedView` rung pops it, and that is cancellation rather than dismissal. Two keys, one outcome, two reasons — and they must not share an implementation, or the ladder starts depending on the keymap. **The owner still has to learn of it**: the rung calls `overlays.pop()` and no owner, so the view's entry and offset outlived the layer until it subscribed to C15's change stream and tore down on the change carrying its id (C15 I25, F944, T2.15) |
...
**Nor does it diff.** A line's `spans` (I10) are a second run stream over the same text, and the renderer merges the two by cutting the token stream at each span boundary with C09's own `sliceRuns` — the arithmetic `code` already uses for its rows — and spreading the span's attributes onto the pieces it covers. One cut, in one place, for both streams.
...
- **I10** — **Word-level intra-line highlighting is `underline` on the changed tokens of a paired remove/add line, written once by the builder and never by the renderer.** The field is `Hunk.lines[].spans`, C04 §3am's `TextSpan[]`, admitted on 2026-09-04 with its writer (C04 I91); a `context` line carries none and an unpaired changed line carries none. **The writer is `intralineLines` (`src/data/viewmodel/intraline.ts`), called by `b.patch`**, and it runs at construction because I7 keeps `render` pure over the block — a renderer that diffed would tokenise-and-compare per frame, and C25 §9 names that as where diff viewers get slow. **The pairing is the renderer's** (`changedRuns`, shared by import rather than restated): within a maximal run of changed lines the *n*th remove is diffed against the *n*th add, exactly the row split layout draws them on, so the underline on the left half of a split row is the underline of the change on its right half. **The token rule**: a run of letters, digits, marks or `_` is one token, a run of whitespace is one token, and every other character is a token of its own — so `+` against `-` is one changed cell and `foo(a)` against `foo(b)` underlines `a` and `b` alone. Changed tokens are the complement of a longest common subsequence over the two token lists, adjacent changed tokens merge into one span, and the spans carry `underline: true` and nothing else (C04 I91's attributes-only rule). **Three rulings the walk produced.** *Whitespace is a token*: a line re-indented, or given a trailing space, underlines the changed whitespace, because `underline` is the one channel visible on a blank cell and an indentation change has nowhere else to be shown — a whitespace-only pair is therefore not "identical", it is a pair whose whole change is whitespace. *An unrelated pair gets no spans*: when the two lines share no non-whitespace token, every token would be underlined on both sides, which restates the line background and distinguishes nothing; the family is delta's `max-line-distance`, and the boundary is at zero rather than at its 0.6 because zero is the one point where the underline is provably vacuous and anything above it is taste. *The cap is 200 tokens a side, per pair*: the LCS is O(n·m) and a 200 × 200 table is 40 000 cells per pair; above it the pair emits no spans, the line background still says the line changed, and the cap is a named constant (`INTRALINE_TOKEN_CAP`) so the number is asserted rather than remembered. **A line the caller already gave spans is left alone**, on both sides of its pair — one writer does not overwrite another's work, and it is what makes `b.patch` idempotent over its own output. **The renderer paints a second run stream over the same text**, which is the collision C04 I88 refuses on `code` and is admitted here because the two streams carry different channels: syntax tokens carry colour, spans carry an attribute, and the merge is `sliceRuns` cutting the token stream at every span boundary and spreading the span's attributes (C10 §4e) — a token cut in two keeps its slot on both halves. The truncation marker is never underlined (C09 §5) and the gutter never is (§3). **Its carrier is decided and is not a background**: C10 §4a measured a stronger background pair and withdrew it, so a changed span takes `underline` over the line's background, and it survives 1-bit where a background would not (§6) (→ C10 §4a). **And `underline` collides with nothing**: a markdown span may also say `underline` (C04 I85), and C10 §4e records why the two never meet on one text — no member has both writers, and a diff line's spans are written by the diff and not by the translator. The day a diff line can carry markdown, C10 §4e is the row to reopen (→ C10 §4e, C04 I91).
...
- **I15** — **Inline expansion is always offered, because there is no second route to be sent to** (§3a, §3b, R-EXA-082). The invariant read *offered iff the expanded row count is within `maxExpandHeight`; above it, fullscreen is the only route* — a threshold between two presentations, of which the design deleted one. What was load-bearing in it is I16's clause and survives there: a patch's dropped content is always reachable. `maxExpandHeight` is not a field of this component, was never built, and is not owed.
- **I16** — **Nothing here is configurable, and a patch's dropped content is always reachable** (D38, §3a). The invariant read *`maxExpandHeight` is the only configurable part*; the field went with the rung it routed to, so the exception has no subject and the rule is unconditional. *What happens above the threshold* was the second half and there is no above.
- **I17** — **The expanded form is every hunk of *this block*, uncollapsed, in the entry the block already sits in** (§3b, R-EXA-082). It never needs data the block does not carry, which is why whole-file-with-changes is a different component — and it takes no region, pushes no layer and leaves the transcript scrollable, which is the three things the pushed view cost. `⏎` toggles it; the collapse markers open and the rows below carry on underneath.
- **I18** — A window of a patch is a `Patch` rebuilt from a slice of its lines, never a list of rows. **The path header and every touched hunk's header are sticky**, because no field suppresses either — so a window carries them and **declares them**, through `skipRows` and `dropRows`, rather than charging them to its caller. Forced by the block shape rather than chosen (§3c). **It read *and its content budget is smaller than the region by exactly that* until M9b** (§3b): a budget is a property of a caller that was handed a **region**, which the pushed view was and the transcript route is not — `BlockDefinition.window` is asked for a row **range** and re-adds the headers above it, saying how many. The sticky rule is unchanged; the arithmetic moved to the side that has the viewport.
- **I19** — **Retired with the caller that cut the line array** (F1251). (§3b.) It read *in split layout a window cuts only at run boundaries*, and the reason was real: cutting inside a run is not additive, a run of one removed and two added lines being two rows whole and three rows cut between them, so a one-row scroll shifts the content by two and a line is drawn twice. **That is a property of cutting the *line array*, which is what a caller holding an offset did.** The transcript route cuts **row-wise**, and I19a is the measurement that a row-wise cut *is* additive — so there is nothing to snap to and no caller to snap. The snap lived in `clampOffset`, which had one consumer.
- **I19a** — **A row-wise cut of a run is additive, and it is what `BlockDefinition.window` uses.** I19's non-additivity is a property of cutting the *line array*; taking the first `min(k, removes)` removes beside the first `min(k, adds)` adds gives exactly `k` rows and leaves exactly the remainder — measured across every run up to 4x4 removes/adds, at every cut point. So the transcript route slices exactly and C09 I26's equality holds with no slack beyond the headers, which `skipRows` carries. ~~**I19 still governs the pushed view's offsets**, whose reason is I20b~~ — struck with the view (§3b): the second route was a caller holding an offset, and there is one route now. Two routes, one row model (`rowsOf`), because a window that disagreed with `pairedRows` about what a row is would be the drift I1 exists to prevent.
- **I19b** — **The run grouping has one implementation and it is `changedRuns`** (`src/data/viewmodel/intraline.ts`), read by three callers: the renderer (`definition.ts`), the intra-line diff (`intralineLines`), and the window's `unitsOf` (`src/presentation/patch/window.ts`), which was a second walk of the same rule until 2026-09-04 (F595/P2). Two walks of one rule drift into a window whose line range disagrees with the rows the renderer draws, and that disagreement is visible only in a frame — which is why the de-duplication is an invariant and not a tidying. **`unitsOf` adds indices and holds them itself**: a `ChangedRun` carries its two sides and no positions and a context entry is a bare line, so indices on every entry would change the union's shape for two callers that would not read them; the cursor is exact because a run is a maximal *contiguous* stretch of non-context lines and therefore spans `removes.length + adds.length` of them — their count and not their order, since `-a +b -c +d` is one run whose sides interleave in the document. `rows` still comes from `pairedRows`, so the row arithmetic stays `height.ts`'s and a unit cannot disagree with `measure` about its height. **A fourth copy is now a visible change**, which is the whole of what this invariant buys (→ C04 I91, I19).
...
- **I20a** — **Retired with its caller** (F1251). (§3b.) It read *the first offset whose window reaches the last row, not `total − height`*, and it was true — sticky headers are counted once in the full rendering and again in every window carrying them, so the arithmetic ceiling stopped short and `collapsedAfter` became unreachable. The subject was a **caller holding an offset into a patch**, which was `patch-view.ts` alone (§3b). The transcript route holds no such offset: `BlockDefinition.window` is asked for rows `from`–`to` by C14, which derives them from the viewport and never from a patch's row arithmetic. **Marked here and retired one commit later**, because a retirement whose whole content is *the subject is deleted* cannot be read — or gated — before the deletion: SP9 refuses a row naming a retired invariant, and the rows go with `patch-view.ts`. Same inversion as A03's rule tables landing with the code, and for the same reason.
- **I20b** — **Retired with I20a and for its reason** (F1251). (§3b.) *A valid offset is a row a window may begin at, and every motion lands on one* — a rule about snapping a caller's offset, in a component with no caller that holds one. The finding it recorded survives where it can still be violated: I19's cut-at-run-boundaries, which is the window builder's own and is asserted at the transcript route.
...
10. Word-level highlighting is `underline` on the changed tokens of a paired line, written once by the builder over the renderer's own pairing, capped, whitespace-aware, and silent on an unrelated pair; the renderer merges it with the syntax stream by slicing and never diffs (I10, → C04 I91).
...
14. Inline expansion is always offered and nothing about it is configurable — there is no second route, so there is no threshold and no number (I15, I16, §3a).
15. The expanded form is every hunk of this block uncollapsed, **in the entry it already sits in** — never the whole file, which is the editor's and is one component rather than two (I17, §3b).
...
17. A window cuts at run boundaries in split layout, because pairing is not additive across an arbitrary cut of the *line array* (I19) — **and a row-wise cut is additive**, which is what lets the transcript route slice exactly (I19a). ~~An offset is always such a boundary, and the ceiling is the first offset that reaches the end rather than `total − height`~~ — both were the pushed view's and retire with it (§3b).
...
20. **A plan is held through the seam and a planned window costs the window** (I22, F1191). `window` reads and holds the plan in the caller's scratch, refusing one for another block or width; `windowRows` touches the window's rows only.
...
- **T1.9** (I10): the token rule — `foo(a, b)` is `foo`, `(`, `a`, `,`, ` `, `b`, `)`; a run of spaces is one token; `x_1` is one token; a combining mark stays with its base.
- **T1.10** (I10): `intralineSpans` over two lines differing in one word emits one `underline` span per side covering that word's offsets and nothing else; identical lines emit none; adjacent changed tokens merge into one span.
- **T1.11** (I10): a pair with `INTRALINE_TOKEN_CAP + 1` tokens on one side emits no spans, and the same pair one token shorter emits them — the boundary, not a point on one side of it.
- **T1.12** (I10): the whitespace ruling — a line re-indented by two spaces underlines the changed indentation on both sides, and a trailing space added underlines that one cell on the add side alone.
- **T1.13** (I10): the unrelated ruling — `foo bar` against `baz qux` shares only whitespace and emits no spans; `foo bar` against `foo qux` shares `foo` and underlines `bar` / `qux`.
- **T1.14** (I10): `intralineLines` pairs the *n*th remove with the *n*th add of a run and writes both; in a lopsided run the unpaired removes carry none; a context line carries none; a line the caller gave spans keeps them and its partner is left alone.
...
- **T1.20b** (I19b): over the whole patch corpus at a split width, at every height from 3 to 9 and every offset, each window's lines begin and end where `changedRuns` closes a group — the boundaries derived from that function rather than restated. **Its blind spot is stated in the file**: it cannot catch a change to `changedRuns` itself, because expectation and code move together (measured — dropping the flush at a context line leaves it green and fails T1.20, whose predicate is independent), so the two rows are complementary and neither is redundant. The layout is read through `layoutFor` and not off the width, because the corpus holds a patch whose `layout` field forces unified at a split width.
- **T1.20c** (I1, I19b): over the corpus at both widths, `hunkHeaderRows` equals the header positions computed from `height.ts`'s `hunkRows`, and the last row plus the tail equals `totalRows`. **The equality, because containment is satisfied by every wrong answer inside the bounds**: a unit that over-states its height shows less diff and never more, so it satisfies every other row here — restating a unit's rows as its line count survived 99 rows across eight files before this one existed (measured 2026-09-04). It states the agreement across the seam, the positions walking `unitsOf` and the expectation coming from the arithmetic `measure` uses.
...
- **T1.24** (I22, F1187): over the patch corpus at the unified and split widths, `windowPlan`'s rows count equals `totalRows`, its header rows equal `hunkHeaderRows`, its start rows are exactly the offsets `clampOffset` returns unchanged, its gutter width equals `numberWidth`; at every offset up to two past the end and at two heights the window built over the plan deep-equals `windowPatch` on the patch and the clamp over the plan equals `clampOffset`, and `windowRows` over the plan equals `windowRows` on the patch at every row range of a region's height; the plan is a frozen value with no reference to a height, and a plan for another block or width is refused rather than read. Not deferred on a component: the code commit replaces this row.
...
- **T2.7** (I10): the builder's pairing is the renderer's — for every run shape up to 3 × 3, the pairs `intralineLines` diffs are the pairs `changedRuns` yields and split layout draws, asserted by the underlined word appearing on both halves of one split row. The gate half is C04 T2.37.
- **T2.8** (I11): `Patch` and `Hunk` carry no `expanded` flag — `collapsedBefore` is the only field, and expansion *is* its rewrite. Rendered twice, the same block gives the same lines, so nothing outside the document decided what was shown; and a `replace` producing the same block without `collapsedBefore` is what makes the hidden lines appear. The structural half is the load-bearing one: a flag would be a second record of what the document already says, and the two would disagree the first time a patch was replaced under an open region.
- **T2.9** (I14, I15): **the absence watch, narrowed.** It asserted `maxExpandHeight` appears nowhere in `src/` and named three invariants that would then owe rows. The field is now specified not to exist (I15, I16), so the watch is a rule rather than a deferral, and it keeps its assertion for that reason. The half that is C25's and is true today stands unchanged: the collapsed and expanded heights are both computable from the block before either is offered, `measure` being pure — and `Measure`'s parameter list carries no height, which §3a names as the one change that must not be made.
- **T2.10** (I17): ~~`PatchViewDeps` is `overlays`, `transcript`, `region`, `redraw` and an optional `probe`~~ — **struck with the view** (§3b). *Never needs data the block does not carry* was asserted on a **dependency list**, and the component holding it is deleted. The claim survives where it is now true by construction: an expanded patch is the block rewritten in place, so there is no dependency list into which a fetch could be added. Its replacement is T2.12 below.
- **T2.11** (I19a): over every run up to 4 removes × 4 adds, at every cut point, a row-wise cut gives exactly `k` rows and leaves exactly `rows − k` — `max(min(k, r), min(k, a)) = k` and `max(r − min(k, r), a − min(k, a)) = max(r, a) − k`. Asserted against `pairedRows` itself rather than against a restatement of it, since the whole invariant is that the window and the renderer share one row model.
- **T2.12** (I17, R-EXA-082): **the deletion is pure, and the row asserts that rather than a replacement.** A patch in a settled entry renders every hunk it carries, at every width, with no action offered and no layer raised — and the rendering is byte-identical to what the pushed view drew at the same width, because the view's *uncollapsed* was I14's hunk **cap** and the cap is unbuilt (§3a, T2.9). **Measured before the code was written and it is why no expand arm was added for a patch**: `collapsedBefore` and `collapsedAfter` are counts of context the block does not carry, so there is nothing an expansion could reveal. The row fails the day the cap lands with no route to what it dropped, which is D38 (I16).
...
- **T3.11** (I15, I16): expansion is offered on every patch — the tall one and the short one alike — and the offer is decided without rendering either. The row asserted a **threshold** until the design deleted the rung it routed to (§3a), and what it now asserts is the absence of one: two patches whose expanded heights differ by an order of magnitude carry the same offer.
...
- **T3.17** (I10): a span straddling the truncation cut is cut with the text, and the marker is never underlined — at both unicode rungs, since the marker is a different string at each.
- **T3.18** (I10): a span boundary inside a syntax token splits the token; both halves keep the slot's colour and only the covered half is underlined.
- **T3.19** (I10): `windowPatch` and `windowRows` carry spans with their lines — a window cut inside a run in split layout, row-wise, shows the same underline on the same word as the whole.
- **T3.21** (I10): a span the writer starts inside a grapheme cluster is snapped outward by `runsOf` (C04 I84), so the cluster is painted whole and nothing measured moves.
...
- ~~**T6.23** (the retired offset rule): the ceiling back to `total − height`~~ — struck with the offsets (§3b). Replaced by **T6.23** (I17, R-EXA-082): a `view` arm restored to the action dispatcher, pushing a fill layer over a patch → T2.12 fails on a layer being raised, and the transcript is a hole where the diff was.
- ~~**T6.24** (the retired offset rule): the snap moved from the clamp back into the builder~~ — struck with the offsets (§3b). Replaced by **T6.24** (C04 I34): `view` put back into `ACTION_KINDS` → T2.13 fails. **The revert that changes no behaviour and is still a defect**, because nothing dispatches the kind: the gate would admit an action from a far side that is silently inert, which is the key-that-does-nothing-and-says-nothing C04 § rules out.
...
- **T6.25** (I21a): dropping the pin, so a window derives its own gutter → T3.20 fails. **This is the shipped behaviour**, and its symptom is not a crash or a wrong height: it is every line of a large diff moving three columns sideways as the reader scrolls past an expanded patch, which reads as the terminal misbehaving.
...
- **T6.26** (I10, I7): moving the diff into the renderer, so a hunk supplied without spans is underlined anyway → the row asserting a span-less document paints no SGR 4 fails, and `render` acquires a per-frame O(n·m) it must never have.
- **T6.27** (I10): letting the truncation marker inherit the last run's attributes → T3.17 fails at both unicode rungs.
- **T6.28** (I10): the builder pairing the *n*th remove with the *n*+1th add — a second pairing that drifts from the renderer's → T2.7 fails, and the underline on the left of a split row describes the change on a different row.
...
| **Word-level highlighting within a changed line** | Built (I10, 2026-09-04) — but its *writer* is L0's `intralineLines`, called by `b.patch`, and this renderer only paints what the line carries. A patch constructed without the builder carries no spans and draws none; the renderer never diffs |
...
| ~~The fullscreen view's implementation~~ | **Deleted, not re-homed** (§3b, R-EXA-082). It was built as `src/shell/patch-view.ts` and the design language ruled the surface out; a run's detail expands in place. The row stays struck rather than removed, because *what it once owed and to whom* is the record that explains why `window.ts` lost four exports |
| A whole-file view with diff decoration | The editor, and it is one component with editing turned off — the one place a patch legitimately wants a scroll model of its own, which is why deleting the pushed view does not touch this row (§3b) |
