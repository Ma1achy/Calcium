...
- **T1.24** (I22, F1187): over the patch corpus at the unified and split widths, `windowPlan`'s rows count equals `totalRows`, its header rows equal `hunkHeaderRows`, its start rows are exactly the offsets `clampOffset` returns unchanged, its gutter width equals `numberWidth`; and at every valid offset the window built over the plan deep-equals `windowPatch` on the patch — and equals the row-wise `windowRows` slice of the same rows where both apply; the plan is a frozen value with no reference to a height. Not deferred on a component: the code commit replaces this row.
...
- **T2.10** (I17): `PatchViewDeps` is `overlays`, `transcript`, `region` and `redraw` — no fetch, no adapter, no transport — so the fullscreen view can read nothing but the block it was opened on, and every hunk of that block is reachable by its motions. *Never needs data the block does not carry* is a claim about the **dependency list**, which no assertion about output can see.
