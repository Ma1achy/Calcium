...
- **I22** — *(C22 §6m, §107, `R-ACC-001`)* **Under `capabilities.renderMode === "linear"` acquisition takes raw mode, bracketed paste and the keyboard protocol, and never the alternate screen, mouse tracking or the hidden cursor.** A linear stream is appended to the terminal's own scrollback, a mouse report would arrive as typed input nobody asked for, and a screen reader follows the cursor. Release restores exactly what was taken, by the same table (I6). **And the missing alternate screen is not fatal on this profile** — the refusal exists because a frame cannot be drawn without one, and linear draws no frame. `escapes.ts` gains the two sequences the input line needs and nothing that addresses a row: `ERASE_LINE`, carriage return and erase-in-line 2, and `cursorColumn(n)`.
- **I23** — *(C22 §6n, ruling 27, → C02 I17)* **Focus reporting (`CSI ? 1004 h`) is taken iff `capabilities.notify` is non-empty**, after the keyboard protocol and released before it, like every mode (I6). With nothing opted in, not a byte of acquisition changes.
- **I24** — *(C22 §6n.3 rows 4–5, `R-NTF-001`)* **The title is a mode taken late: pushed (`CSI 22 ; 2 t`) the first time `title(text)` writes, popped (`CSI 23 ; 2 t`) by `restoreTitle()` and at release when pushed, and never pushed twice without a pop between.** `OSC 2` carries the text, control-stripped. A terminal without a title stack ignores both, and the title it shows afterwards is the last one written.
...
- **T1.20** (I1, I6): `setMouseTracking(false)` emits `1006l` then `1002l`, in that order; `(true)` emits the enter pair back. **Here because nowhere else may write an escape sequence** — SS14 and SS15 refuse a second writer before any suite runs, so the toggle is C01's or it does not exist.
- **T1.21**: the toggle is idempotent — a second call in the same direction emits nothing, so a caller never has to ask what the current state is.
- **T1.22** (I10): with `mouse: false` the mode was never taken, so both directions are no-ops and `held` stays truthful.
- **T1.23** (I6): with tracking left off, `release()` does **not** emit a second `leave` for a mode already left. `held` is the one record; a separate flag beside it is two records that drift.
- **T1.25** (I20): the same shape resolved on three consecutive frames → the sequence is emitted **once**, and a fourth frame with a different shape emits again. Asserted on the bytes C01 yields, because *not emitted* and *emitted identically* are indistinguishable from anywhere else.
...
- **T1.30** (I22, I6): `renderMode: "linear"` → acquisition writes no `?1049h`, no `?1002h` or `?1003h`, no `?25l`, and carries `?2004h` where `bracketedPaste` is true; release writes none of their resets; `altScreen: false` on this profile opens rather than reaching `onFatal`. **And the control**: the same harness under `rich` writes `?1049h` and `?25l`.
- **T1.31** (I23, I24): `notify: ["bell"]` → acquisition writes `?1004h` after the keyboard protocol and release writes `?1004l` before its reset; `notify: []` writes neither. `title("a")`, `title("b")`, `restoreTitle()` → one push, two `OSC 2`, one pop; a second `restoreTitle()` writes nothing; `title("c")` then `release()` → push, title, pop.
...
- **T6.20** (I20): writing the shape as a `mode()` pair, so release emits a *leave* → T1.26's second arm fails, and a session that set no shape resets the terminal's own on exit. **The taxonomy made load-bearing**: a `leave` and a *set to default* are different claims about the same bytes, and only the second arm can tell them apart.
