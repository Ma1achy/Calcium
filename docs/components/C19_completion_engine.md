...
**And the failure goes somewhere** (I6, T3.6). `EngineOptions.onSourceError` is the engine's only report of a dropped source, and for the whole life of this component every test supplied one and the product did not — so I6 held and the record was empty: no line anywhere said which source failed or why. `createSourceErrorSink()` is the product's: one deduplicated line per failing source, with a count and the *first* message, in `BlockFaultLog`'s shape — a pull rather than an emit, drained with the other diagnostics at C22 §8 step 3 rather than painted onto the alternate screen where it would be discarded with it. Keyed by source and not by message, because a source that is down fails on every `Tab` for as long as it is down and each failure may carry a different figure; keyed by message that is a flood. An engine built with no sink drops the failure in silence, and that is stated rather than defaulted away: a test that wants to see failures passes its own.
...
- **I6** — A source that throws or times out is dropped from that request; others still contribute. **And is reported to the sink once per source, not once per keystroke**: `createSourceErrorSink` keeps one line per source id with a count, and the product passes it through `onSourceError` (§3).
...
- **I28** — *(§011, `R-BLK-076`, `R-BLK-078`, C17 I25, I11, I16)* **A candidate may accept into a CHIP rather than into text, and that is the one seam §011 asks for.** *@ pulls a file into the turn, and it is a chip like a paste* — and the two halves of that sentence were both built and had never met: C17 has minted a chip since roadmap 30 and the only caller is a large paste, while every candidate this engine has ever produced accepts as a string. So the mention was expressible in neither direction: a source could offer a file, and accepting it inserted a path.

  **The candidate carries the chip's parts and never its label.** `kind`, `name`, `lines` and `content` are the source's — a file's basename, its line count, what it stands for — and the **ordinal and the form are C17's**, which is C17 I25's rule arriving at a second producer rather than being restated by it. A source handing over a finished string would put §011's label in every application separately, which is what `construct.ts` did before I25 and what I25 exists to stop. §011 draws `[parse.ts · 184L]` and §101 draws `[#1 json · 47L]`; both are that one composer, given a name or given none.

  **Accepting a chip is still one edit and still one undo unit** (I11). The token the caret is in is replaced by the chip, so the span `accept` computes is the span the chip occupies — not a deletion followed by an insertion, which is two units and leaves a reader one `⌃_` short of where they started. **And `delimiter` still applies**: a chip closes its token exactly as a unique match does (I16), so the next keystroke is in the next slot rather than inside a chip's name.

  **The delimiter travels *inside* that one edit, and the first build got this wrong** — which is the clause the row would not have found without reading the whole buffer back. `insertChip` took the span and a second `insert` took the space, and the frame was indistinguishable: the token gone, the chip under the caret, C17's label beside it. What differed was one keystroke later. `⌃_` took back the space and left the chip standing, because the space had a unit of its own. So the span and the delimiter arrive together, as one argument, and `insertChip` writes them in one call: *one edit* means the whole of what the accept does, not the part of it that is the chip.

  **The size shown before attaching is the source's and the size that travels is the chip's**, and they are two strings. §011's menu draws `184 lines · 6.2 KB` and the chip draws `184L`: the first is `Candidate.detail`, free text the source composes because only it knows bytes, and the second is `Chip.lines` through C17's composer. A design reading them as one field would make the engine format a file size.
...
24. **A candidate may accept into a chip, and the chip's form stays C17's** (I28, §011, C17 I25). §011's *@ pulls a file into the turn, and it is a chip like a paste* is one seam between two halves that both shipped and had never met — C17 mints chips and only a paste called it; the engine makes candidates and all of them were strings. The source supplies the parts; the ordinal and the label are C17's, as they are for a paste.
...
- **T1.71** (I28, §011, C17 I25, I11): a candidate carrying chip parts accepts into a **chip** and not into text — the buffer holds one sentinel where the token was, `chipAt()` answers with the source's `kind`, `name` and `lines`, and the drawn label is C17's composer's, `[parse.ts · 184L]`, which the source never spelled. **One undo unit**, asserted by undoing once and reading the **whole buffer** back — `echo par`, exactly — because the shapes this row is written against all draw identically and differ one keystroke later: a delete followed by an insert, and a delimiter inserted separately, which is what the first build did. The buffer before the undo is read as a shape too, with each chip sentinel drawn as one mark: `echo ◆ ` pins the token gone, one grapheme where it was and the delimiter past it, where four counts taken separately are all satisfied by a chip inserted *beside* the token. The control is the same candidate without the chip parts, which inserts its value as every candidate always has.
...
- **T3.6** (I6): one of three sources throws → the other two still contribute; the failure is logged once. **Against the product's sink, not only a test's array**: a source that fails on three consecutive requests is one line in `createSourceErrorSink().messages`, carrying `×3` and the first message; two failing sources are two lines.
