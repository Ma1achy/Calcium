...
- **T1.71** (I28, §011, C17 I25, I11): a candidate carrying chip parts accepts into a **chip** and not into text — the buffer holds one sentinel where the token was, `chipAt()` answers with the source's `kind`, `name` and `lines`, and the drawn label is C17's composer's, `[parse.ts · 184L]`, which the source never spelled. **One undo unit**, asserted by undoing once and reading the buffer back: the shape this row is written against is a delete followed by an insert, which draws identically and leaves the reader one `⌃_` short. The control is the same candidate without the chip parts, which inserts its value as every candidate always has.
