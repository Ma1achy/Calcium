...
mark, so the pair is across two facts and not within one, and a second carrier for selection would
have to be a mark — which is the cell C11 I14 forbids. `inverse` is not a second carrier: it is the
same channel at a rung where colour is gone, which is exactly why it is the honest answer and not
a repair. **Choice is the same shape from the other side**: the mark carries *chosen* and the wash
