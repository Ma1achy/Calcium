...
**And the cells that are not text, recorded so the next reader does not count them as instances.** `sankey.ts`'s half-block puts one owner's slot in the foreground and the lower owner's in the **background**, and `scatter3.ts`'s braille and quadrant cells do the same; both go through `slot()` rather than `resolveBackground`, which is I21's picture-cell admission taken by name. They are art and carry no glyph a reader reads — and the margin says how much rests on that: `categorical × categorical` is **1.00** at worst, so the cell is invisible in greyscale and legible only by hue. **Nothing enforces that such a cell carries no text**, which is the same shape as the exclusion §4f was written about, one channel over. Left owed below.
...
| a sankey half-block, a braille cell | I21 — a picture cell may take a palette ref as a background, because it carries no text | §4 — a foreground on a surface clears a floor | **A, and it is load-bearing rather than obvious**: the worst `categorical × categorical` pair is 1.00 on every shipped theme, so the whole of the argument is *no text is in the cell* and nothing checks that |
...
- **A picture cell's background is still checked by nothing, and the margin is 1.00.** I21 admits a palette ref as a background for a cell that carries no text, and *carries no text* is a property of the caller rather than of the type: `sankey.ts` and `scatter3.ts` both reach it through `slot(...).colour` rather than through `resolveBackground`, so the type cannot refuse a text-bearing cell the way `wash`'s `Span` return does. Owed to C12 and to I21, and it is the same shape as the trigger §4f was written about.
