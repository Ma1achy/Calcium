...
type ThemeTokens = Readonly<{
  ...
  surfaces: Readonly<Record<string, string>>;      // 25 in every shipped theme — below
  fourBit:  FourBitMap;                             // the curated table, by reference
  composed?: Readonly<Record<string, Readonly<Record<string, string>>>>;  // R-THM-001
}>;

/** slot name → ANSI index, 0–15. Curated per theme (§3), never computed. */
type FourBitMap = Readonly<Record<string, number>>;

type ThemeSet = Readonly<Record<string, ThemeTokens>>;   // keyed by name (I27)
...
### A curated table is pinned by value, because a property test cannot see a substitution

**The 4-bit map is curated, and every row written about it measures a property it shares with
many other maps.** T2.5 says no index falls outside it; C10 I26 says the five meaning tones
stay distinct at that rung. Both are satisfied by any number of maps that are not this one,
and the thing at risk is not a property — it is the *curation*: `syntax.type` on plain yellow
so `number`'s bright yellow stays its own, and the rest of that map's argument.

**Measured, in this repository.** The theme generator handed `hcDark` the ordinary dark
theme's indices — `variant === "light" ? LIGHT : DARK` — and `HIGH_CONTRAST_FOUR_BIT` became
data nothing read. Every row about `hcDark` passed, because `DARK_FOUR_BIT` is a legal map and
it also keeps the five distinct. A mutation found it; nothing else could have.

So a table that was **hand-chosen rather than derived** is asserted as the value it was chosen
as, and the list of such tables is **driven from the modules' own exports** rather than kept by
hand — an allow-list is only as good as the discipline of adding to it, and this one has 1 267
registry rules regenerating beside it. I44.
...
The single failure, `mono` `meta` on `selection`, **is the missing override**: the design gives it `#b8b8b8`, which clears, and the tree paints `#8e8e8e`, which does not. The design's own value closes it — no colour is chosen here.

### The derived pairing cannot see the loss, and that is why there are two rows

**Stated because the first draft of this section got it wrong, and the error is the rule's own shape.** A pairing derived from `composed` forms a pair for `mono`'s `meta` on `selection` **because** that composition exists. When the generator dropped it, it dropped the pair with it — so on the tokens as shipped the widened scope is *green*, and the 3.86 : 1 is a ratio nothing computes. The pair and its ink go missing together.

That is the same failure this section is about, arriving inside the remedy: a check whose corpus is the record it is checking is silent about the record. So the two clauses of I49 are **two rows, not one**, and only the first can see a loss:

- **T1.44 reads the registry.** It is the only thing in the tree that compares what the design declares against what the token set carries, and it is red on the tokens as they stand, with the seven named.
- **T1.45 reads the tokens.** It is a *scope*, and what it buys is the next failure rather than this one: a composed ref that arrives at a value below its floor is caught on the commit that adds it, where before nothing on this ground but `tone.default` was measured at all.

**Neither row is worth having alone**, and neither would have been written had the first draft's claim — *T1.45 is red at 3.86* — gone unchecked, because a rule that appears to catch the defect is a rule nobody widens.
...
is an availability fact and `error` a validity fact (§110's twelve `stateAxes`), and neither word
...
### 4k.5 — the carrier table: what holds each axis, and at which rung

**One row per axis the registry carries, each traced to a FIELD and the RENDERER that
reads it** (I56, `R-COR-003`, M11). Not derived from §4k.2's prose: a carrier declared from
this section rather than from the code is a claim with no source, which is the failure this
component met at row 1's third clause and again at I48's 74 of 100 pairs.

A carrier **survives 1-bit** or it does not, and that is the whole of *independent*: `mark`,
`word`, `weight`, `position` and `inverse` survive; `tone` and `ground` do not. So a pair of
`tone` and `ground` is one carrier written twice, which is what `R-COR-003` is about.

| axis | the field | the renderer | carriers | survives 1-bit |
|---|---|---|---|---|
| **lifecycle** | `ToolCallSpec.state` | `callHead` (`documents.ts:390`) | mark + tone | mark — *the head carries a state, not a character*, and the renderer substitutes the state's own mark where tone does not carry |
...
| **pointer** | — | — | **no subject** | 1003 arrives (`lifecycle.ts:125`) and the router discards a hover by rule (§4k.4) |
| **availability** | — | — | **no subject** | `disabled` is a field no block carries (§4k.4) |
| **freshness** | — | — | **no subject** | `stale` likewise; §4k.2 rules it *a weight on the content* and nothing carries the field |
...
**Ruling 33 makes this an exception with an end rather than a grant.** R-THM-005 supersedes
R-THM-003's *the ground is selection's only carrier* with *only ground-level carrier*, and names
the second: the `▌` selection rail, §017's gutter mark. This paragraph used to say a second carrier
...
**Four axes have no subject, and that is a population fact rather than a violation.** An axis no
block can express cannot lose a carrier, and a gate that demanded two would be asserting over an
empty set — A03 §2's class again, this time manufactured by the gate rather than found by it. The
four are named with their reasons so the row goes red the day a field arrives.
...
- **Three of the six frames cannot be drawn, and the three reasons are not one reason.**
  ...
  **This bullet used to say the producer was missing — *mouse mode 1002 sends no motion* — and the
  tree falsified it in both directions.** `lifecycle.ts:125` has taken **1003** behind a
  ...
  change is the **field**, which is what C10 T2.48 asserts. *Disabled* and *stale* are fields no block carries, so the fact cannot be
...
- **The precedence list stops being closed, and R-STA-004 does not close it.** The list was
  read as covering every fact that could want a cell. **Counted from the registry rather than
  from the prose**, `stateAxes` carries **twelve** axes, every one `current` and every one ruled
  by `R-STA-001`, and R-STA-002's six rungs plus R-STA-004's new one partition them three ways:

  | | axes | |
  |---|---|---|
  | **ranked by name** | ownership, pointer, selection, focus, availability | 5 |
  | **reaching a ground only through `semantic extent`** | outcome, validity | 2 |
  | **no rung at all** | lifecycle, resolution, choice, disclosure, freshness | 5 |

  The middle row is the one a count hides: `semantic extent` is a **single** rung, so an outcome
  and a validity fact contesting one cell are ranked against everything else and against nothing
  ...
  **The five with no rung all carry by something else, and none is without a carrier** — so
  ...
  |---|---|---|
  | **lifecycle** | **mark + motion** above 1-bit, **the mark alone** below it, falling back to text | `● mark · verb · duration slot · result summary · ⎿ gutter`; motion-off omits the spinner and static `●` plus elapsed carry liveness (R-MOT-002). **At 1-bit and in ASCII the mark is a function of the state** — `◌ ◐ ● ○ ⊘` and `. % * o /` — where above it the mark is the constant `●` and tone carries the state, so the carrier that survives the rung is the **glyph** and not the ink (`CALL_STATE_GLYPH`, `toneCarries`, `headMark`; SS64 gates *no two states render the same glyph*). This row read the coloured rung alone until the monochrome rung landed |
  ...
  | **disclosure** | **mark**, and both now exist | `▿` is *expanded* and `▹` *collapsed* (`glyphs.ts`'s `collapse`/`expand`, registry `disclosure` and `disclosure-collapsed`, ASCII `v` and `(`); `leaf` is correctly an absence. **This row read *`collapsed` is a named carrier with no glyph, which is M4's*** until the mark landed — hollow says *there is something behind this*, the filled `▸` stays focus's, and the first ASCII pick `:` was withdrawn on SS59's collision check (F1246) |
  ...
  **Lifecycle's one-bit row was the design's and not the tree's, and it has landed.** The head mark
  was a **constant** at every rung with the outcome in bold; it is now a **function of the call's
  state** at 1-bit and in ASCII — `◌ ◐ ● ○ ⊘` and `. % * o /` — keeping the collapse onto one
  `●` for the rungs where tone can carry the state, because shape is what is left when tone is
  gone (`CALL_STATE_GLYPH`, `toneCarries`, `headMark`). So the carrier named above is *mark* on the
  strength of a ruling **and of the code**, and the mark is no longer one glyph short of
  distinguishing the states it names. `R-GLY-002` was red on HEAD when this was written — `running`
  and `step` both resolving to `*` — and is green as **SS64**, a gate as of M4's close. The
  `MILESTONES.md` row it named has been discharged by deletion, which is that file's own rule.
  ...
  **An earlier form of this bullet said seven axes of which the list mentions two**, and both
  ...
  component* — where the seven are the **rows of a component's table** (entry, carriers, actions,
  escape, motion-off, one-bit, residue) and not axes at all; §092 is *six more* **components**,
  not six more axes. A number was carried across a section boundary into a sentence where it read
  as though it had been measured. The measurement is the table above.
...
- **I22** — *(amended — R-THM-001)* **The surface set is the registry's and it is twenty-five, not ten.** Everything I22 says about the *pairing* stands: the two diff surfaces are text-bearing, the twelve slots that land on them are checked against them and no others, `bgDeep` stays excluded because it carries no text, and the withdrawn strong pair stays absent. What moved is the inventory. `ThemeTokens.surfaces` was ten — `bg bgElev bgDeep border borderStrong diffAdd diffRemove selection errorGround errorInk` — and R-THM-001 assigns fifteen more, every one of which the design draws: `focusGround` (the ground M3's precedence stack owes focus), `selection`'s siblings `chosen`/`chosenInk`, `pick`/`pickInk` and `link`/`linkInk`, `skipGround`/`skipInk`, `meterFill`, and the four mode grounds `mAuto`/`mAccept`/`mPlan`/`mManual` with their shared `mInk`. **A ground and its ink are one surface pair and both are surfaces**, which is the placement `errorInk` already had (§4d): an ink put in `tone` would be measured against `bg`, where a black ink for a red ground fails every floor for a pairing nothing draws.

  **The registry's `.bg-error` is not a twenty-sixth.** It carries the same declaration as `.bg-errorGround` in all ten themes — an alias in CSS, and a collision in a token set, because `surface.error` beside `tone.error` is two colours reachable by one word and a typo between them resolves rather than failing. The generator drops it and asserts the two are equal first, so the day the design parts them it is a design change and not a silent dedupe.

  **The count stays the row's whole subject** (T2.14c) for the reason it always was: a spec that measured something out and a token file that quietly kept it is the drift this suite exists to stop.

  - **I22 (as first written)** — The two diff surfaces are text-bearing, and every `syntax` slot and every gutter tone (`ok`, `error`, `muted`) clears its floor against both in both variants (§4a) — **those twelve slots and no others**, asserted on the pairing rather than on its results, because a widened pairing passes on the tokens as shipped and only costs something later. There is no third or fourth: §4a measured a stronger pair for word-level emphasis and found under ten units of one channel between it and the plain pair, so word-level emphasis is `underline`'s and not a background's. `bgDeep` remains excluded because it carries no text; the criterion is text, not the word "surface".
...
- **I25** — *(amended — R-THM-001)* **Six themes inherit and four paint, and the field is what says which.** I25 stands word for word; only the population under it moved. At three themes the split was two painting and one inheriting, which reads as `dark` being the odd one. At ten it is `dark ink warm nord viol mono` inheriting and `light hcDark hcLight paper` painting — **every light theme paints, and one dark theme does**, which is the shape the field was for: a dark theme can sit on whatever ground the emulator already has, and a light theme cannot, because the emulator's ground is dark far more often than not. `hcDark` paints because high contrast is a promise about a measured pair, and a pair one end of which the terminal chose is not measured (T2.24).

  **This is where the M2 caution lands as a rule.** The design HTML draws every specimen inside a terminal frame with its own background, and that frame is the specimen's chrome — `.term`, `.sw` and `.rmp` are classified as preview chrome by R-THM-001's own text. No registry rule assigns a terminal background, so nothing in the ten themes paints one that the theme's `background` field did not already ask for.

  - **I25 (as first written)** — **A theme's background declaration is a choice and never a colour.** `background: "terminal" | "surface"` — the value names **where the colour comes from**, the terminal or the theme's own surface, rather than naming the act — and what `surface` paints is `surfaces.bg` — the one surface every floor is already measured against (I19). A colour in this field would be a second source of truth for the same thing, so a theme could paint one value and prove its floor against another, which is roadmap 39's own defect arriving from the other side (§4c row 1). `--no-bg` overrides it for one invocation and is **session state, not an `Overrides` entry**: overrides merge into `tokens`, bump the serial and change the theme's identity, and a per-invocation switch that did any of that would be sticky by construction and would put a paint decision into every cache keyed on identity (I11, → C22 I58).
...
- **I27** — *(amended — R-THM-001)* **The shipped set is ten, `high-contrast` is two themes, and declaration order is load-bearing.** I27's mechanism is unchanged and this is the case it was written to allow: `ThemeSet` is keyed by name, so a set can hold seven dark themes and three light ones, and `high-contrast` splitting into `hcDark` and `hcLight` is exactly the *two themes of one polarity* T1.21 already covered. The shipped set is `dark light hcDark hcLight ink warm nord viol mono paper`.

  **`high-contrast` is not an alias and is not kept.** This is 0.x and the name named one theme that was dark; keeping it pointed at `hcDark` would ship a name whose polarity is a historical accident, and a persisted preference for it now fails the membership test the guard already performs (§5a.3) and falls back rather than resolving to something arbitrary.

  **Declaration order is the resolution order, so it is chosen rather than inherited.** When a terminal reports a polarity and the app has expressed no preference, C22 I68 takes the *first* theme of that polarity, so the order decides which light theme a light terminal opens on. The registry lists `hcLight` before `light` because its HTML groups the high-contrast pair; inheriting that would ship high contrast to every light terminal. The generator states an order instead — the canonical pair, then the accessibility pair, then the rest — and asserts it covers the registry by equality, so a theme added there and not named is a build failure rather than a silent drop.

  - **I27 (as first written)** — **A theme set is keyed by name and a theme declares its own polarity.** `ThemeSet` is `Readonly<Record<string, ThemeTokens>>`; `dark` and `light` are names in the shipped set rather than a closed vocabulary, so `high-contrast` is a third theme and not a third variant. The keys carried polarity only because a two-theme set had nowhere else to put it, and I25 is what took that job — a theme now declares the background it assumes, which is the fact the keys were standing in for (§5a.1). **Nothing reads `variant` as polarity**: its five readers are all in `store.ts`, each a key into the set or part of the memo identity, and no consumer in `src/` reads it at all — which is the measurement the fork rests on rather than the argument for it. **That measurement has since expired, in the direction this sentence predicted**: `construct.ts` reads `variant` to choose the opening theme from the terminal's detected polarity (C22 I68), so there is now one consumer and it is the use the next clause published the field for. The sentence stays true of `store.ts` and stops being true of the tree, which is why it is corrected here rather than deleted — the fork's evidence was the count at the time and not a claim about the future. `ResolvedTheme.variant` stays published, because *no reader here* is not *no reader* and choosing an asset by polarity is what a published field is for. The persisted preference needs no migration, since the two literals remain valid names, and the guard restoring it becomes a membership test against the set (C10 §5a.3 rows 1–2, → C22 I40).
...
- **I32** — **`errorGround` and `errorInk` are one pair, minted together and checked together at the full meaning floor; the ground is authored beside the tone rather than being it, and `error` carries no lowered floor.**

  **AMENDED against the design registry (R-THM-001), which supersedes the equality and the exception together.** Everything below this paragraph is the record of why the equality was taken and stays because the reasoning was sound about the constraint it faced; what changed is that the constraint is not binding any more.

  The equality existed because one value had to be both legible on `bgElev` and dark enough to hold white — *"zero of 262,144 colours"* on a dark page — so the slot was pinned to the ground and the floor on `error` was dropped to 2.5 to pay for it. The registry does not pin them: `dark` takes `tone.error` `#f05a5a` with `errorGround` `#c62828`, `light` takes `#c21f10` with `#a81f12`. **Two values do what one could not**, and the measurement is the whole argument — dark's `error` goes from **2.97 / 2.83** against `bg` / `bgElev` to **5.23 / 4.78**, while `errorGround` still holds white at **5.62**.

  So `FLOORS.error` returns to the 4.5 default. **Measured across all ten shipped themes before removing it**: the tightest is `dark` on `bgElev` at **4.78**, and every other theme clears with more room. An exception this component carried under duress is retired rather than inherited — and a floor that no shipped theme needs lowered is a floor, not a preference.

  **What the equality was protecting is kept by other means.** *Two literals kept in step by hand* was its real argument, and the answer now is that both values come from one generated projection of the registry: a hand edit in `tokens.generated.ts` is a value the design does not hold, which `make design-check` and `make themes` enforce between them. `hcDark` still ships ground and tone as one value — the design does that too — so the equality survives as a theme's choice rather than as a rule every theme must take.

  **The original record follows.**

  - **I32 (as first written)** — **`errorGround` and `errorInk` are one pair, minted together, checked together at the full meaning floor — and the floor on `error` is 2.5 because the slot now answers to two constraints that a dark page cannot satisfy at once.** The tag is the only painted run in C09's `status` box, and `tone.error` cannot stand in as its ground: it is authored as a foreground for a dark page, which is I21's rule from the other direction. The **ground is `tone.error` itself**, so text and box are one red by construction rather than by two literals kept in step by hand. **Measured over the whole 8-bit cube rather than over reds**: on dark and on high-contrast, **zero of 262,144** colours are both legible on `bgElev` and dark enough to hold white at 4.5; on light, **81,907** are, which is why light clears 4.5 unaided at 6.42–7.01. So 2.5 buys `#c62828` at **5.62 : 1** in the tag and costs the message text **2.83** against `bgElev` — `muted`'s standard, the quietest thing that must still be readable. **The alternative is real and is shipped**: high-contrast takes a light ground with dark ink, `#3d0000` on `#ff7171`, and needs no exception; the same would work on dark and gives up the dark red, which reads as a warning rather than a failure. **So this is a preference honoured, not a constraint discovered** — stated here because someone lightening the red to satisfy `FLOORS` would be undoing a decision rather than repairing an oversight. What keeps it honest is that the text is never the only carrier: the `▲` and the painted word both survive it, and both survive 1-bit where colour does not (§4d, F34). **And the pair has a 4-bit rung, which it was shipped without** (F240). The ground is `tone.error`'s **index** there for the same reason it is `tone.error`'s hex at 24-bit — one red by construction rather than two values kept in step — and the ink is the half that reads on it, which flips dark's from white to black because `tone.error` is a dark red at 24-bit and the bright one at four. **No ratio is claimed and none can be**: I26 rules the floor best-effort at this rung, 0-15 being the emulator's own values, so what is curated is a decision and not a measurement. **1-bit is untouched and is not the same case** — I8 leaves nothing to arrive, so the tag is distinguishable by being the one run that carries no styling, which is C09 §3a's rule and correct there alone.
...
- **I40** — **`sampleRgb(map, t)` and `shadeRgb(r, g, b, k)` are the numeric form of `sample` and `shadeColour`'s rgb arm, equal bit-for-bit over every (map, t, k), and they exist so a per-sample caller can hold the eight-bit channels instead of a hex string it would parse straight back.** `sampleRgb` returns the three rounded eight-bit channels `sample` computes, and `sample` is `rgbHex(sampleRgb(...))` — one interpolation, expressed once, so the two cannot drift. `shadeRgb` runs `overChannels`' own sequence on those channels — `toSrgb(toLinear(c / 255) · k) · 255`, rounded and clamped to `[0, 255]` — with **`toLinear` read from a 256-entry lookup table** whose entry `i` is `toLinear(i / 255)`; the table is exact rather than approximate because the only input `overChannels` ever gives `toLinear` is an eight-bit channel over 255, so the table's domain is the whole of it. `shadeColour(colour, k)` on an `rgb` value equals `{ kind: "rgb", hex: rgbHex(shadeRgb(...)) }`, `k` clamped and `k === 1` returning the colour unchanged. **The eight-bit arm is unmoved**: `continuousColour`'s `ansi256` answer and `shadeColour`'s `ansi256` arm still go through `overChannels` and the cube, because there is no hex round-trip to remove there — the channels are already an index. **What it does not reach, stated because an unrecorded limit reads as strength**: `by: "series"` takes a slot and no map, so it is untouched; and the numeric path is a byte-identical optimisation of a hot loop, its only consumer `scatter3.ts`'s per-sample fill, so nothing about the drawn frame changes and the `plot-meshes` golden is its integration gate (F1150).
- **I41** — **A shipped theme's quantisation is a shipped constant, and the DP never runs for it.** `quantiseSet` is a pure function of the set's values — the same hexes give the same picks on every machine — so its answers for the shipped set (three themes, their surfaces and their palettes) are computed once by a generator and shipped as a table keyed by the set's own values, in slot order; `quantiseSet` reads the table before it computes, and a set the table does not hold — an app's own theme, a `--no-bg` patch — computes as before. **Exact by construction**: the table is the computation's output and a row sweeps every entry against the computation and every shipped set against the table, so a theme edited without a regeneration is a red row rather than a stale constant. **The cost it removes is the interpreter, not repetition** (F1163): the two cold runs a load pays for `light` and `high-contrast` are 65–78 ms of a 600 ms native startup, and eight warm calls read 34, 35, 18, 22, 4, 1.1, 1.3, 1.1 ms — the optimiser arriving, not a cache. Memoising by value would have saved nothing on a cold session, whose two calls are on two different themes; a table saves both.
- **I42** — **`samplePacked(map, t)`, `shadePacked(packed, k)` and `packedHex(packed)` are I40's forms on one integer, equal to them bit for bit, and they exist so a per-sample caller holds one number in a typed array and allocates nothing per write.** `samplePacked` is `sampleRgb`'s three rounded channels packed as `r·2¹⁶ + g·2⁸ + b`; `shadePacked` runs `shadeRgb`'s channel function — the same `toSrgb(LUT[c] · k) · 255`, rounded and clamped — on each unpacked channel and packs the results, `k` clamped as `shadeRgb` clamps it; `packedHex(p)` equals `rgbHex` of the unpacked channels. The channel function is one function, shared by `shadeRgb` and `shadePacked`, so the two forms cannot drift; the packing is lossless because every channel is an eight-bit integer. **What it does not reach**: the eight-bit arm and `by: "series"`, exactly as I40 does not; and a caller that unpacks to build a hex per write has paid what the form exists to avoid, which C12 I132 is the rule about (→ I40, C12 I132, F1171).
- **I43** — *(R-THM-002)* **A theme that promises more than the common floor declares the floor it promises, and is held to it on the same path as every other floor.** `ThemeTokens.floor` is optional; absent means *this theme promises the common floor*, which nine of the ten shipped themes do. `hcDark` and `hcLight` declare `7`, and `validateHighContrast` holds every **meaning** ink of a declaring theme to `max(declared, floorFor(slot))` against every surface the theme paints text on, honouring composition like every other floor. **It raises and never lowers**: a theme declaring `2` would otherwise weaken `muted`'s own 2.5, which is a promise that promises less and the shape A03 §2 calls vacuous.
...
- **I48** — *(R-THM-001, R-THM-005, R-THM-004)* **The resolver takes the ground a run lands on, and `inkOn` is its composition step.** `resolve(ref, theme, caps, on?)` and `resolveTone(tone, theme, caps, on?)` answer *what ink does this slot take on that ground*; `on` absent is the page, which is every existing call site unchanged. **The composition is one function and it is the gate's own** — the band first (R-THM-005), then the theme's `(ground, ref)` composition (R-THM-001), then the flat slot — so the painter and `validateTokens` read one answer rather than two implementations of one rule. **This is what makes every floor in this component a claim about a drawn frame.** Before it, `inkOn` was called from `contrast.ts` and nowhere else and `resolve` had no ground to ask about, so **74 of 100 tone × surface pairs across the five themes that compose were an ink the gate checked and the painter never emitted** — R-THM-005's *one ink per band* verified against values the terminal never received, which is A03 §2's vacuity class wearing a passing gate (F1240). **The ladder is unchanged, and the ground binds on the two rungs that carry the theme's own values**: at 24-bit the composed hex is the hex; at 8-bit the composed inks are quantised **as a set on that ground**, per `(theme, palette, ground)`, because rank order and distinctness are properties of a set and a per-slot neighbour can see neither (§3). **At 4-bit and 1-bit the ground is inert, and that is a limit rather than an omission**: 4-bit is a curated sixteen-entry map the theme authored per ref and composed no second time, and at 1-bit no colour is emitted at all (I2), so there is no ink for a ground to change — a floor is a 24-bit claim and those two rungs already depart from the hexes wholesale. **What it does not reach, stated because an unrecorded limit reads as strength**: a ground reaches the resolver only where a painter passes it, so a renderer that washes a region and forgets to say so paints the page's ink on it. The seam is `RunContext.on` and the row that watches it is the composition frame, which reports each run's ink **beside the ground it stands on** rather than asserting one in isolation.
- **I49** — *(R-THM-001, R-THM-004)* **A ground's pairing is every tone the design paints on it, and a composed value is a declaration that the tone lands there. Its stated blind spot is that a pairing derived from a record cannot see that record go missing** — the pair and its ink vanish together — which is why the second clause below is checked against the registry and not against the token set. `SELECTION_SLOTS` is derived from the registry's `(theme, ground, tone)` compositions rather than authored, so a tone the design repaints on a ground enters that ground's floor check on the commit that adds it, and a tone it never paints there is never measured against it — §4a's scope rule with its premise read off the design instead of off this tree. **Every composition the registry declares is carried at the value it declares**, over every selector in a rule and not the first: a rule may name several tones in one selector list, and a value dropped by the generator is a value no floor can miss, because the pairing that would measure it is derived from the same records. The two clauses are one invariant because either alone is silent — a lost value on an unmeasured pairing is exactly the state this repository shipped in seven places (§4b.1).
- **I50** — *(§074, `R-BLK-590`, I15)* **A tone's monochrome class is the design's ladder, and `identifier` is emphasised.** `ok · warn · error · accent · identifier` are `emphasised`, `default · info · meta` are `normal`, `dim · muted` are `deemphasised` — §074's own three lines, over the `tone` palette of every theme. **`identifier` shipped as `normal` in all three source tables**, so at one bit it was indistinguishable from `default`, `info` and `meta`: the tone that names *a thing you can type* had no carrier at the rung where the colour is gone, which is the one rung §074 is written about. **The syntax palette is not this ladder and keeps its own** — §074 says so in the same breath, *syntax roles resolve first: `syntax.keyword` → bold; every other syntax slot → plain* — so a generic `ok` or `meta` alias never overrides a syntax slot's class, and the two tables are compared separately.
- **I51** — *(§073, `R-THM-001`, I21, I32)* **`pick` and `pickInk` are a matched pair, declared together and measured against each other, and they are the third surface to ship with neither.** The registry assigns both in every one of the ten themes (`bg-pick`, `c-pickInk`), `from-registry.mjs` collects every `.bg-X` and every `.c-XInk` generically, so the values have been in `tokens.generated.ts` since the port — **undeclared in `ThemeTokens` and read by nothing**. That is exactly the state `meterFill`'s own note records for itself and `focusGround`, and it is the third instance, so it is worth stating as a shape rather than fixed a third time in silence: **a generator that projects more slots than the type declares ships values no floor measures and no renderer can reach**, and the tell is a slot whose only occurrences in `src/` are generated keys.

  **The pair is `errorGround`/`errorInk`'s shape and is checked by `errorTagPairs`' mechanism, not folded into `validateDiffSurfaces`.** Both sides come from `surfaces`, so both are read from `surfaces` — that function takes its value from `tokens.palettes[palette].slots[slot]` and `continue`s when it finds nothing, so a pair whose foreground lives in `surfaces` would be skipped **in silence**, which is a check that cannot fire dressed as one that passes (A03 §2). It was written the first way once already and caught there.

  **At the meaning floor**, because the chosen affordance reads *this is the one* rather than decorating it, and `pickInk` has no slot it could borrow: the registry authors it as `#000000` on a light ground in some themes and `#ffffff` on a dark one in others, which is a value chosen *for* the ground and meaningless away from it.
...
38. **An ink is resolved against the ground it lands on, and the gate and the painter share the resolution** (I48, R-THM-005, F1240). The composition frames drew row 1 of §4k.2 and its third clause was false: a focused or selected row was repainted in one ink, so `failed` kept its glyph and its word and lost its tone — and retiring that rule alone would have painted the flat ink on a band, which is the value the gate already measures as wrong there. Both halves are one change: `resolve` gains the ground, `inkOn` becomes its composition step, and C11 I14's drop-to-one-ink rule retires in the same commit.
...
40. **The weight ladder is the design's and one tone was off it** (I50, §074, `R-BLK-590`). `classes` has existed since I15 and nothing has ever compared it to anything: the three source tables each declare ten entries by hand, and a validator that only refuses a *missing* table is satisfied by a wrong one. `identifier` read `normal` in `tokens-dark.ts`, `tokens-light.ts` and `tokens-high-contrast.ts` alike, which is all ten themes through `classesOf`. **A frame is what found it**: §079's table drawn at 1 bit is nothing but the weight column, and five tones were bold where the fixture names six.
41. **A ground and its own ink land together, and `pick` is the third that did not** (I51, §073, `R-THM-001`). The registry has assigned `bg-pick` and `c-pickInk` in all ten themes since the port, and the generator — which collects every `.bg-` slot rather than a named list — carried both into `tokens.generated.ts`, where `ThemeTokens` declared neither and no renderer could name them. `focusGround` and `meterFill` were each found the same way and fixed one at a time; the third makes it a shape, and the tell is a slot whose only occurrences in `src/` are the generated keys.
42. **Every state axis the registry carries is declared, in both directions** (I52, §4k.4, M11). The registry is the population and §4k.3 and §4k.4 are the declaration; an axis with neither a rung nor a carrier fails the build, and a rung or carrier naming an axis the registry has dropped fails it too. Equality rather than a subset, because a subset lets a dead entry outlive its subject.
43. **The hue palette is per theme, three tiers, against the registry by equality in both directions** (I53, §070, §093, M2). Ten hues × ten themes × `c-h`, `bg-h`, `c-hi`. The assertion is per theme rather than once, because the ground varies across all ten and the light themes lift the ink — an assertion taken once over a shared vocabulary is satisfied by whichever theme was read last, which is what the generator's collectors would have produced had anything emitted them.
44. **The ink on a hue's band is derived, not authored** (I54, §070). It is the higher-contrast of black and white against that hue's ground, and it is asserted as the computation rather than copied from the registry — which is what makes the registry's own 100 answers checkable instead of merely transcribed.
45. **A band's ground is resolved by the band's name** (I45, `R-THM-005`). `bandInk` is keyed by surface name, so the ground is `surfaces[name]` — not a conditional over the two names that happen to exist. A third band added to a theme is measured against its own ground or the gate is reporting about a ground it does not stand on.
...
- **I52** — *(§4k.4, M11, `R-STA-001`, `R-COR-003`)* **Every state axis the registry carries is declared here — with a precedence rung in §4k.3, or a carrier in §4k.4's table, or both — and an axis with neither is a build failure.** The registry's `stateAxes` is the population and this section is the declaration, which is the direction that cannot go stale: an axis added to the design arrives in `calcium-registry.json` and the check fails until something here says how the fact is carried. **The other direction is checked too and is the one that catches a rot rather than an addition** — a rung or a carrier row naming an axis the registry does not have is a declaration for a fact that no longer exists, which reads exactly like coverage. Both are equality against the registry rather than a subset test, for the reason every exemption list in this repository is compared by equality: a subset lets a dead entry outlive its subject.
- **I53** — *(§070, §093, M2, `R-THM-001`, I52's shape)* **The ten agent hues are a palette of three tiers, each resolved PER THEME from the registry by equality in both directions — and the generator that collected them emitted nothing and called them theme-independent.** §070 names ten hues and §093 orders them: *blue orange cyan pink lime violet yellow green red purple*, by **perceptual separation** and not spectrally, because the first assignment was *red, orange, yellow, lime, green — five identities a deuteranope cannot separate*, and *the first four are separable under every common deficiency, which is where a session with three subagents lands*. **The values were never missing**: `calcium-registry.json` carries **300 hue tokens** — ten hues × ten themes × three tiers — as `themeRules`, which is why a search for *a record carrying all ten* found none. The three tiers are not one thing repeated: `c-h-*` is the hue's **ink**, `bg-h-*` is its **ground**, and `c-hi-*` is the ink drawn **on** that ground, with `font-weight:600`. **Theme-independence is false at every tier**, measured 2026-09-24: `c-h-blue` is `#3b82f6` in eight themes and `#4e8ef6`/`#4e8df5` in `light` and `paper`, where a mid blue needs lifting off a light ground; `bg-h-blue` takes **nine distinct values across ten themes**; and `c-hi-*` is `#000000` or `#ffffff` by theme, which is the readable ink for that theme's ground and cannot be shared. **The generator states the opposite and acts on it**: `tools/theme/from-registry.mjs` keys `hues[name]` and `hueGrounds[name]` by hue alone, under a comment saying *every theme declares the same `h-blue`, so emitting them per theme would write one vocabulary out ten times* — so ten themes' values overwrite each other and the last one read would win. It has never shipped that, because **the destination does not exist**: the same comment says *they land once, as `HUES`*, and there is no `HUES` anywhere in the tree. A deferral naming its own satisfier, with nothing watching whether it arrived. The comparison is by equality both ways per theme, for I52's reason: a subset lets a dead hue outlive its subject.
- **I54** — *(§070, `R-THM-001`, I53's third tier)* **The `c-hi` tier is a COMPUTATION and is asserted as one: the ink on a hue's band is whichever of black and white has the greater contrast against that band.** §070 states it — *the ink on each is chosen by CONTRAST against that hue, so yellow takes black and blue takes white without anyone deciding per theme* — and the registry holds to it **100 times out of 100**, measured 2026-09-24 over ten hues × ten themes. A tier that can be computed should not be asserted by copying: equality against the registry says the projection is faithful, and says nothing about whether the registry is right. This says both. **Two figures carry it.** White wins **seven** times — `purple` in `dark`, `ink`, `warm`, `viol`, `mono` and `hcDark`, and `blue` in `hcDark` alone — so a build that hard-coded black is right 93 times and a row without that count passes it. And the winning ink clears **4.5:1 in all 100**, worst 4.62 at `purple`/`nord` and `blue`/`hcDark`, where the two candidates sit within 2% of each other and the rule is a tie-break; the floor is asserted on that **minimum** rather than on a count, because a ground drifting darker loses the floor before it loses the tie and nothing else would say so. **The phrase *without anyone deciding per theme* is right about the mechanism and loose about the axis**: nobody decides, and the answer still moves per theme, because the ground is derived against each theme's background and the winner flips when it does. **And §070's illustration is wrong where its rule is right.** *Blue takes white* is true in `hcDark` and in no other theme — `bg-h-blue` at `dark` is `#3776dc`, 4.79 on black against 4.39 on white, so black wins. No ruling is owed on it: the rule is verified over the whole population, the example is the only wrong part, and nothing in the tree reads the example. Recorded rather than parked, and T2.54 asserts both readings so the correction cannot be lost.
- **I55** — *(§070, `R-THM-001`, I53's ink tier)* **The hue's ink is a palette named `hue`, emitted from the same record as the other two tiers, so the one resolver every block goes through can name a hue.** I53 landed the values and left them unreachable: measured 2026-09-24 against `dist/`, `t.tokens.hues` carried all ten while `resolve('hue.blue', theme, {colourDepth:24})` returned `{}`, because `ColourRef` is `palette.slot` and the theme's palettes were `tone categorical syntax spectrum`. **A block names a palette slot and C10 resolves it** — that is the rule the whole tree is built on — and these values sat beside the palettes rather than in them. Carried and unreadable, which is what I53 was written about: it was true of the generator and it became true of the resolver one layer out. **Only the ink tier becomes a palette, and the reason is what a palette is.** A palette is named slots with one hex each, resolved against a ground; the ground tier IS a ground and the `on` tier is the ink for a label drawn on it, and neither is a slot a block asks for by name. So `hues` stays the record where the three tiers sit together and `palettes.hue` is projected from it in the same template — **not collected a second time**, which is I53's own lesson: two collectors for one fact is how the fact ends up with two values. T2.56 asserts the two agree in the shipped file, so the single source survives a hand edit as well as a regeneration. **`carries: "decoration"` and `monochrome: "foreground"`, and §070 settles both in its own first line**: *the tones mean something; this is you choosing what your terminal looks like*, and *chrome only — a painted label is furniture*. A decoration palette owes no typographic class per slot and may degrade to nothing at 1-bit, which is what `foreground` means here; a meaning palette could not, because ten identities have no four classes to fall to. **So no carrier is lost at 1-bit, and that is a consequence of the design's own classification rather than a convenience.** **The 4-bit rung is PARKED (21), and the first draft of this clause was wrong about why.** It said the map was an enhancement because decoration may degrade to nothing — and the tree already carries a gate saying otherwise, *every slot and surface has a 4-bit entry, so nothing silently loses colour at depth 4*, which went red on the first run. **Then the obvious fix was measured and is worse than the gap.** `nearestAnsi16` over the ten inks returns **six distinct indices in `dark` and seven in `light`**: `blue` and `green` both land on 6, and `orange`, `lime` and `yellow` all on 11. A mechanical map satisfies the gate and destroys the ten identities it exists to keep — which is why `DARK_FOUR_BIT` is curated in the first place. A curated hue map is ten visible choices the registry does not carry, so it is parked rather than invented. **And 4-bit is the ONLY rung that loses them, which is a fact about the mechanism rather than about low depth.** At 8-bit the ten resolve to **ten distinct `ansi256` indices in every one of the ten themes**, because `quantiseSet` quantises a palette **as a set** (§3) and so can hold rank order and distinctness, where a per-slot neighbour sees neither. Sixteen colours leave no room for a set to spread into; 256 do. So the park is about one rung, not about degradation. **The exemption is an amendment and not a weakening**, and it is written to stay one: the gate's skip list is compared **by equality** against the palettes each theme carries, so a dead entry cannot outlive its subject and a third cannot be added in silence; I55 carries the measurement; and T2.56 asserts the 4-bit answer is `NO_STYLE` rather than a wrong colour, so what the rung does is stated rather than left to be discovered.
- **I56** — *(§4k.5, `R-COR-003`, M11, I52's shape)* **Every state axis the registry carries has a carrier row, traced to a field and a renderer — and `tone` with `ground` is one carrier written twice.** `R-COR-003` is the largest rule in the contract and it was **enumerated over no population**: I52 held the axes to the registry by equality, and nothing said which two carriers hold each one, so *tone and ground are not two carriers, because both die at 1-bit* had nothing to be wrong about. **The test of independence is the 1-bit rung and nothing else**: `mark`, `word`, `weight`, `position` and `inverse` survive it, `tone` and `ground` do not. A pair drawn from the second set is one carrier written twice. **Each row is traced, and the tracing corrected two readings.** `CallState` looked like one field where the design has two axes — it is a *derived* value, and `state` and `outcome` are separate fields, so lifecycle and outcome are two rows with two carriers each. And the obvious instrument for the survey measured the prose rather than the tree: `readonly` 330 for the TypeScript keyword, `live` 119 and `open` 85 for ordinary English, `unchosen` 0 although `choice` ships a renderer. **Two axes carry ONE carrier and the tree declares both, which is why they are exceptions and not violations.** `paint.ts:250` reads *`inverse` because the ground is selection's only carrier; focus has `▸`* — `R-SEL-006` puts the pair across two facts rather than within one, and a second carrier for selection would be a mark, the cell C11 I14 forbids. `inverse` is the same channel at a rung where colour is gone, not a second channel. Choice is the same shape from the other side (`controls.ts:32`): the mark carries *chosen* and the wash carries *focused*, and the mark is the one that survives. **Four axes have no subject — `resolution`, `pointer`, `availability`, `freshness` — and a gate demanding two carriers of them would assert over an empty set**, which is A03 §2's class manufactured by the gate rather than found by it. They are named with their reasons, so the row goes red the day a field arrives rather than staying quiet because nothing could be wrong.
- **I57** — *(`MILESTONES.md`, M4–M16)* **Each MR's seam has a row naming a symbol and a file, and the row is a REVERT DETECTOR rather than a completeness proof.** The milestones file recorded only what an earlier MR **owed** a later one; nothing recorded that an MR had **landed**, so *is M5 finished?* was answerable only by reading the tree again — asked eight times in one session, it was re-derived eight times, and a survey that must be redone is a belief rather than a record. **What a row asserts is the seam's existence and nothing more.** `Verdict` being in `router/types.ts` does not say M5 is correct; that is what M5's own invariants and rows are for, and they number in the hundreds. It says the seam M5 introduced is **still there**, so a revert, a rename or a merge that drops it goes red here instead of being discovered by the next survey. **Stating the limit is the point**: a row that claimed completeness would be the kind of green that reads as thoroughness, which is what the mutation pass exists to distrust. **Two of this row's own drafts were wrong, and both were caught by the pass rather than by reading.** The section was located by a prefix — `## The landing record` matches `## The landing recordx` — so the control that renames the heading **survived**, which is the identical defect §4k.5's gate had and whose lesson was written into §4k.5 an hour earlier: **a correction stops at its own sentence unless the next instance is looked for.** And the population was a floor, `>= 15` over sixteen rows, so deleting a row left fifteen that all still resolved — true and useless. **A count is not a population**: the claim is that every MR from 4 to 16 has a seam recorded, so losing one is losing an MR. **And the control is load-bearing rather than decorative**, because a table of fifteen true statements passes exactly like a checker that reads nothing: T2.58 fabricates both ways a row can be wrong — a file that does not exist, and `PushedSurface`, the name M9 retired, in the file that still holds its replacement.

  **`mono` is the row worth stating on its own, and it is the one that could be read as a projection defect.** Measured 2026-09-24 over the emitted tokens: every one of `mono`'s **25 surfaces** and **10 tones** is grey — `r === g === b` in each — and **not one of its ten hues is**, its `blue` ink being `#3b82f6`, byte-identical to `dark`'s. A greyscale theme carrying ten saturated bands looks at first like the hue block being emitted unchanged for every theme, and it is not: `mono`'s hue **grounds** are distinct from every other theme's (`#3775db` against `dark`'s `#3776dc`), so they were derived against `mono`'s own background one at a time. The registry is normative and says so, and the reason holds on its own terms — **ten identities cannot degrade to one grey.** Every other fact `mono` carries has a second carrier once colour is spent; an agent's identity is the hue and nothing else, so identity is the single axis the greyscale theme does not flatten. T2.53 asserts both halves, because the claim that is easy to "fix" is the one that needs a row.

  **What the projection costs, stated because the type is optional.** `ThemeTokens.hues` is `?`, so the three hand-written lender files (`tokens-dark.ts`, `tokens-light.ts`, `tokens-high-contrast.ts`) need no edit and a consumer must narrow. The type system therefore holds nothing here, and the equality row is what stands in its place — which is why T2.53 asserts presence per theme before it asserts a value. The generator's own guard is the other half: `huesBlock` throws on a hue the registry orders and a theme does not carry, and on a tier missing from one it does, because **a hue with two of its three tiers is a band painted with a guessed ink** and it would project as a record that looks complete to every reader but the one that needs the missing tier.

  **This is the gate two records already named as their watcher and neither could see.** §4k.4 says *what M11 owes them is the declaration, since a fact with no declared carrier is a build failure under M11's gate*, and `MILESTONES.md`'s M11 row gives *M11's own gate, on the day a fact arrives with no declaration* as what goes red. Measured before it was built: **nothing in `src/`, `tools/` or `test/` read `stateAxes` at all**, so the population was never enumerated and neither watcher existed. A deferral whose watcher is imaginary is the strongest form of the class this repository names about itself, because the citation reads as coverage.

  **What it does not check, stated because an unrecorded limit reads as strength.** It asks whether each axis is declared, not whether the declaration is *right*, and not whether the carriers named survive 1-bit — the two-carrier property is asserted at its sites (`R-COR-003`) and over the head mark by SS64, and a rule quantified over prose carriers would be parsing English. So this closes the *absence* half of M11's gate and names the other half as still owed.
...
- **T1.45** (I49, §4b.1): the selection pairing is derived from the theme's own compositions, so it grows with them — `nord` 22 pairs, `light` 9, `hcDark` 1 — and every derived pairing clears its floor through `inkOn`. **Green before the seven values land and green after**, on purpose: a pair derived from a composition disappears with it, so this row is a scope for the next failure and T1.44 is what sees this one.
...
- **T1.42** (I40, F1150): **the LUT's whole domain, exactly** — for every eight-bit channel `c` in `0..255` and 65 values of `k`, `shadeColour({rgb: c, c, c}, k).hex` equals `rgbHex(shadeRgb(c, c, c, k))`, with `shadeColour` kept on `overChannels`' direct `toLinear` as an implementation independent of the table; and, as the integration, over every shipped map × 64 values of `t` × 8 of `k`, `shadeColour(continuousColour(map, t, caps24), k).hex` equals `rgbHex(shadeRgb(...sampleRgb(map, t), k))` and `sample(map, t)` equals `rgbHex(sampleRgb(map, t))`. **The domain is the 256 channels and not the `t` axis**: `t` only feeds `sampleRgb`, which `sample` delegates to, so a wide `t` sweep re-tests the same source against itself ten million times and times out under load (measured at 23 s against a 30 s ceiling) while adding no coverage — a control written as a magnitude measures the machine. A rounding that drifts one channel by one is caught by the channel sweep, at every channel.
- **T1.43** (I42, F1171): over I40's own sweep — every eight-bit channel × 65 `k` on grey and the mixed-channel grid, and every map × 64 `t` × 8 `k` — `packedHex(shadePacked(samplePacked(map, t), k))` equals `rgbHex(shadeRgb(...sampleRgb(map, t), k))` byte for byte, `k ≥ 1` included, and `samplePacked` on a non-finite `t` and on an empty map equals `sampleRgb`'s answer packed.
...
- **T2.24** (I43, roadmap 24, R-THM-002): **both** high-contrast themes declare `floor: 7`, `validateTokens` accepts them at it, and the sweep names the tightest pair — `hcLight`'s is exactly **7.00**, because it was composed to. `muted` is named separately because a passing sweep does not say which slot was in question; the three greys stay ordered, quieter than `dim`, quieter than `default`; the ten tones on `bgElev` stay ten distinct colours, because darkening four inks toward a floor is the edit that converges a palette and a ratio check cannot see two slots arriving at one value; and no other shipped theme declares a floor, so the field is not becoming a second way of writing the ordinary one. **The row no longer *is* the promise**: it used to be the only thing standing between the name and a claim, asserted against the tokens as they stood, and that is how `hcLight` reached 6.55 : 1 on `bgElev` with every gate green.
...
- **T2.14c** (I22, §4a, §4b, §4d, R-THM-002): `surfaces` has exactly **twenty-five** entries, listed by name, and `diffAddStrong` and `diffRemoveStrong` are not among them. The withdrawn pair asserted absent rather than merely unmentioned: a spec that measured something out and a token file that quietly kept it is the drift this suite exists to stop. **The count is the row's whole subject**, so it moves whenever a surface does — it was ten before R-THM-001 assigned fifteen more, and `.bg-error` is not a twenty-sixth because it is `.bg-errorGround` under a second name.
- **T2.14e** (I32, §4d, R-THM-002): **the tag's ground and `tone.error` are two values, each held to the pair it forms.** The equality was I32's answer to two hex literals in two files drifting for five rounds, and what it cost was the floor — one value serving as ink on the page and as a ground behind white text cannot clear 4.5 in both directions. R-THM-001 splits them and the drift hazard is answered differently: each is measured against what it actually sits on, so a change to either is caught by the check that owns it. Two themes still have them equal — `hcDark` and `hcLight`, where the tone is already loud enough to be a ground — and that is asserted **as a count**, so a consequence of the palette cannot silently become a rule in ten again.
...
- **T2.39** (I44, §2): every curated table is the value it was curated as — seventeen pins over the three 4-bit maps, `MUST_STAY_DISTINCT`, the CVD calibrating set and its visions, the colormaps, the categorical refs, the required slots, the spinner sets, both substitution tables, the glyph tokens, and `glyphs()` and the bar alphabets each at the full and ASCII rungs — the bar alphabets at **both** because `barStyle` takes the capabilities first, and a first draft that passed it a name pinned nine copies of one default — each compared by equality against its pinned canonical form, table by table so the failure names the table, and entry by entry inside it so the failure names the slot and its old and new values. `COLORMAPS`' samples are pinned by a per-map digest and everything else whole, for the reason I44 records.
- **T2.39a** (I44, C10 I26): the entries whose docblocks argue for them, written out — `HIGH_CONTRAST_FOUR_BIT`'s `syntax.type` on plain yellow and the dark map differing there, the five slots `MUST_STAY_DISTINCT` names, and `hcDark` lending the curated map by reference where `hcLight` takes the light one. The row that carries the reasoning a digest cannot.
- **T2.40** (I44): every exported frozen table in the curated modules is pinned or declared derived, **compared by equality in both directions** — a new table pinned by neither fails, and a derived-table exemption whose table is gone fails. The half that makes this a gate rather than a list.
- **T2.41** (C09 I45, C02 I9): the ASCII set is also the wide set, asserted so it is not read as a dropped input. Two capability records giving an identical set is ordinarily that tell; here box drawing is `East_Asian_Width=Ambiguous` throughout, the ASCII rung is the only set with no ambiguous member, and a third set of narrow substitutes was considered and refused.
- **T2.42** (I45, R-THM-005): the four band contrasts each fire on their own — one value moved per row, asserted on the **path** the failure names rather than on a count, because the message is the reason and a count would pass for any four. The shipped set clears all four, so a sweep over it agrees with a gate that checks none; these rows are what distinguishes the two. The focus band's two constraints are shown failing in **opposite directions**, which is the correction the gate made to this row's first draft: sliding the band onto the page moves it away from selection, and no single move breaks both.
- **T2.43** (I45, R-THM-005): every meaning ink on a band resolves to the band's ink — all nineteen slots of both palettes on both bands in both high-contrast themes, **including slots no enumeration named**, which is the property an enumeration cannot have and the one that makes the promise hold by construction.
- **T2.44** (I46, R-THM-004): the floor's scope is three named surfaces — `bg`, `bgElev`, `focusGround` — asserted as a **membership** and not as a sweep, because every ratio row in the file passes over a narrower scope and that is exactly how `focusGround` went unmeasured. `spectrum`'s exclusion is asserted with them: 22 of its slots across four themes sit below 4.5 on `focusGround` and every one is a pairing nothing draws, because R-FOC-004 puts a plot's focus on its border or axes rather than painting its data. An exclusion stated in a comment is an exclusion nothing checks. **Amended by I60:** `textSurfaces` is still those three, and `bgDeep` is no longer asserted text-free — it carries `tone.meta` in `textGrounds`.
...
- **T2.49** (I48, R-THM-001, R-THM-005, F1240): **the painter's ink and the gate's ink are one value**, over every meaning slot × every ground the theme paints text on × all ten themes at 24-bit — `resolve(ref, theme, caps, ground).colour.hex` equals `inkOn(tokens, ref, ground)`. Asserted as an equality between the two functions rather than against a table of hexes, because a table is a third record and the defect was two records disagreeing. The count of pairs is asserted with it, so a ground or a slot leaving the sweep is a red row rather than a smaller silent one.
- **T2.50** (I50, §074, `R-BLK-590`): every theme's `tone` palette × the three classes → the partition is §074's exactly, by equality in both directions, so a tone added without a class and a class given to a tone the design puts elsewhere both fail. The `syntax` palette is asserted **separately and not merged**, on §074's own sentence that syntax resolves first: `keyword` emphasised, every other slot plain. Red on the tree it was written against — `identifier` was `normal` in all three source tables and therefore in all ten themes.
- **T2.52** (I52, §4k.4, M11, `R-STA-001`): the registry's `stateAxes` and this section's declarations are **equal as sets** — every axis has a precedence rung in §4k.3 or a carrier row in §4k.4, and every rung and carrier names an axis the registry still carries. Read out of `calcium-registry.json` and out of this file, so neither side is a transcription. The control is the direction that catches rot rather than addition: dropping an axis from the registry must fail as loudly as adding one, and a subset test passes the first and not the second.
- **T2.53** (I53, §070, §093, `R-THM-001`): the ten hues, three tiers, **per theme**, against `calcium-registry.json` by equality in both directions — 300 tokens, no residue in either direction. The order is asserted as a sequence and not a set, because §093's whole argument is the ordering: a set comparison passes the spectral assignment it replaced. The control is the pair of themes that lift the ink, `light` and `paper`, and the nine distinct grounds — without them *the hues are the registry's* is satisfied by one theme's vocabulary written out ten times, which is exactly what the generator's collectors would have emitted. `mono` carries a row of its own: its surfaces and tones are wholly grey and its ten hues wholly chromatic, and the pair is asserted together so that neither half reads as a defect to be tidied away.
- **T2.54** (I54, §070, `R-THM-001`): over all ten themes × ten hues, `on` equals the higher-contrast of `#000000` and `#ffffff` against `ground` — 100 pairs, computed with `ratio` rather than read from the registry. The floor is asserted on the **worst** winning contrast against `DEFAULT_FLOOR`, not on a pass count. The control is the seven white answers and the two readings of §070's own example: `blue` on `hcDark` takes white, `blue` on `dark` takes black — without them a build that hard-codes black satisfies 93 of the 100.
- **T2.56** (I55, §070, `R-THM-001`): over all ten themes, `resolve('hue.<name>', theme, {colourDepth: 24})` answers each hue's ink for all ten names — 100 resolutions through the public resolver, not a field read. The palette and `hues` are asserted **equal in both directions** in the shipped file, because they are one fact written twice and only the generator knows they share a source. At 1-bit every hue resolves to `NO_STYLE`, which is what `decoration` and `foreground` mean and is asserted rather than assumed; at 4-bit it is `NO_STYLE` too, with the reason recorded on I55. The control is a hue name the palette does not carry, which must resolve to nothing rather than to a neighbour.
- **T2.57** (I56, §4k.5, `R-COR-003`, M11): the registry's `stateAxes` and §4k.5's rows are **equal as sets**, so an axis added to the design fails until it has a row and a row naming a dropped axis fails too. Every row names a field and a renderer or says `no subject` with its reason; **no row's carriers are `tone` and `ground` together**, which is the independence rule and the one the gate exists for; and every single-carrier row cites the file that declares it, so an exception cannot be taken by writing one. The control is the fabricated violation: a row whose carriers are `tone + ground` must fail, and a row with `mark + tone` must not — without it the rule passes on a table that happens to contain no such pair today.
- **T2.58** (I57, `MILESTONES.md`, M4–M16): every row of the landing record resolves — the file exists and still contains the symbol — over at least fifteen rows with no MR claimed twice. The control is a fabricated row in both of its failing shapes: a file that does not exist, and `PushedSurface` in `surface.ts`, which is the name M9 retired and the file that holds its replacement, so the row fails on a rename as well as on a deletion.
- **T2.51** (I51, §073, `R-THM-001`): `pick` and `pickInk` resolve in all ten themes and their ratio clears the meaning floor, measured from `tokens.generated.ts` rather than from a table of hexes written here — a second record is what I49 was written about. **The premise is asserted first**: the registry declares the pair for every theme, so a theme losing it fails as a missing value rather than passing as a pair nobody measured. And the pairing is asserted to be *in* `themePairs`' output, not merely computable — `errorTagPairs`' sibling defect was a check that could not fire, and a row calling the pair function directly would pass under it.
...
- **T6.100** (I41): dropping an entry from the generated table, or editing a shipped surface without regenerating → T3.73 fails on the coverage half; making the resolver skip the table → T3.73's *never runs the DP* half fails, and the cold load is 65 ms heavier with every row green.
...
- **I59** — *(§047, C09 I110)* **`recede(theme)` is the theme a stale reading is drawn in: every palette slot resolves as `tone.dim` does, at every depth.** At 24- and 8-bit the slot takes `dim`'s hex, at 4-bit its curated index, at 1-bit the `deemphasised` class, and a composition on a ground takes the composition `tone.dim` has there. Surfaces are untouched — a ground is not content — and so is a band's ink, which already answers for every ref (R-THM-005). **Its own name**, `<name>/receded`, because `resolve` memoises on the name and a receded theme sharing one would be served the live theme's styles. One object per theme, so the frame path allocates it once.
- **I60** — *(R-THM-002, R-THM-004)* **The floor's scope is a table from each ground to the slots that land on it, and a theme that promises more holds it over the whole table.** `textGrounds(tokens)` is `textSurfaces`' three page grounds × every meaning slot, **plus** the two diff grounds × the twelve slots §4a names (`DIFF_SLOTS`), **plus** `bgDeep` × `tone.meta`. `validateHighContrast` walks that table, not `textSurfaces`. **`bgDeep`'s exclusion rested on a false premise**: *no text lands on it*, while the prompt chip paints `tone.meta` on `surface.bgDeep` (`src/shell/paint.ts`, `R-BLK-628`). Measured 2026-09-24, against the 7 : 1 both high-contrast themes declare: in `hcDark` all eleven diff pairs on `diffAdd` sit at 4.77–4.82 and all eleven on `diffRemove` at 6.08–6.14; in `hcLight` seven on `diffAdd` sit at 6.27–6.66 and nine on `diffRemove` at 5.73–6.68; `tone.meta` on `bgDeep` is 6.38 and 6.51. None of it was composed, and none of it was reported, because the scope was a list. The shortfalls are composed by I46's rule — the least that clears, hue kept — with tones in the registry and a lent palette in its token file. **And the table is derived rather than remembered**: A03 SS67 requires every ground a renderer names beside text (`tone(…, on)`, `slot(…, on)`, and every `surface.*` a renderer puts behind a run) to have a disposition in SS67's `SURFACE_ROLES`: **`text`** (a ground of `textGrounds`, and T2.61 holds that set equal to the function's), **`gated`** (a ground with its own pairs and a named check — `selection`, `pick`, `errorGround`), **`ink`** (a surface token drawn as a foreground — `errorInk`, `pickInk`, `border`), or **`excluded`** with its reason (`meterFill`, whose painted rung draws spaces). The list is driven both ways: an entry no renderer names fails too. **Dispositioned per surface, not per site**, and that is SS67's stated blind spot: the plot draws `surface.bgDeep` as a box *ink* with `tone.default` on it as a median mark, and passes because `bgDeep` is `text` while its refs name only `tone.meta`. A new text ground then fails the build instead of waiting for a fourth widening. → T2.59, T2.60, T2.61, T2.62
...
- **T2.59** (I60, R-THM-004): `textGrounds` is the page grounds × every meaning slot, the diff grounds × `DIFF_SLOTS`, and `bgDeep` × `tone.meta`, asserted as membership for every shipped theme.
- **T2.60** (I60, R-THM-002): every pair in `textGrounds` clears the declared floor in `hcDark` and `hcLight`, the ink resolved through `inkOn`, and `validateTokens` is clean for both. The control removes one composition at a time — `tone.ok` and `syntax.keyword` on `diffAdd`, `tone.meta` on `bgDeep` — and requires the validator to name that slot's path and that ground; a validator still walking `textSurfaces` alone passes the first two in silence.
- **T2.61** (I60, SS67): the `text` entries of `SURFACE_ROLES` equal the names `textGrounds` returns, and every `gated` entry names a function `contrast.ts` exports — where every gate lives — the scan is `.mjs` and cannot import the table it guards, so the copy is compared by equality.
- **T2.62** (I60, SS67): SS67 is clean on the tree; a renderer naming an undispositioned surface is reported at its file and line; an entry no renderer names is reported; and a corpus that names no `surface.diffAdd` reports the control alone.
