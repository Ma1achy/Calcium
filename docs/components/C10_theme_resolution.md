...
- **T2.60** (I60, R-THM-002): every pair in `textGrounds` clears the declared floor in `hcDark` and `hcLight`, the ink resolved through `inkOn`; the control is the same walk over `textSurfaces` alone, which the shipped tokens passed before the compositions.
