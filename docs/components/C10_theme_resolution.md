...
- **I44** — **A curated table is pinned by its exact value, and the set of curated tables is driven from the modules' own exports.** A table in `src/presentation/theme/` or in the glyph alphabets that was hand-chosen rather than computed is asserted **by value** — a digest over its canonical form, so any legal-but-different substitution fails, beside the entries whose docblocks argue for them written out, so a failed digest arrives with its reasoning rather than only with a hash. The set is **compared by equality against every exported frozen table in those modules**: a table pinned by neither a value nor a stated reason for being derived is a failure, and a stated reason whose table no longer exists is a failure too. **What it does not reach**, stated because an unrecorded limit reads as strength: a table private to its module is pinned through the projection that ships it — `glyphs(caps)`, `barStyle(name)` — which is the better subject anyway, being what a caller receives; and a value that is *both* derived and load-bearing, like the generated token sets, is pinned by the instrument that owns it and named as derived here rather than pinned twice.
...
- **T2.39** (I44, §2): every curated table is the value it was curated as — seventeen pins over the three 4-bit maps, `MUST_STAY_DISTINCT`, the CVD calibrating set and its visions, the colormaps, the categorical refs, the required slots, the spinner sets, both substitution tables, the glyph tokens, `glyphs()` at the full and ASCII rungs, and the bar styles — each asserted as a digest over its canonical form **and** as an entry count, because a dropped entry and an edited one read the same from a hash alone.
