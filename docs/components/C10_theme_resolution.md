...
- **T1.42** (I40, F1150): over every shipped map × 1 024 values of `t` × 64 values of `k`, `shadeColour(continuousColour(map, t, caps24), k).hex` equals `rgbHex(shadeRgb(...sampleRgb(map, t), k))`, and `sample(map, t)` equals `rgbHex(sampleRgb(map, t))` — the two numeric forms proved bit-identical to the hex ones over the whole domain rather than at a handful of points, because a rounding that drifts one channel by one at one `t` is a golden mover no smaller sweep would catch.
