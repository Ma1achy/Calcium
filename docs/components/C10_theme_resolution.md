...
| palette | `categorical.c((i mod 8) + 1)` resolved as a slot | as a slot | as a slot — the curated map separates them at 4-bit, which is `CATEGORY_COLOUR_FLOOR` | `undefined` |
...
`refOf(index)` were C12's (`plot/marks.ts`), and C09's resolver cannot import C12 without closing
a cycle inside L1 (A02 §1; C12 imports C09's paint helpers). They live in
`theme/categorical.ts` now — this component owns slots — and `marks.ts` re-exports them so no
plot call site changes. One copy, because F382 is the measured record of what two copies of this
function did: a legend swatch in one colour and the line it named in another.
...
- **I37** — **`CATEGORY_REFS` and `refOf` live in `theme/categorical.ts`, one copy, re-exported by C12's `marks.ts`; a palette ramp cycles them and no data palette.** The move exists because C09 cannot import C12 without a cycle (A02 §1), and it is a move and not a copy because F382 measured two copies disagreeing. C10 I16's one categorical palette is what makes `palette` a fill with no name (C04 I106, F837).
...
- **T2.30** (I37): `refOf` and `CATEGORY_REFS` are exported from `theme/categorical.ts` and `plot/marks.ts` exports the **same** references — identity, not equality; `blocks/**` imports nothing from `plot/**` after the ramp resolver lands (MG1's acyclicity over the L1 graph, asserted by the module walk and not by grep).
...
- **T6.94** (I37): a second `refOf` inside `blocks/ramp.ts` → T2.30's identity row fails; importing it from `plot/marks.ts` instead → MG1 names the cycle.
