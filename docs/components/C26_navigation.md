...
- **T4.32** (I25, §7a, C04 I124): a tape in a session slides and stays — walking the current to the far end and back leaves the window where the walk put it, where a window recomputed from the head each frame snaps back the moment the current comes near the start.
