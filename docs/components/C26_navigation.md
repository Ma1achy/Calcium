...
- **T2.34** (I11): resolution is a pull — `elementsOfEntry` is a function of its arguments, so the same entry resolved after its document changed reports the new elements with nothing having been notified, and there is no registration or subscription API to call. The failure this forbids is a cached list surviving a patch: a click landing on the element that *used* to be at that row.
- **T2.35** (I15): **a watch, not a coverage row.** §4d records the outcome of the check §4 committed to: *not one value of either vocabulary has an inhabitant in the tree*, nothing is adopted and no field is added — so `ArrowPolicy` and `EscapePolicy` appear in no source file and I15's resolution shape is a mechanism looking for a policy to resolve. This row asserts the absence and fails the day either lands, which is when I15 owes a real row. Beside it, the half that is decided: `NavElement` carries no `arrow` and no `escape` field, which is what *nothing is adopted* means where a reader can check it.
