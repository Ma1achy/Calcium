...
**`design/language/` — the design language.** The source of truth for **appearance,
interaction and keyboard navigation**. Its normative form is
[`calcium-registry.json`](design/language/calcium-registry.json) — rules with permanent
`R-XXX-NNN` ids, of which only `status: current` is normative — and
`calcium-design-language-revised.html` is a *projection* of it, generated by
`build-calcium.mjs` and never edited by hand. `make design` regenerates both it and the
keymap page; `make design-check` holds them to the registry and holds released rules to
`released-baseline.json`, and `make enforce` depends on it.

**Where a C-spec and the registry disagree, the registry wins and the spec is amended** —
including on mechanism, not only on appearance. [`AUTHORITY.md`](design/language/AUTHORITY.md)
carries the three things that are *not* overridden: the measurement invariant
(`measure(block, width)` equals the rows rendered; appearance animates, geometry never
does), A02's layering rules, and `make enforce` itself.
