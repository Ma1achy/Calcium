...
- **R5.9** (R4.7): **Retired with R4.7** (F1209). It read: *removing the launcher's `prepareLaunch()` call, or moving it below the app import → R4.7's row fails on the count*. Both the call and the count are gone. The argument it carried — that a startup claim is measured at the artefact and not read off the source — is R5.8's as well, and R5.8 still holds it.
