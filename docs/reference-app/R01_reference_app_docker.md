...
- **R4.7**: the launcher calls `prepareLaunch()` from `@fmx/calcium/launch` between the compile cache and the app import, so the loader hands Ink one module instead of es-toolkit's compat barrel (C24 I37, F1192). Measured on a container-local copy, six interleaved pairs of a cold import: 235 → 208 ms median without the compile cache, 205 → 162 with it, six of six each way. The row spawns the launcher under a counting resolve hook of its own and reads the count of `es-toolkit/` resolutions back: under ten as shipped, over a thousand with the call removed. `plots-tui` carries the same line by the same rule; `svc-tui` in `minimal` carries neither this nor R4.6, because it is the stranger's example (R4.4) and stays two lines.
...
- **R5.9** (R4.7): removing the launcher's `prepareLaunch()` call, or moving it below the app import → R4.7's row fails on the count — the barrel's thirteen hundred modules are resolved in the launcher's process — while the app starts and every other row stays green, which is why the row counts resolutions and does not read the source.
