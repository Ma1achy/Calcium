...
**It restates nothing.** The frame is S01, the welcome is S02, construction and shutdown are C22 §3 and §8, terminal restoration is C01, identity is C22 §7. An earlier draft of this document described all of them — written before any of them existed — and by the time they did, it was five documents' worth of stale duplication. That is the same drift that removed A02's interface section and dropped B02 entirely. **Five references to B02 outlived it**, and they were found by reading C23's — which cited it as "the narrative" — and then grepping for the rest: a row in `TUI_SPEC_MAP` §Behaviours listing it among documents that exist, M-T3's contents, the build-order diagram, and two out-of-scope rows pointing at it for the narrative of execution. This paragraph's own consequence arriving, one document later: a document can be dropped and the citations into it are not dropped with it, so the drop is not done when the file is gone. The map recorded the drop in one place and contradicted it in three.
