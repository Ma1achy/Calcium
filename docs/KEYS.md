...
| key | profile | child | overlay | copyMode | pushedView | interaction | prompt | liveBlock | global |
|---|---|---|---|---|---|---|---|---|---|
| `+` | both |  |  |  |  |  |  | dollyIn |  |
...
| `escape` † | both |  | dismiss | exitCopyMode | viewPop |  |  | focusPrompt |  |
| `m+f` | both |  |  |  |  |  | wordRight |  |  |
| `f1` | both |  |  |  |  |  |  |  | helpKeymap |
| `g` | both |  |  |  | viewTop |  |  |  |  |
| `c+h` | both |  |  |  |  |  | backspace |  |  |
...
| `p` | both |  |  |  | viewPrevHunk |  |  |  |  |
| `pagedown` † | both |  |  |  | viewPageDown |  |  | blockPageDown | scrollPageDown |
| `pageup` † | both |  |  |  | viewPageUp |  |  | blockPageUp | scrollPageUp |
...
| `s+tab` † | both |  |  |  | viewPrevSection |  | focusTranscript | entryPrev |  |
| `tab` † | both |  | menuNext |  | viewNextSection |  | complete | entryNext |  |
