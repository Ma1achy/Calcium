...
| **R-PTR-002** — Remembered focus re-resolves | `owed` | **the tree took the other road, and that is the finding rather than a gap.** `construct.ts:2970` makes pointer parity *structural rather than remembered* — nothing stores a location the keyboard could not — so there is no remembered-focus store to re-resolve. Whether this rule has a subject at all depends on a view that departs and returns, which is what M9 removed when `kind: "view"` was retired (R-EXA-082). Resolving it means deciding whether the `child` rung's surface is that view |
