...
| D7 | Popping a pushed view returns to a live block with selection preserved. **Amended during C16's specification**: the original wording also promised "and leaves a one-line trace", and the two clauses could never both hold. A trace is an entry; an entry freezes its predecessor (C13 §4 step 2, C13 I2); a frozen entry is unfocusable by construction (C13 T4.6). So the trace destroyed the selection the same sentence promised. **The trace loses**, on the trade rather than the mechanism: it costs the user nothing recoverable, while a cleared selection is unrecoverable and buys a re-navigation. Dropping it also removes the one place two mechanisms were mixed for one step — B03 commitment 1 says append and push are never mixed, and the pop's append was the exception. **The cost was accepted, not absent, and it was checked rather than assumed.** The tempting argument is that the trace was redundant because the command opening the view already appended — that argument is false, and was made twice before B03 §2 was read: `--logs` and `/dashboard` sit in that table's **Push** column, whose effect is "the transcript is untouched", and B03 commitment 1 forbids mixing the two mechanisms in one step. So a push appends nothing, the trace was the *only* record, and a logs excursion is now unrecorded — the archived O4's "a session should read end to end" no longer holds for the two pushed views. Whether that is acceptable is a genuine open question. **If it turns out to matter, the fix is that a push should append, which is a change to B03 §2's table and not a reversal of this row** — the trace can never come back, because it would freeze the block it returns to |
...
| D31 | Dimensions read as one snapshot, the sole input to a frame. **Resize is not debounced; it is coalesced on a fixed 16 ms deadline** (C03 I15, amended 2026-09-01) — the first clause is untouched and the second was reworded on a measurement. **A debounce and a fixed window are not the same thing, and the difference is this decision's own purpose**: a debounce delays the frame for as long as events keep arriving, so a three-second drag draws nothing for three seconds, where a fixed deadline draws throughout and the screen is at most one window stale. What D31 protects — *dragging an edge stays continuously correct* — survives the second and not the first. Measured: a 30-event drag wrote 30 frames and now writes 8, each re-measuring the whole transcript because width invalidates every cached height, **544 ms of blocking work at a thousand entries** (F423). The literal wording forbade the fix and the reason behind it asked for it |
...
| D34 | Mouse on by default; `/mouse` toggles; native selection is mandatory; Shift-drag documented as the native-selection bypass |
