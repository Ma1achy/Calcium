...
**And the deciding reason was windowing, until it stopped being true.** This paragraph read *`logs` is the only kind in the tree that implements `window` … and `raw` has none. A surface built to scroll a long log specified the one kind that cannot be scrolled.* `Raw.window` landed on **2026-09-04** and **seven** kinds window today — `keyValue`, `table`, `logs`, `code`, `patch`, `terminal`, `raw` — measured through the registry rather than by grepping the block-kinds directory, which returns five because two of the seven register from other components (F1055). The sentence is replaced rather than caveated: a refusal accumulates reasons and nobody re-reads the stack.

**What survives is appearance, and it is enough.** The figure has three columns, which is `Logs`'s shape exactly, and `levelTone` has one caller — the `logs` renderer — so the same three strings through `raw` draw `WARN` in body colour. Windowing is no longer part of the argument; the re-aligned record and the level tone are.

**What this does not license, and it is why the block looked unused.** `ViewPatch` has `append` (a whole block), `replace`, `merge` (table rows), `status` and `expand` — **there is no operation that appends a line to a `logs` block.** A bounded view like this one is unaffected: it fetches, builds the block once, and windows it. A *streaming* view is not — growing a `logs` block means `replace` with a larger one every tick, re-sending every line each time. **Two sentences that stood here are false, and measuring them is what F1065 asked for.** The first was *which is why the reference app's `/logs -f` emits one `raw` block per line and is right to*: at 8 000 ticks the `raw` arm costs **5 138 ms** against the whole-block replace's **496 ms**, ten times worse, because it grows the *document* and every patch walks that instead. The second was *the incremental operation is owed*: under deep immutability an append **is** a copy, so a line-append op would save the producer's array copy — **36 ms** across those 8 000 ticks — and nothing else. What was actually costing was `deepFreeze` re-walking already-frozen lines on every patch, now memoised and walking arrays by index: **4 502 ms to 496 ms**, 9×, with no change to any vocabulary (C04 I1, F1065, F1078). The residue is the shape rather than the op: both arms stay quadratic, and a genuinely unbounded stream wants a bounded block rather than a new operation.
