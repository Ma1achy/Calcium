...
**The fence is a diagram, and its box is not rendered.** The outer border marks where the region begins and ends; Calcium draws no frame around anything (S01 §3 — header, viewport, prompt, footer, and nothing between them). Counting the border gives two rows the terminal does not have, and the side rails give two cells every row does not have.
