...
| M20 | leak counting converges **to one short, and the one is not noise** | `FinalizationRegistry` over 1 000 objects: **999 finalised** under `--expose-gc` after **one** collection, and further collections change nothing. The survivor is always the **last registered** — measured at N = 1, 2, 10, 100, 1 000, 5 000, and over eight repeats at 1 000. **Controls**: dropped without a forced collection **0**; still referenced with one forced **0**. **And the floor is one object in the whole report, not one per class**: three names through one registry, 50 each, gave `alpha` 50 · `beta` 50 · `gamma` **49** — the short one is whichever registered last. This row read *1 000 finalised after two collections* until it was re-measured before anything was built on it (F893) | here; the leak row, which asserts `N − 1` with the floor named — an unexplained off-by-one is what the next reader fixes |
...
|---|---|---|---|
| D1 | the dashboard open at tier `off` | R9 × R5 | opening the view **raises** the tier and closing it lowers it. A pane at `off` draws a notice, never an empty plot — an empty plot reads as *measured, and zero*. **Corrected 2026-09-07 (F895): the condition is not the tier.** This row's cell was implemented as written and C28 I23 states it wider — *a pane with **no data*** — which is the right rule and was never anyone's task. An empty **ring** at tier `spans` produced exactly the picture this row forbids: `latency` is emitted whenever the tier is spanning, so it is present at `count: 0` and `overview` drew four zero bars, `distribution` five. Two panes were repaired, a third was printing *raise the tier* to a reader already on `spans`, and the axis to guard on is whether the ring holds anything |
