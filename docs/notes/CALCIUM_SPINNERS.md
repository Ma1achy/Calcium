...
![Twenty-seven spinner sets in three columns, each a loading line beside the set's name, each turning at its own interval](../media/spinner-sets.gif)
...
list. One frame per 80 ms `TICK_MS` tick, for as many ticks as the longest set's cycle takes
(`agent`'s, today); **each set steps at its own `intervalMs`** (C09 I112), so a set shows the
same cadence here as it does alone. Until I112 every set advanced one frame per tick, because
the session's tick was a frame counter, and `agent` walked at 80 ms where its interval says
120. The
