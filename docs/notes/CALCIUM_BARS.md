...
## The gallery — nine styles, four fills, three arms

![Nine bar styles at 0, 33, 66 and 100 percent, drawn three times: full capabilities, unicode ascii where every style is hash and dot, and ambiguousWidth wide where every narrow-only style falls to ASCII and braille stays](../media/bar-styles.png)

`docs/media/bar-styles.png`, generated by `tools/animation-proof.mjs` from `barStyleNames()`
(C24 §6). The three arms are the ladder in §Degradation drawn rather than described: the top is
the full arm, the middle is `unicode: "ascii"`, and the bottom is `ambiguousWidth: "wide"` — where
eight styles fall to `#` and `.` and `braille`, the one Neutral style, is the one that stays. The
plots demo's `/bars` draws the top arm live, and the other two with `--ambiguous-wide` or an ASCII
locale.
