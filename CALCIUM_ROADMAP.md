...
| 20 | BUILT | `visible: (host: RefreshHost) => boolean`, `src/shell/refresh.ts:344`, wired at `src/shell/construct.ts:1399` | — |
...
| 25 | PART | drawn: `src/shell/paint.ts:444` reads `ghost()` fresh per paint (I50) | sole-candidate only, and static — `ghost()` in `src/interaction/completion/engine.ts` returns a hint only when `staticCandidates` has exactly one. The *design with 40* dependency expired: 40 is BUILT (2026-09-03) |
...
`Cameras.key` (`src/shell/cameras.ts:126`) and the orbit bindings (`src/shell/construct.ts:2036`, `orbitBlock`).
