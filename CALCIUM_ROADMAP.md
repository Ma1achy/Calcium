...
| 25 | PART | drawn: `src/shell/paint.ts:728` reads `ghost()` fresh per paint (I50) | sole-candidate only, and static — `ghost()` in `src/interaction/completion/engine.ts` returns a hint only when `staticCandidates` has exactly one. The *design with 40* dependency expired: 40 is BUILT (2026-09-03) |
