...
logs       WINDOWS   presentation/blocks/kinds/structured.ts:320
code       kept whole
table      kept whole
keyValue   kept whole
...
**So the work is three windows, not a mechanism**: `code`, `table`, `keyValue` — **all three landed** (`table` 2026-08, `keyValue` at `structured.ts:163`, `code` with `raw` on 2026-09-04 under C14 §4a; `lineRange` is `code`'s pin). `windowPatch`
...
- And `keymap.ts:222` already binds `{name: "a", ctrl: true}` → `home` on the prompt.
...
carries both machines — bracketed and heuristic — and a `PasteState`, and `keys.ts:1159` already
...
| `shell/keys.ts:768`, `:1012` | `history.previous(text)`, `searchOpen(text)` — **C20 stores strings** |
...
BUILT 16 ONE POPUP ★             confirm · completion · peek · question are one mechanism with
                                   ...
                                   `{kind: "centred"}` at `src/shell/confirm.ts:265` and the
...
PART  30 paste as a chip          Claude Code's idea; Calcium can reference a BLOCK rather than
                                   ...
                                   nine sites, so a structured buffer is a change to C19, C20,
                                   ...
                                   it was three until 2026-09-11: `applyEdit` slices `before` at
                                   `edit.start`/`edit.end`, offsets C19 computed against the
                                   unresolved string, so a resolving getter does not read at a
                                   wrong offset — it WRITES at one, and it is the same member
                                   F1092 called the row nobody listed (F1103). A CHIP RESOLVES TO
                                   ITS CONTENT AT SUBMISSION, and the tree makes that forced
                                   rather than likely: `construct.ts:2525` hands C23 a string,
...
| 8 | BUILT | **C14 I4/I5/I6.** `src/viewport/viewport/viewport.ts:387` — `#afterContent()` restores from the anchor on **every** content change, not only on resize; `T5.3` is the tier-5 row — *a `--logs` tail at 1,000 lines/s while scrolled up → the view does not move* | the floating jump-to-bottom indicator, which is chrome and belongs to 29 |
| 10 | PART | `ask: (opts: AskOptions) => Promise<string>` with `choices` — `src/shell/local/registry.ts:59`, reached as `ctx.ask` at `src/shell/execution.ts:1091` | the in-transcript menu block, and the popup unification (16) |
| 12 | BUILT | `src/shell/render-frame.ts:183` `body()` — `previous()`, per-row `cursorTo(i, 0)`, `SGR_RESET` per row (I57) | — |
...
| 38 | BUILT | `Group` ships with `direction: "row" \| "column"` — `Group.direction` in `src/data/viewmodel/types.ts`, `b.group`, `groupDefinition` in `src/presentation/blocks/kinds/containers.ts` — and the weights C04 §3 deferred are built: `flex?: readonly Share[]` on `Group` in `src/data/viewmodel/types.ts`, `groupChildWidths` at `src/data/viewmodel/measure.ts:79`, refused at `b.group` and by `checkFlex` — `src/data/viewmodel/validate.ts:1968`. C04 §3, I42, I43, commitments 39–40, T1.20, T3.16–T3.19, T6.20–T6.22; `tools/mutate/runs/c04-weights.mjs` — and `Share` closes it: `examples/docker/src/banner.ts:199` builds a row group whose frame is identical to the composed golden, and `examples/docker/src/dashboard.ts:450` puts it in the document, so **`direction: "row"` has a caller**. C04 I44, commitment 41, T3.20, T3.21, T6.23, T6.24 | **CHECKED FROM THE SATISFIER'S SIDE 2026-08-15 AND TWO OF THE THREE HAVE MOVED.** `align` **is built and this row said it was not**: `Valign` at `src/data/viewmodel/types.ts`, `align?: readonly Valign[]` on `Group`, accepted by `b.group` and read by `groupDefinition` — `block.align?.[index]` into `justifyContent`. **And no test names it**: zero matches for `Valign` or `align` under `test/` for a group, so a published field with a renderer shipped untested while the record said it did not exist — two documents wrong about one field in opposite directions. T3.22 is the row it lacked. **`height: "fill"`'s blocker is met**, which is the deferral CLAUDE.md already names: the condition was *the producer cannot see the height*, and `ProducerContext.height` is granted at `producerContext(deps.region().height)` in `src/shell/execution.ts`, non-null exactly on the view route and `null` for a live part inside one. **Both line citations were already wrong before they were corrected 2026-09-01** — 580 pointed at a doc comment and 612 at a member of the plot-form union, neither of them the field named — and `roadmap-status` only noticed when an unrelated edit pushed 612 onto a *blank* line. That is F401a's class: a line citation passes on non-blankness, so it is wrong silently until the file shortens under it. **And it has now rotted a third time, in the other direction and for a reason with nothing to do with groups**: widening `RampAnimation` from six members to twenty-four (R-MOT-012) added sixty lines above it in the same file, and `flex`'s citation landed on a blank — so both were re-taken 2026-09-21 at `:2947` and `:2968`. **A fourth rot, 2026-09-23 — and this time the argument for keeping the line citation did not hold.** Adding `Progress.painted` (C09 I96) put fourteen lines into the same file and `roadmap-status` reported `:2947` blank. Re-taking it showed the citation had **already** been wrong: at the previous commit `:2947` was `newNo?: number` on `Hunk`, **157 lines** above the `direction` member it names, and the sweep had been green on it throughout. So the non-blankness check did not catch this rot each time — it caught it 157 lines late, when an unrelated edit happened to land the number on whitespace. Both are re-taken at `:3104` and `:3125`, and the claim that the check is what makes a line citation safe is withdrawn: it is what makes a line citation *eventually* noisy. Three rots on one cell, every one of them caused by an edit elsewhere in a 3 000-line file, is the argument for citing the symbol rather than the line. **A fifth rot, 2026-09-23, and this time the argument was taken.** Widening `RampFill` to four members (C04 I106, §037) put twenty lines into the same file and `:3104` landed on a blank again — the fourth edit in five weeks to move this cell, none of them about groups. The sentence above concluded *cite the symbol rather than the line* and then kept both line citations, which is a ruling that lands as prose and not as a change. Both are now bare-file citations naming `Group.direction` and `Group.flex`, so the cell resolves against what it is about and an edit above it moves nothing. The `measure.ts` and `validate.ts` line citations are left as they are: neither file has moved under this cell, and a rule is earned per instance here rather than swept. The type name was stale too — `flex` is `readonly Share[]`, not `readonly number[]`. **`padding` is the only one of the three still a separate step.** The wordmark's leading blank row stays the app's until `padding` lands, and that is now the whole of the remainder |
| 35 | RULED | **the ruling is in the entry.** The spinner is one frame by a premise that has expired — `src/shell/paint.ts:161` says a ticker is *"a timer this layer does not own and must not grow"*, and the refresh driver has owned one since 18 landed; the `steps` block already animates off `ctx.tick` (`src/presentation/blocks/kinds/structured.ts:532`). The pending entry is appended blank — `src/shell/execution.ts` `compose({ … blocks: [] })` in the shell route (re-measured 2026-09-03, still so) | nothing composes the notice, and there is no elapsed-time part — **the rendering half exists** (`continuation` `⎿` is a `GLYPH_TABLE` slot with `prefixCells` in `kinds/simple.ts`), so what is missing is the composer, not the glyph. **The adapter override has no surface yet** |
...
`Cameras.key` (`src/shell/cameras.ts:126`) and the orbit bindings (`src/shell/construct.ts:2053`, `orbitBlock`).
