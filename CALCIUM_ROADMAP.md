...
| 20 | BUILT | `visible: (host: RefreshHost) => boolean`, `src/shell/refresh.ts:344`, wired at `src/shell/construct.ts:1578` | — |
...
| 25 | PART | drawn: `src/shell/paint.ts:455` reads `ghost()` fresh per paint (I50) | sole-candidate only, and static — `ghost()` in `src/interaction/completion/engine.ts` returns a hint only when `staticCandidates` has exactly one. The *design with 40* dependency expired: 40 is BUILT (2026-09-03) |
...
| 15 | BUILT | **four steps and a step 0, and the mode is a target throughout.** Native selection: `#setNativeSelection` holds the state at `src/shell/session.ts:1343`, C03 gains `suspend`/`resume` at `src/terminal/frame-scheduler.ts:361` (§4a), C01 gains `setMouseTracking` at `src/terminal/lifecycle.ts:437` because nowhere else writes an escape. The prompt: an anchor plus the cursor, with `⌥a`/`⇧←`/`⇧Home` bound after `modifiersOf` — `src/interaction/router/decode.ts:180` — learned xterm's fourth bit. One clipboard: `copyText`, `src/interaction/editor/editor.ts:478`, written by `⌥w` and by the transcript's `copyElement`, `src/shell/keys.ts:1065`, over a range held by `extendRow`, `src/interaction/router/focus.ts:366`, copying `rowCopyText`'s source text, `src/presentation/table/definition.ts:477`. The wash is entry 23 | OSC 52 is a separate axis and is not built: whether a copy **also** reaches the system clipboard is a capability question about the terminal, and it changes nothing about where the text lands in-process |
...
`Cameras.key` (`src/shell/cameras.ts:126`) and the orbit bindings (`src/shell/construct.ts:2044`, `orbitBlock`).
