...
PART  24 more default themes      two ship and `light` WAS dark-on-dark — CLOSED by 39, which
...
| 24 | PART | **the mechanism ships and no theme uses it.** `ThemeSet` is `Readonly<Record<string, ThemeTokens>>` — `src/presentation/theme/types.ts:125` — and the store switches by name: `setTheme` at `src/presentation/theme/store.ts:104`, with `names` beside it. `validateVariant` in `src/presentation/theme/contrast.ts:224` checks the declaration against `luminance(bg)`, which nothing did. `withThemeNames` — `src/data/manifest/parse.ts:570` — supplies `/theme`'s `enum` where the composition root holds both facts, and the parse declares none, so a manifest that skips it refuses every invocation rather than quietly accepting two. The premise is closed too: `light` declares `background: "surface"` at `src/presentation/theme/tokens-light.ts:22`. C10 §5a walks it in both artefacts; I27 and I28, commitments 24–25, T1.20, T1.21, T1.21a, T2.22, T2.23, T4.35, T4.36, T6.24–T6.26; `tools/mutate/runs/c10-named-set.mjs` | **no third theme is authored**, which is the entry's content and is A01 A.1's token work. `high-contrast` is the one with an argument — the accessibility case, obliged by a five-depth degradation story more than a solarised port is |
