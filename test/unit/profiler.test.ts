...
describe("C28 — profiler, tier 1 spec-first rows", () => {
  ...
  it.todo("T1.17 (C28 I24, C28 I42): two elements measured inside entry(\"e1\") over a clock the callee advances by 2 ms and 1 ms, in a frame that waited 97 → byEntry.e1.sum is 3, and 97 appears in no byEntry row and in no byKind row. The setup is the correction: the row was written against a per-frame apportionment this round deleted, and it was unwritable because the key lacked the entry rather than the report lacking a member — not deferred on a component: lands with the recorder in src/shell/profiling/");
  it.todo("T1.67 (C28 I42): two transcript entries each holding a block with id b1, both measured → nodes holds two rows, each with its own calls, and no row reports the summed calls over either entry's frames. Both halves, because the count is the harm and the ratio is the signal — not deferred on a component: lands with the recorder in src/shell/profiling/");
  it.todo("T1.68 (C28 I42): an element measured with no entry scope open → present in nodes with no entry, counted in byKind, named by no byEntry row, and the difference between the sum of byEntry and the sum of nodes.self is that element's cost rather than zero — not deferred on a component: lands with the recorder in src/shell/profiling/");
  it.todo("T1.69 (C28 I42): the same entry scope opened around two frames' worth of work → byEntry.e1 accumulates across frames while each element keeps its own nodes row, so an entry's total and its parts are not the same number counted twice — not deferred on a component: lands with the recorder in src/shell/profiling/");
});
