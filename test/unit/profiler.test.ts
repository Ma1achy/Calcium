...
describe("C28 — profiler, tier 1 spec-first rows", () => {
  ...
  it.todo("T1.64 (C28 I41): every span found inside a frame's tree is declared `frame` in SPAN_SITE and every one opened outside a frame is declared `session`, with the unobserved names listed and the observed count asserted — not deferred on a component: lands with SPAN_SITE in src/shell/profiling/types.ts");
  it.todo("T1.65 (C28 I41): checkPhases over a report holding both populations → the session spans carry no share of latency.work and the residue is at or above zero — not deferred on a component: lands with checkPhases in src/testing/profile.ts");
  it.todo("T1.66 (C28 I41): checkPhases over an all-in-frame report → the residue is exactly work - parts - frame and the session group is empty rather than absent — not deferred on a component: lands with checkPhases in src/testing/profile.ts");
});
