...
describe("C28 §3c — the profiler's view", () => {
  ...
  it.todo("T1.113 (C28 I61): every card's draws unioned with the undrawn register equal the keys of ProfileReport, compared both ways, so a key reaching no card fails and an undrawn entry for a key the type no longer has fails too; the key list is exhaustive against keyof ProfileReport at compile time, since a hand-written corpus checked against a hand-written corpus is a tally over two populations that agree by being wrong together — not deferred on a component; it lands with this round's register");
