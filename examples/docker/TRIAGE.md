...
| **9** | [**The instrument was wrong**](#9) | 41 | **16** | — | **new at F80** · tooling — F155's instrument is not ours, and **F157's cause is the language the harness is written in** |
| **10** | [**A claim carried without a record**](#10) | 17 | **9** | — | **new at F80** · method — six findings disproved, and **F166 unblocked an entry while F168 found what the same row was silent about**; F184 is the first where the unrecorded claim was a *rule* rather than a fact, governing thirty forms from a parameter's doc comment |
...
| **F240** | **a surface slot's 4-bit answer, and whether one was ever meant to exist** | `FourBitMap` is `Readonly<Record<string, number>>` — partial over an open key — so a slot with no entry resolves to `NO_STYLE`, which is also what a slot *deliberately* unpainted resolves to. **Three of ten are unanswered in all three themes**: `selection`, `errorGround`, `errorInk`. The tag I32 calls *the only painted run* is the only **un**painted one at 4-bit, and at 1-bit it goes further and cancels the emphasis its own rule carries, because `withBackground(NO_STYLE, NO_STYLE)` is `{}` rather than the tone. C10 §3's *"Surfaces degrade too"* **enumerates** the five slots that existed when it was written; five landed after and two were given arms. **Three missing arms are probably three cases** — `selection` is a wash behind text it does not own, the pair is a tag that brings its own ink — and lumping them ships the wrong fix for two. **The pair is fixed**; `selection` is roadmap 23's and is named rather than folded in |
...
## 10 · A claim carried without a record — **9: five findings disproved, three claims caught before anything was built on them, one rule that governed thirty forms** · new at F80
