...
| **9** | [**The instrument was wrong**](#9) | 28 | **12** | — | **new at F80** · tooling — F155's instrument is not ours, and **F157's cause is the language the harness is written in** |
...
|---|---|---|
| **F200** | **the golden corpus, for the defect written into it** | `slope · full · 80` sat in the corpus containing `⠭⠭⠝⠛⠛⠛⠛⠛⠛⠓` — six- and seven-dot cells through a chart whose curves are one and two dots thick everywhere else — and `radar · full · 80` had `⣯`, `⠿`, `⣤`, `⡷` in a figure of thin strokes. Both are F199 drawn as text, legible with no tooling, past review and commit. **Third instance of `-u` writing a record**, and the first where the defect is legible in the snapshot itself rather than needing a second frame beside it: *a diff is read and a file is not*, and the rows were correct the day they were written. **No remedy proposed** — a rule that flags dense braille beside sparse braille is a heuristic about drawings, and reading the corpus is something a person does deliberately or not at all |
